...
test('LE TEST QUI COMPTE : tarifer n ouvre PAS — regle Channex « only send changes »', () => {
  // ⚠ RAPPELEE PAR THIERRY LE 12 SEPTEMBRE 2026, contre mon correctif.
  // J'avais complete `availability` pour toute date tarifee dont l'intention
  // etait ouverte, afin de reparer « le prix part mais la date reste fermee ».
  // C'etaient DEUX fautes : emettre un champ que l'hote n'a pas touche — ce
  // que la certification Channex interdit (#13) — et SUPPOSER que tarifer une
  ...
  // Ce qui reste : on le DIT. Une nuit tarifee qui reste fermee est
  // invendable, et l'ecran affichait « Enregistre et publie » sans rien
  // signaler.
  assert.ok(/tarifeesMaisFermees/.test(cal))
  assert.ok(/reste.*FERMEE/.test(cal), 'l avertissement nomme l etat reel')
  assert.ok(/passez la disponibilite sur « Ouvert »/.test(cal),
    'et dit le geste exact qui resout')
  assert.ok(/if \(availByDate\[ds\] != null\) continue/.test(cal),
    'un choix explicite de l hote ne declenche pas l avertissement')
})
