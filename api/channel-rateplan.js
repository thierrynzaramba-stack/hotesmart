    ...
    if (hasMinStay) {
      ...
      try {
        ...
        const { data: bien } = await supabase.from('properties')
          .select('id, user_id, name, provider, migration_target_property_id, inventory_units, provider_property_id, provider_rate_plan_id, provider_room_type_id, base_price, capacity, included_guests, extra_guest_fee')
