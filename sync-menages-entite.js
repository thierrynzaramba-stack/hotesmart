...
async function synchroniserMenages (results = null, opts = {}) {
  ...
  for (const l of lignes) {
    ...
    if (!isActiveStatus(l.snapshot, providerParBien.get(`${l.user_id}|${String(l.property_id)}`))) continue
    vivants.add(k)
  ...
  for (const m of (existants || [])) {
    if (m.provider_id || m.assigned_by === 'manual' || m.status === 'cancelled') continue
  ...
  if (aRessusciter.length) {
    const { error: errRes } = await supabase.from('menages')
      .update({ status: 'unassigned', updated_at: new Date(maintenant).toISOString() })
      .in('id', aRessusciter.map(m => m.id))
    if (errRes) { console.error('[sync-menages-entite] resurrection echec:', errRes.message); bilan.erreurs++ }
    else {
      bilan.ressuscites = aRessusciter.length
  ...
  for (const { menage, choix } of aReassigner) {
    ...
    bilan.assignes_apres_coup = (bilan.assignes_apres_coup || 0) + 1
