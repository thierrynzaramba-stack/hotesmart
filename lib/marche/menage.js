...
function menageFacture (raw) {
  const r = raw || {}
  // Une CHARGE seulement (review) : une ligne de paiement ou de remboursement
  // decrite « menage » n'est pas un frais facture. Type absent : on garde.
  for (const i of r.invoiceItems || []) {
    if (i && (i.type == null || i.type === 'charge') && MOT.test(String(i.description || '')) && positif(i.lineTotal ?? i.amount)) return true
  }
  // Les deux mots (« Cleaning », « Ménage »), et jamais un montant NEGATIF :
  // « Cleaning -45 » est une remise, pas un menage facture (review).
  const m = String(r.rateDescription || '').match(/(?:cleaning|m[ée]nage)[^\d-]{0,20}(-?)(\d+(?:[.,]\d+)?)/i)
  if (m && !m[1] && positif(m[2].replace(',', '.'))) return true
