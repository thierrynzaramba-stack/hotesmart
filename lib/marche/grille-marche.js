...
//     du poids (40 % des 5 comparables) — sinon « reference amincie » : les
//     niveaux QUAND MEME, marques, avec les nombres reels et les motifs
//     (Thierry, 24 septembre 2026 : ne rien montrer, c'est repondre « non » a
//     une question dont la reponse est « je ne sais pas ») ;
//   - gestionnaire dominant : AVERTISSEMENT, jamais bloquant (regle 17) ;
//   - stabilite : prix pondere 12 derniers mois contre 12 precedents, sur les
//     MEMES MOIS que les niveaux (mois a moins de 5 nuits ecartes — Thierry,
//     24 septembre 2026 : deux filtres sur la meme donnee produiraient un jour
//     un ecart que personne ne saurait expliquer) ; > 20 % = « niveau
//     instable », MONTRE, jamais exclu ; < 6 mois retenus d'un cote =
//     « stabilite non mesurable ».
...
/**
 * ⚠ LA FENETRE DES DONNEES, PAS CELLE DU CALENDRIER — releve en review (V2.5,
 * bloquant). Les metriques en cache ont jusqu'a 90 jours : calculee depuis la
 * date du jour, la fenetre perdait ses derniers mois SANS BRUIT (releve du
 * 20 decembre sur un cache arrete a aout : 9 mois sur 12), pendant que la
 * mesuree 12 mois en lisait 12 — deux periodes differentes, un ecart faux,
 * et un releve en ajout seul qu'on ne refait pas.
 * La fenetre finit donc au DERNIER MOIS PRESENT CHEZ TOUS les comparables (et
 * jamais apres le dernier mois complet) ; elle est rendue, et le controle la
 * passe telle quelle a la mesuree.
 */
function fenetreDesDonnees (comparables, aujourdHui) {
  const calendrier = fenetreDouzeMois(aujourdHui)
  const derniers = (comparables || []).map(c => (c.mensuel || []).map(l => moisDe(l && l.date))
    .filter(m => estMois(m) && m <= calendrier.fin).sort().pop()).filter(Boolean)
  if (!derniers.length) return { ...calendrier, retard_mois: null }
  const fin = derniers.sort()[0]
  const retard = (Number(calendrier.fin.slice(0, 4)) - Number(fin.slice(0, 4))) * 12 +
    (Number(calendrier.fin.slice(5, 7)) - Number(fin.slice(5, 7)))
  return { debut: decalerMois(fin, -11), fin, retard_mois: retard }
}
...
function stabilite (mensuel, fenetre) {
  ...
  const avec = mois => mois.filter(x => x.nuits >= MIN_NUITS_MOIS)
  ...
  if (recent.length < STABILITE.moisMin || avant.length < STABILITE.moisMin) {
    return { statut: 'non_mesurable', mois_recents: recent.length, mois_precedents: avant.length,
      phrase: `stabilité non mesurable (${Math.min(recent.length, avant.length)} mois d’au moins ${MIN_NUITS_MOIS} nuits d’un côté, il en faut ${STABILITE.moisMin})` }
  }
...
function grilleMarche ({ comparables = [], aujourdHui, prixMinimum = null } = {}) {
  const { retard_mois: retard, ...fenetre } = fenetreDesDonnees(comparables, aujourdHui)
  ...
  // ⚠ Le poids BRUT juge le seuil (un 50,04 % arrondi a 50,0 % passait —
  // review) ; l'arrondi ne sert qu'a l'affichage.
  for (const c of lus) { c.poidsBrut = nuits > 0 ? c.nuits / nuits : 0; c.poids = Math.round(c.poidsBrut * 1000) / 1000 }
  ...
  const plusLourd = avecNuits.reduce((m, c) => (!m || c.poidsBrut > m.poidsBrut ? c : m), null)
  const motifs = []
  ...
  if (plusLourd && plusLourd.poidsBrut > poidsMax) {
  ...
  if (retard) {
    avertissements.push({ type: 'donnees_anciennes', comparables: [],
      phrase: `Les données du marché s’arrêtent ${retard} mois avant le dernier mois complet : la grille porte sur la période qu’elles couvrent (${fenetre.debut} → ${fenetre.fin}).` })
  }
  for (const c of lus) {
    const presents = new Set((comparables.find(x => String(x.listing_id) === c.listing_id)?.mensuel || [])
      .map(l => moisDe(l && l.date)).filter(m => m >= fenetre.debut && m <= fenetre.fin))
    if (presents.size < 12) {
      avertissements.push({ type: 'donnees_incompletes', comparables: [c.listing_id],
        phrase: `« ${c.nom || c.listing_id} » : ${12 - presents.size} mois sans donnée sur la période — il pèse selon les mois connus.` })
    }
  }
  ...
  // ⚠ REFERENCE AMINCIE : les niveaux se calculent QUAND MEME, par la meme
  // regle, et le statut les marque. Le seuil de la V1 en reservations ne
  // s'applique pas ici : un comparable n'est pas une reservation, et le seuil
  // du marche (comparables, nuits, poids) a deja dit ce qu'il avait a dire.
  // Seule limite gardee : le seuil de la V1 en NUITS — en dessous, il n'y a
  // pas de quantile qui veuille dire quelque chose, et on le dit.
  const g = grilleDeBase(prix, { reservations: avecNuits.length, seuilResas: 1 })
  if (!g.fiable) {
    return { ...base, statut: 'reference_amincie', niveaux: null,
      motifs: [...motifs, `${prix.length} nuits : trop peu pour calculer des niveaux`] }
  }
  const plancher = Number.isFinite(Number(prixMinimum)) && Number(prixMinimum) > 0 ? Number(prixMinimum) : null
  // Les drapeaux de construction VOYAGENT (review) : un releve en ajout seul,
  // qu'on ne recalcule pas, doit dire si un ecart vient d'un etirement ou d'un
  // niveau confondu, pas de la methode.
  const niveaux = g.niveaux.map(n => ({ nom: n.nom, prix: n.prix, prix_mesure: n.prix_mesure, etire: !!n.etire,
    confondu_avec: n.confondu_avec || null, sous_plancher: plancher != null && n.prix < plancher }))
  if (niveaux.some(n => n.sous_plancher)) {
    avertissements.push({ type: 'sous_plancher', comparables: [],
      phrase: `Le marché place ${niveaux.filter(n => n.sous_plancher).map(n => n.nom).join(', ')} sous votre prix plancher (${String(Math.round(plancher * 100) / 100).replace('.', ',')} €) : on ne descend pas.` })
  }
  return { ...base, statut: motifs.length ? 'reference_amincie' : 'fiable', niveaux, min: g.min, max: g.max }
}

module.exports = { grilleMarche, fenetreDouzeMois, fenetreDesDonnees, nuitsDuMois, moisUtiles, stabilite, gestionnaires, SEUIL, STABILITE, MIN_NUITS_MOIS }
