...
function expliquerMarche ({ calendrier, pacing, vacances } = {}) {
  ...
  const evenementsPossibles = evenements.filter(e => e.jours.length >= EVENEMENT_MIN_JOURS).map(e => {
    ...
    return {
      ...
      a_lire: !pic || !pic.faiblement_marque,
      motif_non_affiche: pic && pic.faiblement_marque
        ? 'pic faiblement marqué : aucune de ses frontières n’est une rupture (force < ×1,2)' : null,
...
module.exports = { expliquerMarche, calendrierFrancais, causesDuJour, jugerEcart, PART_EXPLIQUEE, SURCROIT, EVENEMENT_MIN_JOURS, NUITS_MIN_COTE, NUITS_RESERVEES_MIN }
