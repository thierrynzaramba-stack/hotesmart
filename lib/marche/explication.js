...
/**
 * UN EVENEMENT POSSIBLE EST-IL A LIRE ? (Thierry, 24 septembre 2026 ; regle
 * choisie seule.) Une liste a lire ne contient que ce qui pourrait changer une
 * decision. Masque SEULEMENT un evenement dont le pic est borne par DEUX
 * transitions MESUREES (aucune rupture ≥ ×1,2) ET qui ne porte aucun jour en
 * surcroit. Un bord de fenetre (`debut`, `fin`) n'est pas mesure : « je ne
 * sais pas » n'est pas « non » — le pic reste a lire (review). Un surcroit
 * local reste a lire meme dans un pic faible (review).
 * @returns { a_lire, motif_non_affiche }
 */
function jugerALire (pic, joursEnSurcroit) {
  const masque = !!(pic && pic.entree === 'transition' && pic.sortie === 'transition' && !joursEnSurcroit)
  return { a_lire: !masque,
    motif_non_affiche: masque ? 'pic faiblement marqué : ses deux frontières sont des transitions (force < ×1,2), sans surcroît local' : null }
}
...
/**
 * L'ecart d'UNE periode, PUR : `semaine` et `we` sont des listes de jours
 * { prix, remplissage, n (nuits reservees) }. Sous l'un des seuils : aucun
 * chiffre, le motif seulement (Thierry : « l'ecart negatif sur faible
 * effectif n'est pas un resultat, c'est du bruit »).
 */
function jugerEcart (semaine, we) {
  const moyenne = (l, k) => l.reduce((t, x) => t + x[k], 0) / l.length
  const base = { nuits_semaine: semaine.length, nuits_week_end: we.length,
...
function expliquerMarche ({ calendrier, pacing, vacances } = {}) {
  ...
  // La zone de derniere minute n'est pas interpretee : ni pic, ni surcroit.
  const dm = calendrier.derniere_minute
  const presents = joursEntre(debut, fin).filter(d => lu.jours.has(d) && !(dm && d >= dm.debut && d <= dm.fin))

  // 1. Les pics : les saisons hautes du pacing (forte, tres forte), chacune
  //    lue dans le calendrier.
  // ⚠ UN PIC BORNE PAR DEUX TRANSITIONS le dit (review) : aucune rupture
  // franche ne l'encadre — une saison faiblement marquee, pas un saut.
  const pics = calendrier.saisons.filter(s => s.saison === 'forte' || s.saison === 'tres_forte').map(s => {
    const faible = s.entree !== 'rupture' && s.sortie !== 'rupture'
    ...
    return {
      ...
      faiblement_marque: faible, entree: s.entree || null, sortie: s.sortie || null,
      ...
      phrase: (causes.length
        ...
        (faible ? ' Saison faiblement marquée : aucune rupture franche ne l’encadre, seulement des transitions.' : '')
    }
  })
  ...
  for (const d of presents) {
    ...
    for (let k = -VOISINAGE; k <= VOISINAGE; k++) {
      const v = decaler(d, k)
      // Les jours de la zone de derniere minute ne servent pas non plus de
      // VOISINS : leur remplissage gonfle la reference (review).
      if (k === 0 || v > fin || !lu.jours.has(v) || estNuitDeWeekEnd(v) !== we || causesDuJour(v, cal).length || (dm && v >= dm.debut && v <= dm.fin)) continue
  ...
  const evenementsPossibles = evenements.filter(e => e.jours.length >= EVENEMENT_MIN_JOURS).map(e => {
    ...
    return {
      ...
      // ⚠ UN PIC FAIBLEMENT MARQUE N'ENTRE PAS DANS LA LISTE A LIRE (Thierry,
      // 24 septembre 2026 ; regle choisie seule) : une liste a lire ne
      // contient que ce qui pourrait changer une decision — meme regle que
      // l'ecart week-end sur faible effectif. Un pic n'y entre que si AU MOINS
      // UNE de ses deux frontieres est une rupture (force >= ×1,2) ; sinon il
      // reste dans la donnee stockee (`a_lire: false`, avec son motif), jamais
      // affiche par defaut. Bagneres : le 2 → 11 octobre (×1,06 / ×1,13) sort.
      // Alternative : exiger que LES DEUX frontieres soient des ruptures.
      ...jugerALire(pic, rapports.length),
  ...
  // +7,5 % (octobre) et +8,5 % (fin octobre - mi-decembre) plus cher ; a
  // quatre et six mois, janvier et mars n'ont que 226 et 213 nuits de
  // week-end reservees (4 et 5 dates) : non calculables. Une mediane par nom
  // melangerait le proche et le lointain : faux. L'ecart se donne PERIODE PAR
  // PERIODE, avec sa distance a la capture.
...
module.exports = { expliquerMarche, calendrierFrancais, causesDuJour, jugerEcart, jugerALire, PART_EXPLIQUEE, SURCROIT, EVENEMENT_MIN_JOURS, NUITS_MIN_COTE, NUITS_RESERVEES_MIN }
