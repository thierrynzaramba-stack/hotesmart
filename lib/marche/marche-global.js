...
function adrOccupationMensuel (marche60) {
  ...
  if (!mois.some(x => x.adr != null) || !mois.some(x => x.occupation != null)) {
    return { statut: 'non_calculable', motif: 'ni ADR ni occupation mesures dans l historique', mois, profil: [], ecartes: base.ecartes }
  }
...
function poidsDuJour (iso, vacances, feries, ponts) {
  const zones = new Set(vacances.filter(v => v && iso >= v.date_debut && iso <= v.date_fin && ZONES.includes(v.zone)).map(v => v.zone))
  const raisons = []
  let w = 1
