...
const positif = v => (typeof v === 'number' && Number.isFinite(v) && v > 0 ? v : null)
...
function revparMensuel (marche60) {
  ...
  for (const l of results) {
    ...
    mois.push({
      mois: m,
      ...Object.fromEntries(QUANTILES_REVPAR.map(q => [q, positif(Number(r[q]))])),
      annonces: positif(Number(l.active_listings_count)),
