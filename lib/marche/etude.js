...
const { GARDES } = require('../airroi/cout')
...
async function etudierBien ({ supabase, client, bien, listingIds = null }) {
  ...
  // ⚠ LE COUT DE L'ETUDE SE JUGE AVANT LE PREMIER APPEL (review) : deux appels
  // par comparable (fiche + mois, 0,20 $). Au-dela du plafond par bien, on
  // refuse d'emblee, au lieu de s'arreter au milieu apres avoir paye.
  const appels = ids.flatMap(id => [
    { endpoint: 'GET /listings', params: { listing_id: String(id), currency: 'native' } },
    { endpoint: 'GET /listings/metrics/all', params: { listing_id: String(id), num_months: 60, currency: 'native' } }])
  const estime = await client.estimer(appels)
  if (estime > GARDES.plafondBien90jUsd) {
    return { refus: 'etude_trop_chere', message: `Cette étude coûterait ${estime.toFixed(2)} $ (${ids.length} comparables), au-delà du plafond par logement (${GARDES.plafondBien90jUsd} $).` }
  }
  const comparables = []
  const dates = []
  for (const id of ids) {
