...
const { TARIFS } = require('../airroi/cout')
...
async function etudierBien ({ supabase, client, bien, listingIds = null }) {
  ...
  // ⚠ LA MARGE, PAS LE PLAFOND BRUT (review du 24 septembre) : ce qui reste
  // sous le plus petit des trois garde-fous (bien 90 jours, compte 30 jours,
  // budget du mois), deja depense deduit. Juge contre le plafond brut, un
  // compte qui avait deja depense passait l'estimation, puis `plafond_compte`
  // tombait au milieu des mois, apres paiement.
  const plafond = await client.marge(ctx)
  const trop = (usd) => ({ refus: 'etude_trop_chere', message: `Cette étude coûterait ${usd.toFixed(2)} $ (${ids.length} comparables), au-delà de ce qui reste disponible (${Math.max(0, plafond).toFixed(2)} $ sous les garde-fous AirROI).` })
  ...
  // des mois — l'etude ne s'arrete jamais au milieu des mois.
  // ⚠ RELIQUAT ACCEPTE (review du 24 septembre, constat 2) : le premier
  // jugement ne connait pas encore les absents. Si la liste revele trop
  // d'absents, l'etude refuse APRES avoir paye la fiche du bien et la liste
  // (0,20 $ au plus, une fois : a la relance, les deux viennent du cache).
  // Compter d'emblee toutes les fiches (le vrai pire cas) refuserait des
  // etudes de 15 comparables presents dans la liste — le cas normal.
  ...
  if (bien.airbnb_listing_id) {
    ...
    // ⚠ TAILLE LISIBLE OU PAS DE LISTE (review) : `Number(null)` vaut 0 et
    // passerait la validation — une recherche « 0 chambre » en silence, tous
    // les retenus absents, le cout qui monte. Chambres et salles de bain :
    // des nombres (un studio a 0 chambre) ; voyageurs : au moins 1. Sinon on
    // lit les fiches une par une, comme un bien sans annonce Airbnb.
    const nombre = v => typeof v === 'number' && Number.isFinite(v)
    if (nombre(pd.bedrooms) && pd.bedrooms >= 0 && nombre(pd.baths) && pd.baths >= 0 && nombre(pd.guests) && pd.guests >= 1) {
      const recherche = { latitude: Number(bien.latitude), longitude: Number(bien.longitude),
        bedrooms: pd.bedrooms, baths: pd.baths, guests: pd.guests }
