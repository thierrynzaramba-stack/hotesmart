...
function construireLigne ({ marche, pacing, marche60 = null, vacances = [], calculeLe }) {
  ...
  // ⚠ UN CALENDRIER DE VACANCES INCOMPLET NE SE STOCKE PAS (vecu le
  // 24 septembre 2026 : la table des vacances etait VIDE en staging ; la
  // ligne ecrite donnait Noel et fevrier pour « sans cause calendaire »). En
  // ajout seul, une ligne fausse occuperait la cle d'unicite : on refuse
  // AVANT d'ecrire, avec le manque.
  if (ex.statut === 'calcule' && !(ex.couverture_calendrier && ex.couverture_calendrier.complete)) {
    throw new Error(`[calendrier-marche] vacances scolaires incompletes sur l horizon (${(ex.couverture_calendrier && ex.couverture_calendrier.manque) || 'couverture inconnue'}) : l explication serait fausse, rien n'est stocke`)
  }
  return {
    ...base,
