...
function construireLigne ({ marche, pacing, marche60 = null, vacances = [], calculeLe }) {
  ...
  // La zone de derniere minute NON MESUREE se dit dans la ligne (review) : la
  // table n'a pas de colonne pour elle, `limites` la porte.
  const limiteZone = cal.derniere_minute && cal.derniere_minute.statut === 'non_mesuree'
    ? [`Zone de dernière minute non mesurée : ${cal.derniere_minute.motif}.`] : []
  ...
  return {
    ...
    limites: [...(ex.statut === 'calcule' ? ex.limites : [ex.motif]), ...limiteZone]
  }
}
