...
function plancherAmplitude (valeurs, etiquettes) {
  ...
  // ⚠ LES NOMS SUIVENT LE NOMBRE DE SAISONS QUI RESTENT (decision prise
  // seule) : une classe jamais fusionnee garderait sinon son rang d'origine,
  // et un marche a deux niveaux montrerait une « tres forte » qui n'existe
  // pas. Quatre : les quatre noms ; trois : basse, moyenne, forte ; deux :
  // basse, forte ; une : moyenne.
  const restantes = [...new Set(e)].sort((a, b) => a - b)
  const noms = { 1: [1], 2: [0, 2], 3: [0, 1, 2], 4: [0, 1, 2, 3] }[restantes.length]
  const rang = new Map(restantes.map((c, k) => [c, noms[k]]))
  return e.map(c => rang.get(c))
}
...
function formeMensuelle (marche60, { apres = null, jusqua = null } = {}) {
  ...
  const classes = plancherAmplitude(indices.map(Math.log), indices.map(v => seuils.filter(s => v >= s).length))
