...
function zoneDerniereMinute (lu) {
  ...
  if (ref.length < 10) return { jours: [], seuil: null, reference: null, motif: 'reference trop courte' }
  const m = mediane(ref)
  const mad = Math.max(mediane(ref.map(x => Math.abs(x - m))), 0.01)
  const seuil = m + 3 * mad
  const jours = []
  ...
  return { jours, seuil: Math.round(seuil * 100) / 100, reference: { mediane: Math.round(m * 1000) / 1000, dispersion: Math.round(mad * 1000) / 1000 } }
}
...
function calendrierDuMarche (entree = {}) {
  ...
  const ruptures = frontieresToutes.filter(r => r.force != null && r.force >= RUPTURE_MIN)
  const transitions = frontieresToutes.filter(r => !(r.force != null && r.force >= RUPTURE_MIN))
  ...
  saisons.forEach((s, k) => {
    ...
    s.entree = !f ? 'debut' : (f.force != null && f.force >= RUPTURE_MIN ? 'rupture' : 'transition')
  ...
  return {
    ...
    statut: 'calcule',
    capture_le: lu.debut,
    derniere_minute: derniere.jours.length
      ? { debut: derniere.jours[0], fin: derniere.jours[derniere.jours.length - 1], jours: derniere.jours.length,
