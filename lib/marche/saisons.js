...
// ⚠ ZONE DE DERNIERE MINUTE (Thierry, 24 septembre 2026 ; regle choisie seule,
// sur mesure). Les tout premiers jours apres la capture sont deja presque
// remplis : la correction de l'eloignement, une pente reguliere, ne reproduit
// pas cette remontee et les sur-releve (une saison « forte » et une rupture a
// ×1,01 sur Bagneres). Ils ne recoivent ni saison ni rupture, et la pente
// s'estime SANS eux. MESURE, pas une duree ronde : pour chaque jour, le
// rapport nuits reservees du jour / meme jour de semaine sept jours plus
// tard ; reference = les jours REF_DERNIERE_MINUTE (semaines 2 a 4) ; seuil =
// mediane + 3 ecarts absolus medians ; la zone = les jours consecutifs depuis
// la capture au-dessus du seuil. Bagneres, 24 septembre : reference mediane
// ×1,10, dispersion 0,047, seuil ×1,24 ; jours 0-2 a ×1,25 / ×1,33 / ×1,47,
// jour 3 a ×1,16 → 3 jours (24-26 septembre). Sensibilite dite : avec les
// jours 4-31 ou 7-34 comme reference, 2 jours ou 1.
const REF_DERNIERE_MINUTE = [7, 27]
const DERNIERE_MINUTE_MAX = 14
// ⚠ FORCE D'UNE RUPTURE (Thierry, 24 septembre 2026 ; seuil choisi seul) : le
// rapport des nuits reservees trois jours apres / trois jours avant, lu dans
// le sens du plus fort (×0,73 a la force 1,37). Sous RUPTURE_MIN, la
// frontiere entre deux saisons est une TRANSITION, pas une rupture — meme
// seuil que le plancher d'amplitude (20 %).
const RUPTURE_MIN = 1.2
...
/**
 * La zone de derniere minute, MESUREE (voir REF_DERNIERE_MINUTE). Rend
 * { jours: [dates], seuil, reference } ; jours vide si rien ne depasse.
 */
function zoneDerniereMinute (lu) {
  const rapport = d => {
    const a = lu.jours.get(d)
    const b = lu.jours.get(decaler(d, 7))
    return a && b && b.reservees > 0 ? a.reservees / b.reservees : null
  }
  const ref = []
  for (let k = REF_DERNIERE_MINUTE[0]; k <= REF_DERNIERE_MINUTE[1]; k++) {
    const r = rapport(decaler(lu.debut, k))
    if (r != null) ref.push(r)
  }
  if (ref.length < 10) return { jours: [], seuil: null, reference: null, statut: 'non_mesuree', motif: 'reference trop courte (jours 7 a 27 absents ou incomplets)' }
  const m = mediane(ref)
  // Plancher RELATIF de la dispersion (review) : sur un marche sature, une
  // dispersion quasi nulle mettait le seuil a ×1,03.
  const mad = Math.max(mediane(ref.map(x => Math.abs(x - m))), 0.02 * m)
  const seuil = m + 3 * mad
  // ⚠ LA ZONE EST CONTIGUE, et s'arrete au premier jour non mesurable (trou
  // le jour meme ou sept jours plus tard) : prudent, et dit (review). Un jour
  // gonfle APRES un trou recoit donc une saison — le trou lui-meme est deja
  // dit `absent_du_pacing`.
  const jours = []
  for (let k = 0; k < DERNIERE_MINUTE_MAX; k++) {
    const d = decaler(lu.debut, k)
    const r = rapport(d)
    if (r == null || r <= seuil) break
    jours.push(d)
  }
  return { jours, statut: 'mesuree', seuil: Math.round(seuil * 100) / 100, reference: { mediane: Math.round(m * 1000) / 1000, dispersion: Math.round(mad * 1000) / 1000 } }
}
...
/**
 * Une frontiere est-elle une RUPTURE ? Force >= RUPTURE_MIN, ou saut depuis
 * / vers zero nuit (la plus forte possible, sans rapport fini). Sinon, une
 * transition. PURE, exportee pour etre testee seule : le cas « depuis zero »
 * est defensif — le calcul complet l'atteint rarement (les jours a zero nuit
 * sont ecartes du classement), et un test a travers lui passait a vide.
 */
function estRupture (r) {
  return !!(r && (r.depuis_zero || (r.force != null && r.force >= RUPTURE_MIN)))
}
 ...
 */
function calendrierDuMarche (entree = {}) {
  // ⚠ LA DETECTION EST AVEUGLE — REGLE DE METHODE (Thierry, 24 septembre
  // 2026). Saisons et ruptures se calculent sur le SEUL pacing (et la forme
  // des 60 mois), sans jamais lire le calendrier ; le calendrier n'intervient
  // qu'ENSUITE, pour expliquer un relief deja etabli (explication.js). Tant
  // que la separation tient, des ruptures qui retombent sur le 19 decembre et
  // le 13 fevrier sont une PREUVE ; si le calendrier entrait un jour dans la
  // detection, on perdrait le moyen de montrer que la methode n'a pas ete
  // reglee sur le resultat attendu. Separation STRUCTURELLE, pas d'usage :
  // toute autre cle est refusee, meme vide, meme `undefined` ; et ce module
  // n'importe rien.
  const permises = new Set(['pacing', 'marche60'])
  const intrus = Object.keys(entree || {}).filter(k => !permises.has(k))
  if (intrus.length) {
    throw new Error(`[saisons] le calendrier n'entre jamais dans la detection : entree refusee (${intrus.join(', ')})`)
  }
  const { pacing, marche60 = null } = entree || {}
  ...
  // La zone de derniere minute sort de TOUT : de la pente et du classement.
  const derniere = zoneDerniereMinute(lu)
  const enZone = new Set(derniere.jours)
  const dansHorizon = lisses.filter(l => l.present && !enZone.has(l.date) && l.date <= finHorizon && l.remplissage != null && l.remplissage > 0)
  ...
  if (!droite || dansHorizon.length < TRONCON_MIN * 2) {
    return { source: 'marche', statut: 'non_calculable', motif: derniere.jours.length && dansHorizon.length < TRONCON_MIN * 2
      ? `horizon trop court une fois la zone de derniere minute ecartee (${derniere.jours.length} jours)`
      : 'horizon trop court pour separer la saison de l eloignement',
      fenetre: { debut: lu.debut, fin: lu.fin, jours: lu.dates.length }, ecartes: lu.ecartes }
  }
  ...
  const frontieresToutes = recalees.map((j, k) => {
    const d = dates[j]
    ...
    return { date: d, datee_au_jour: auJour[k], de: saisons[k].saison, vers: saisons[k + 1].saison,
      ...
      force: avant && apres ? Math.round(Math.max(apres / avant, avant / apres) * 100) / 100 : null,
      // Un saut DEPUIS ou VERS zero nuit est la plus forte frontiere possible,
      // sans rapport fini (review : il passait pour une transition).
      depuis_zero: avant != null && apres != null && (avant === 0) !== (apres === 0) }
  })
  // Sous RUPTURE_MIN : une TRANSITION (la saison change, sans saut franc des
  // reservations) — dite a part, jamais affichee comme une rupture.
  const ruptures = frontieresToutes.filter(estRupture)
  const transitions = frontieresToutes.filter(r => !estRupture(r))
  // Chaque saison dit COMMENT elle commence : par une rupture (et sa force),
  // une transition, ou le debut de la lecture. Le stockage n'a ainsi besoin
  // d'aucune colonne neuve (la migration ne bouge pas).
  saisons.forEach((s, k) => {
    const f = k > 0 ? frontieresToutes[k - 1] : null
    s.entree = !f ? 'debut' : (estRupture(f) ? 'rupture' : 'transition')
    s.sortie = k + 1 < saisons.length ? (estRupture(frontieresToutes[k]) ? 'rupture' : 'transition') : 'fin'
    s.force_entree = f ? f.force : null
  })
  ...
  return {
    ...
    statut: 'calcule',
    capture_le: lu.debut,
    // « Je ne sais pas » n'est pas « non » (review) : une zone NON MESUREE se
    // dit, elle ne se confond pas avec une zone mesuree vide (null).
    derniere_minute: derniere.statut === 'non_mesuree'
      ? { statut: 'non_mesuree', motif: derniere.motif, jours: 0 }
      : derniere.jours.length
      ? { statut: 'mesuree', debut: derniere.jours[0], fin: derniere.jours[derniere.jours.length - 1], jours: derniere.jours.length,
        seuil: derniere.seuil, reference: derniere.reference, regime: 'derniere_minute',
        phrase: 'Zone de dernière minute, non interprétable : ces jours sont déjà presque remplis, la saison ne s’y lit pas.' }
      : null,
    ...
    regimes: [
      // La zone de derniere minute : une periode a part, non interpretee.
      ...(derniere.jours.length ? [{ debut: derniere.jours[0], fin: derniere.jours[derniere.jours.length - 1], regime: 'derniere_minute',
        phrase: 'Zone de dernière minute, non interprétable : ces jours sont déjà presque remplis, la saison ne s’y lit pas.' }] : []),
      { debut: derniere.jours.length ? decaler(derniere.jours[derniere.jours.length - 1], 1) : lu.debut, fin: finHorizon, regime: 'pacing',
...
function saisonDuJour (calendrier, jour) {
  ...
  const dm = calendrier.derniere_minute
  if (dm && jour >= dm.debut && jour <= dm.fin) return { jour, saison: null, motif: 'derniere_minute', regime: 'derniere_minute' }
...
module.exports = { calendrierDuMarche, estRupture, RegimesMelanges, REGIMES, rangDansSonRegime, comparerSaisons, ordonnerSaisons, saisonDuJour, formeMensuelle, lirePacing, lisser, SEUIL_NUITS_JOUR, NOM_SAISON, QUANTILES }
