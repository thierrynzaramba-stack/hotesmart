...
//
// ⚠ « YIELD NE TOUCHE JAMAIS UNE FERMETURE DE L'HOTE » (4.6.2). Une nuit
// couverte par une fermeture (l'objet : debut, fin, raison) est IGNOREE et
// comptee a part — `fermees_par_l_hote` — quoi qu'on demande pour elle : ni
// ouverture, ni prix. C'est la frontiere du mode auto-pilote, et elle se lit
// dans la table `fermetures`, jamais devinee depuis un `stop_sell`.
//
// ⚠ ET UNE NUIT `stop_sell` SANS fermeture n'est PAS rouverte non plus, d'ici
// le 4.6.3. Elle est fermee « calculee » (sans prix) ou fermee a la main sans
// raison : c'est au moteur d'ouverture de decider ce qu'il en fait, avec sa
// regle ecrite. En attendant, ce canal reste plus strict que la regle finale,
// jamais moins — `deja_fermees` le compte a part de `fermees_par_l_hote`,
// pour que le cron sache lequel des deux il regarde.
 ...
 * @returns { ok, refus?, message?, ecrit?, ignorees: { passees, hors_fenetre, fermees_par_l_hote, deja_fermees, invalides } }
 */
async function demanderAuCalendrier (supabase, bien, demande, deps = {}) {
  ...
  const ignorees = { hors_fenetre: [], fermees_par_l_hote: [], deja_fermees: [], invalides: [], passees: [] }
  ...
  const dates = [...new Set(nuits.map(n => jourISO(n && n.date)).filter(Boolean))].sort()
  // ⚠ LES FERMETURES DE L'HOTE D'ABORD : une lecture en echec LEVE dans
  // `fermeturesDuBien` — un vide par erreur ferait OUVRIR dans une fermeture.
  // On la transforme en refus, jamais en « aucune fermeture ».
  let parLHote = new Set()
  if (dates.length) {
    try {
      parLHote = nuitsFermees(await fermeturesDuBien(supabase, bien.id, dates[0], dates[dates.length - 1]), dates[0], dates[dates.length - 1])
    } catch (e) {
      return { ok: false, refus: REFUS.DEMANDE_INVALIDE, message: `Lecture des fermetures impossible : ${e.message}` }
    }
  }
  ...
  for (const n of nuits) {
    ...
    // ⚠ NI OUVERTURE, NI PRIX dans une fermeture de l'hote : la nuit est
    // ignoree EN BLOC, avant meme de regarder ce qu'on demande pour elle.
    if (parLHote.has(date)) { ignorees.fermees_par_l_hote.push(date); continue }
  ...
  if (!dateSegments.length) {
    return { ok: true, ecrit: null, ignorees,
      message: 'Rien à écrire : toutes les nuits demandées sont passées, hors fenêtre, fermées par l\'hôte, déjà fermées ou invalides.' }
  }
