...
function nuitsAOuvrir ({ bien, aujourdHui, lignes, fermetures, prixCalcule = null, vendues = null }) {
  ...
  for (let j = aujourdHui, n = 0; j <= fin && n < 5000; j = decaler(j, 1), n++) {
    ...
    const { prix, raison } = prixDOuverture(bien, l, prixCalcule && prixCalcule.get ? prixCalcule.get(j) : null)
...
async function ouvrirLaFenetreDuBien (supabase, bien, { aujourdHui, appel, demander = demanderAuCalendrier, prixCalcule = null, fermetures: fermeturesFournies = null, vendues: venduesFournies = null } = {}) {
  ...
  const decision = nuitsAOuvrir({ bien, aujourdHui: auj, lignes, fermetures, prixCalcule, vendues })
