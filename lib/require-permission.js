 ...
 */
async function requirePermission(req, res, { domaine, niveau = 'read', bien = null, bienRequis = false, booking = null, bookingRequis = false } = {}) {
  // ─── 1. Session ───
 ...
 */
async function requirePermissionPourCanal(req, res, { channelId, channelCall, domaine = 'reglages', niveau = 'write' } = {}) {
  if (!channelId) {
  ...
  const garde = await requirePermission(req, res, { domaine, niveau, bien: bienDuCanal, bienRequis: true })
...
module.exports = { requirePermission, requirePermissionPourCanal, resoudreBien, resoudreBooking, chargerProfil, UUID_RE, REF_SURE_RE }
