...
//
//  3. UNE PANNE N'EST PAS UNE ABSENCE. L'erreur Supabase etait avalee : un
//     timeout PostgREST rendait `null`, donc « bien introuvable », donc 404 —
//     et cote profils, « aucun droit », donc 403. Un incident de trente secondes
//     faisait croire a tous les hotes qu'ils avaient perdu leurs biens. Les
//     lectures renvoient desormais le marqueur { erreur: true }, que la garde
//     traduit en 503.
async function resoudreBien(ref) {
  if (ref == null || ref === '') return null
  const valeur = String(ref)

  // ⚠ `migration_target_property_id` FAIT PARTIE DES IDENTIFIANTS D'UN BIEN.
  // Pendant une migration, le bien porte ses canaux sur sa propriete CIBLE : tout
  // ce qui remonte du provider — webhook, evenement, activation de canal — le
  // designe par CET identifiant. Sans lui ici, la garde ne trouvait aucun bien et
  // refusait l'acces a son proprietaire legitime, pendant toute la bascule.
  //
  // Ce n'est pas un elargissement de droits : la colonne porte un index UNIQUE
  // (migrations/2026-09-09-migration-cible.sql), donc un identifiant designe au
  // plus un bien, et l'ambiguite reste refusee explicitement plus bas.
  if (UUID_RE.test(valeur)) {
    const { data, error } = await supabase
      .from('properties')
      .select('id, user_id, name, provider, provider_property_id, migration_target_property_id')
      .or(`id.eq.${valeur},provider_property_id.eq.${valeur},migration_target_property_id.eq.${valeur}`)
      .maybeSingle()
    if (error) { console.error('[require-permission] lecture bien', error.message); return { erreur: true } }
    return data || null
  }
  ...
  // ⚠ AMBIGUITE, PAS PANNE. `provider_property_id` n'est PAS unique entre comptes
  // (deux hotes Beds24 peuvent porter le meme propId ; le depot a deja un test
  // dedie a ce cas). `.maybeSingle()` echouait alors en PGRST116, que le marqueur
  // d'erreur transformait en 503 permanent — le kill switch et la simulation
  // devenaient inutilisables pour ces biens. On lit toutes les lignes et on refuse
  // EXPLICITEMENT l'ambiguite, comme le fait deja resoudreBooking.
  const { data, error } = await supabase
    .from('properties')
    .select('id, user_id, name, provider, provider_property_id, migration_target_property_id')
    .or(`provider_property_id.eq.${valeur},migration_target_property_id.eq.${valeur}`)
  if (error) { console.error('[require-permission] lecture bien', error.message); return { erreur: true } }
  if (!data || !data.length) return null
  if (data.length > 1) {
    console.error(`[require-permission] provider_property_id ${valeur} present sur ${data.length} comptes : refus`)
    return { ambigu: true }
  }
  return data[0]
}
...
async function resoudreBooking(bookingId) {
  ...
  if (error) { console.error('[require-permission] lecture reservation', error.message); return { erreur: true } }
  if (!data || !data.length) return null
  if (data.length > 1) {
...
async function chargerProfil(userId, accountUserId) {
  const { data: profil, error: errProfil } = await supabase
    ...
    .maybeSingle()
  // ⚠ Une panne ne doit pas se lire comme « ce membre n'a aucun droit ».
  if (errProfil) { console.error('[require-permission] lecture profil', errProfil.message); return { erreur: true } }
  if (!profil) return { profil: null, permissions: null }
  const { data: permissions, error: errPerms } = await supabase
    .from('profile_permissions')
    .select('*')
    .eq('profile_id', profil.id)
    .maybeSingle()
  if (errPerms) { console.error('[require-permission] lecture droits', errPerms.message); return { erreur: true } }
  return { profil, permissions }
}

// ─── Le compte cible demande par le client (en-tete X-Compte) ────────────────
//
// ⚠ CET EN-TETE VIENT DU CLIENT. Il ne DESIGNE rien par lui-meme : il DEMANDE.
// Le compte n'est retenu que si l'appelant en est titulaire, ou membre ACTIF
// dont l'invitation est acceptee. Sans cette revalidation, poser un en-tete
// suffirait a travailler sur le compte de n'importe qui.
//
// Renvoie l'identifiant du compte a utiliser, ou { refus: true } — jamais une
// valeur par defaut permissive.
async function compteDemande (req, appelant) {
  const brut = req.headers?.['x-compte']
  if (!brut) return { compte: appelant }
  const vise = String(brut).trim()
  if (!UUID_RE.test(vise)) {
    console.warn('[require-permission] X-Compte au format refuse')
    return { refus: true }
  }
  // Son propre compte : rien a verifier.
  if (vise === String(appelant)) return { compte: vise }

  const { data, error } = await supabase
    .from('profiles')
    .select('active, accepted_at')
    .eq('account_user_id', vise)
    .eq('member_user_id', appelant)
    .maybeSingle()
  if (error) {
    console.error('[require-permission] verification X-Compte', error.message)
    return { erreur: true }
  }
  // ⚠ Profil desactive ou invitation non acceptee : AUCUN acces. C'est ce qui
  // rend la desactivation immediate, y compris pour un onglet deja ouvert qui
  // aurait garde le compte en memoire.
  if (!data || data.active !== true || !data.accepted_at) {
    console.log('[require-permission] refus X-Compte : pas membre actif de ce compte')
    return { refus: true }
  }
  return { compte: vise }
}
 ...
 */
async function requirePermission(req, res, { domaine, niveau = 'read', bien = null, bienRequis = false, bienResolu: bienDejaResolu = null, booking = null, bookingRequis = false, bookingResolu: bookingDejaResolu = null, userId: userIdVerifie = null,
  compteDelegue = false } = {}) {
  // ─── 1. Session ───
  ...
  // `email` n'est renseigne que lorsque la session est verifiee ICI. Un appelant
  // qui a deja valide la session ailleurs (et passe `userId`) ne le recoit pas :
  // api/stripe.js en a besoin et ne passe donc pas `userId`.
  let email = null
  if (!userId) {
    const token = req.headers.authorization?.replace('Bearer ', '')
  ...
  // `bookingDejaResolu` : ligne bookings_snapshot deja lue par l'endpoint (voir
  // api/beds24.js, qui doit la connaitre pour choisir sa garde). Parametre
  // interne, jamais fourni par le client — sans lui, le meme SELECT partait deux
  // fois par envoi de message.
  // ⚠ Meme confrontation que pour le bien (voir plus bas) : la ligne fournie doit
  // porter le booking_id demande. Sans elle, un appelant qui passerait `booking: X`
  // avec la ligne de Y ferait deriver accountUserId, le bien ET le perimetre de Y.
  let bookingResolu = null
  if (booking != null && booking !== '') {
    const cacheBookingOk = bookingDejaResolu && !bookingDejaResolu.erreur &&
      !bookingDejaResolu.ambigu &&
      String(bookingDejaResolu.booking_id) === String(booking)
    bookingResolu = cacheBookingOk ? bookingDejaResolu : await resoudreBooking(booking)
    if (bookingResolu && bookingResolu.erreur) {
  ...
  // `bienDejaResolu` : ligne properties deja lue par l'endpoint. Parametre
  // INTERNE — jamais renseigne depuis le client — qui evite de relire la meme
  // ligne quand l'appelant a du la resoudre pour choisir sa garde.
  //
  // ⚠ La ligne fournie doit CORRESPONDRE a `bien`. Sans cette confrontation, le
  // cache court-circuiterait la resolution : `accountUserId` et le perimetre
  // derivent ensuite du seul `bienResolu`, donc un appelant qui passerait la
  // reference A et la ligne du bien B ferait verifier les droits sur B. Le
  // parametre est interne, mais une garde ne doit pas dependre de la discipline
  // de ses appelants.
  // ⚠ Comparaison insensible a la casse : Postgres rend les UUID en minuscules,
  // UUID_RE accepte les majuscules. Une reference client en majuscules faisait
  // rejeter un cache pourtant valide — donc un second SELECT, et un point de
  // panne de plus, sans que le resultat change.
  const memeRef = (a, b) => String(a).toLowerCase() === String(b).toLowerCase()
  const cacheUtilisable = (ligne, ref) => ligne && !ligne.erreur && !ligne.ambigu &&
    (memeRef(ligne.id, ref) || memeRef(ligne.provider_property_id, ref))
  let bienResolu = null
  if (bien != null && bien !== '') {
    bienResolu = cacheUtilisable(bienDejaResolu, bien) ? bienDejaResolu : await resoudreBien(bien)
    if (bienResolu && bienResolu.erreur) {
      res.status(503).json({ error: 'Service temporairement indisponible' })
      return { ok: false }
    }
    if (bienResolu && bienResolu.ambigu) {
      res.status(409).json({ error: 'Référence de bien ambiguë' })
      return { ok: false }
    }
  ...
  // ⚠ `compteDelegue` : L'EN-TETE X-Compte N'EST HONORE QUE SUR DEMANDE EXPLICITE
  // DE L'ENDPOINT. Defaut `false`, donc comportement inchange partout.
  //
  // Le faire globalement etait une FAILLE : tous les endpoints sans `bien`
  // auraient bascule d'un coup sur le compte du titulaire, y compris ceux que
  // personne n'a audites pour la delegation. Cas reel le plus grave —
  ...
  // TITULAIRE. Idem agent-config, extract-kb, alert-test, et la creation de bien
  // de channel-property.
  //
  // Un endpoint ne devient delegable qu'apres avoir ete relu pour ca — et, s'il
  // renvoie une collection, apres avoir applique le filtre de perimetre.
  //
  // ⚠ L'ORDRE COMPTE. Une ressource designee (reservation, bien) prime toujours
  // sur l'en-tete : c'est la regle « le compte cible vient de la ressource ».
  let accountUserId = bookingResolu ? bookingResolu.user_id
                    : bienResolu    ? bienResolu.user_id
                    : userId
  if (!bookingResolu && !bienResolu && compteDelegue) {
    const demande = await compteDemande(req, userId)
    if (demande.erreur) {
      res.status(503).json({ error: 'Service temporairement indisponible' })
      return { ok: false }
    }
    if (demande.refus) {
      res.status(403).json({ error: 'Droits insuffisants sur ce compte' })
      return { ok: false }
    }
    accountUserId = demande.compte
  }
  ...
  if (domaine === 'titulaire') {
    ...
    return { ok: true, userId, email, accountUserId, bien: bienResolu, booking: bookingResolu, contexte: null }
  }
  ...
  // ─── 5. Droits ───
  const charge = await chargerProfil(userId, accountUserId)
  if (charge.erreur) {
    res.status(503).json({ error: 'Service temporairement indisponible' })
    return { ok: false }
  }
  const { profil, permissions } = charge
  ...
  return { ok: true, userId, email, accountUserId, bien: bienResolu, booking: bookingResolu, contexte }
}
 ...
 */
async function requirePermissionPourCanal(req, res, { channelId, channelCall, domaine = 'reglages', niveau = 'write', userId = null, biensAttendus = null } = {}) {
  if (!channelId) {
  ...
  try {
    ...
    biensDuCanal = Array.isArray(attrs.properties) ? attrs.properties.map(String) : []
  } catch (e) {
  ...
  if (!biensDuCanal.length) {
    res.status(404).json({ error: 'Canal introuvable ou sans bien rattaché' })
    return { ok: false }
  }

  // ⚠ UN CANAL PORTE PLUSIEURS BIENS, ET `properties[0]` N'EST PAS « LE » BIEN.
  // Troisieme occurrence de la meme hypothese fausse, trouvee le 11 septembre
  // 2026 : un canal Airbnb partage entre trois logements rendait toujours le
  // PREMIER. L'ecran de connexion d'un logement neuf recevait donc un 403
  // « Ce canal ne releve pas du bien indique » — et le front l'affichait
  // « Impossible de recuperer vos annonces ».
  //
  // Quand l'appelant DIT sur quel bien il agit (`biensAttendus`), c'est sur
  // CELUI-LA qu'on verifie les droits, et il doit figurer parmi les biens du
  // canal. C'est a la fois plus juste et plus strict que `properties[0]` :
  // avant, les droits pouvaient etre valides sur un bien que l'appelant
  // n'avait pas nomme.
  let bienDuCanal = biensDuCanal[0]
  if (Array.isArray(biensAttendus) && biensAttendus.length) {
    const attendus = biensAttendus.filter(Boolean).map(String)
    const commun = attendus.find(id => biensDuCanal.includes(id))
    if (!commun) {
      console.log('[require-permission] refus : canal rattache a un autre bien')
      res.status(403).json({ error: 'Ce canal ne releve pas du bien indique' })
      return { ok: false }
    }
    bienDuCanal = commun
  }
  ...
  // `canal` est rendu pour que l'appelant n'ait pas a refaire le meme GET.
  return { ...garde, bienDuCanal, biensDuCanal, canal }
}
...
module.exports = { requirePermission, requirePermissionPourCanal, verifierSession, compteDemande, resoudreBien, resoudreBooking, chargerProfil, UUID_RE, REF_SURE_RE }
