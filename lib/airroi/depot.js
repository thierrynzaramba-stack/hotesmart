...
function depotSupabase (supabase) {
  ...
  return {
    ...
    // ⚠ LA DEPENSE SE RESERVE AVANT LE RESEAU (statut « parti »), puis se
    // termine (« ok » / « erreur ») : un appel paye puis interrompu reste
    // COMPTE par les garde-fous (review du 24 septembre 2026).
    async reserver (ligne) {
      const { data, error } = await supabase.from('airroi_appels').insert({
        endpoint: ligne.endpoint, cle: ligne.cle, cout_usd: ligne.cout,
        user_id: ligne.userId || null, property_id: ligne.propertyId || null,
        statut: 'parti', http: null, created_at: ligne.le
      }).select('id').single()
      if (error) throw new Error(`[airroi] journal des appels (reservation) : ${error.message}`)
      return data.id
    },
    async terminer (id, { statut, http }) {
      const { error } = await supabase.from('airroi_appels').update({ statut, http: http ?? null }).eq('id', id)
      if (error) throw new Error(`[airroi] journal des appels (fin) : ${error.message}`)
    },
...
function depotFichier (dossier) {
  ...
  return {
    ...
    async reserver (ligne) {
      const id = lignes().length + 1
      fs.appendFileSync(journal, JSON.stringify({
        id, endpoint: ligne.endpoint, cle: ligne.cle, cout_usd: ligne.cout, user_id: ligne.userId || null,
        property_id: ligne.propertyId || null, statut: 'parti', http: null, created_at: ligne.le
      }) + '\n')
      return id
    },
    async terminer (id, { statut, http }) {
      const tout = lignes().map(l => (l.id === id ? { ...l, statut, http: http ?? null } : l))
      fs.writeFileSync(journal, tout.map(l => JSON.stringify(l)).join('\n') + '\n')
    },
    async appelsDepuis ({ depuis, userId = null, propertyId = null }) {
