...
async function pollChannelReviews (results, deps = {}) {
  ...
  while (page <= MAX_PAGES) {
    ...
    for (const item of liste) {
      ...
      if (!ligne) { bilan.erreurs++; continue }
    ...
    if (lot.length) {
      const { error } = await sb.from('ota_reviews')
        .upsert(lot, { onConflict: 'user_id,provider,external_review_id' })
      if (error) {
        bilan.erreurs += lot.length
        console.error('[cron-reviews] upsert du lot echec', error.message)
      } else {
        bilan.ecrits += lot.length
      }
    }
