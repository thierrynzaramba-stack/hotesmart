...
function versLigne (item, { userId, propertyId, propertyRef }) {
  ...
  return {
    ...
    // Les poser a null "pour faire propre" detruirait la resolution obtenue lors
    // d'un passage precedent.
    //
    // Leur ABSENCE ne suffit toutefois PAS a les proteger dans une ecriture par
    // lot : c'est upsertAvis() qui s'en charge, en separant les lignes qui
    // portent ces colonnes de celles qui ne les portent pas. Lire le commentaire
    // qui s'y trouve avant de fusionner les deux lots.
    //
    // Idem pour menage_event_id, ai_clean_verdict, ai_clean_excerpt et
    // ai_analyzed_at, jamais ecrits ici : les colonnes ai_* sont remises a null
    // par le trigger quand le texte change, ce qui remet l'avis en file de
    // classification.
  }
}
...
// Colonnes d'ancrage du sejour, posees seulement quand la reservation est
// resolue. Elles decident du groupe d'ecriture, voir ci-dessous.
const COLONNES_ANCRAGE = ['booking_uid', 'stay_start', 'stay_end']

async function upsertAvis (sb, lignes) {
  const lot = Array.isArray(lignes) ? lignes : [lignes]
  if (!lot.length) return { ecrits: 0 }

  // ⚠ DEUX GROUPES, ET C'EST INDISPENSABLE.
  //
  // La liste de colonnes d'un upsert PostgREST est determinee PAR REQUETE, pas
  // par ligne : supabase-js envoie l'union des cles de toutes les lignes, et les
  // cles absentes d'une ligne partent a NULL (defaultToNull, son defaut).
  //
  // Consequence, verifiee en base : des qu'UN avis d'une page resolvait sa
  // reservation, les trois colonnes d'ancrage entraient dans le DO UPDATE SET de
  // TOUTE la page, et tous les autres avis voyaient leur booking_uid, stay_start
  // et stay_end ECRASES A NULL. Un temoin pose a la main est revenu a null au
  // poll suivant. C'est exactement la destruction qu'un commentaire de ce
  // fichier affirmait empecher — il avait tort.
  //
  // L'effet reel n'etait pas visible sur un passage isole (les memes 11 avis se
  // resolvaient a chaque fois) : il se serait manifeste a mesure que les
  // reservations sortent de la fenetre de fetch, en vidant l'ancrage dont le
  // pricing et la fiche prestataire dependent.
  //
  // On ecrit donc deux lots HOMOGENES en cles. Le lot sans ancrage ne porte pas
  // ces colonnes du tout : il ne peut plus les toucher.
  const avecAncrage = lot.filter(l => COLONNES_ANCRAGE.some(c => c in l))
  const sansAncrage = lot.filter(l => !COLONNES_ANCRAGE.some(c => c in l))

  let ecrits = 0
  for (const groupe of [sansAncrage, avecAncrage]) {
    if (!groupe.length) continue
    const { error } = await sb.from('ota_reviews').upsert(groupe, { onConflict: CONFLIT })
    if (error) return { ecrits, error }
    ecrits += groupe.length
  }
  return { ecrits }
}
...
async function pollChannelReviews (results, deps = {}) {
  ...
  while (page <= MAX_PAGES) {
    ...
    for (const item of liste) {
      ...
      if (prep.ignore) {
        // Acces par cle dynamique : on ne compte que des raisons DECLAREES,
        // sinon une nouvelle raison produirait un NaN silencieux dans le bilan.
        if (prep.ignore in bilan) bilan[prep.ignore]++
        else { bilan.erreurs++; console.error('[cron-reviews] raison ignoree inconnue:', prep.ignore) }
        continue
      }
