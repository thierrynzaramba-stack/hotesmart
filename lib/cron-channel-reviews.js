...
function versLigne (item, { userId, propertyId, propertyRef }) {
  ...
  return {
    ...
    provider:            'channex',
    ota,
    external_review_id:  String(at.id || item.id),
...
async function proprietaireDuBien (sb, providerPropertyId) {
  ...
  if (data.length > 1) return { ambigu: true }
  return { owner: data[0] }
}
...
// Resolution UNITAIRE d'une reservation, pour le traitement d'un avis isole
// (webhook `updated_review`, lot suivant) : le poll, lui, passe par l'index
// ci-dessus, qui evite une requete par avis.
// On matche `ota_reservation_id` contre
// snapshot->>'otaReservationCode', TOUJOURS filtre par user_id (regle 1 :
// jamais l'identifiant seul, une reference OTA n'est pas unique entre comptes).
// Renvoie les dates de sejour a denormaliser, ou null si non resolu.
async function resoudreReservation (sb, userId, otaReservationId) {
  if (!userId || !otaReservationId) return null
  const { data, error } = await sb
    .from('bookings_snapshot')
    .select('booking_id, snapshot')
    .eq('user_id', userId)
    .eq('snapshot->>otaReservationCode', String(otaReservationId))
    .limit(2)
  if (error || !data || data.length !== 1) return null   // 0 = pas encore la, 2+ = ambigu
  const s = data[0].snapshot || {}
  return {
    booking_uid: String(data[0].booking_id),
    stay_start:  s.arrival   || null,
    stay_end:    s.departure || null
  }
}
...
async function pollChannelReviews (results, deps = {}) {
  ...
  await sb.from('cron_logs').upsert({ id: MARQUEUR, last_run: new Date(maintenant()).toISOString() })
  ...
  while (page <= MAX_PAGES) {
    ...
    const r = await appel('GET', `/reviews?pagination[page]=${page}&pagination[limit]=${PAGE_LIMIT}`)
    if (!r.ok) {
    ...
    for (const item of liste) {
      ...
      if (!cacheOwner.has(cle)) cacheOwner.set(cle, await proprietaireDuBien(sb, ref))
      const res = cacheOwner.get(cle)

      if (res.erreur)  { bilan.erreurs++; continue }
      // Un bien non rattache a HoteSmart, ou rattache de facon ambigue, n'est
      // PAS ecrit : mieux vaut un avis manquant qu'un avis chez le mauvais hote.
      if (res.absent)  { bilan.bien_inconnu++; continue }
      if (res.ambigu)  {
        bilan.bien_ambigu++
      ...
      const { error } = await sb.from('ota_reviews')
        .upsert(ligne, { onConflict: 'user_id,provider,external_review_id' })
      if (error) {
        bilan.erreurs++
        console.error('[cron-reviews] upsert echec', error.message)
      } else {
        bilan.ecrits++
      }
    }
