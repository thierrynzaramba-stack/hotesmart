...
const { canPushRates, RATE_PUSH_BLOCKED, proprieteQuiRecoitLARI, estEnMigration, estRelieAuCanal } = require('./rate-sync')
...
function raisonDeNePasPousser (bien) {
  ...
  if (!proprieteQuiRecoitLARI(bien)) {
...
async function pousserAri (bien, { dryRun = true, appel = null } = {}) {
  ...
  const cible = proprieteQuiRecoitLARI(bien)
...
async function etatAri (supabase, bien, { appel = null } = {}) {
  ...
  if (error) return { etat: 'bloque', raison: 'lecture_coeur', message: `Lecture du coeur impossible : ${error.message}` }
  let duCoeur = (lignes || []).filter(l => Number(l.rate) > 0)
...
module.exports = { pousserAri, etatAri, raisonDeNePasPousser, HORIZON_CONTROLE_JOURS }
