...
// ⚠ PAR COMPTE, PAS GLOBAL — CORRECTIF DE REVIEW.
// Ma premiere version tenait un compteur unique pour tous les hotes. L'incident,
// lui, est cloisonne par `${userId}|${provider}` : avec deux comptes, le SUCCES
// de l'un remettait a zero les echecs de l'autre, et l'incident de l'hote en
// panne rapportait « 1 echec » indefiniment. Le compteur repondait « une fois »
// a la seule question pour laquelle il existe — combien de fois — et « a
// l'instant » a « depuis quand ». Latent avec un seul compte en production, mais
// ce compteur est fait pour trancher sur plusieurs semaines si les correctifs de
// portee et de cache ont marche.
// Les cumuls d'INSTANCE, eux, restent globaux : c'est leur sens.
const compteurs = { lectures: 0, echecs: 0 }
const parCompte = new Map()   // `${userId}|${provider}` -> { echecsDepuisSucces, dernierSucces }
function compteurDe (cle) {
  if (!parCompte.has(cle)) parCompte.set(cle, { echecsDepuisSucces: 0, dernierSucces: null })
  return parCompte.get(cle)
}
function statistiques (cle) {
  const c = cle ? compteurDe(cle) : { echecsDepuisSucces: null, dernierSucces: null }
  return { ...compteurs, ...c }
}
...
// ⚠ DEUX REMISES A ZERO DISTINCTES, ET C'EST VOULU. Le cache et les compteurs
// n'ont pas la meme duree de vie : un test qui vide le cache pour forcer une
// relecture ne veut pas, ce faisant, effacer le compte d'echecs qu'il mesure.
function _vider () { cache.clear() }              // tests uniquement
function _viderCompteurs () { parCompte.clear() } // tests uniquement
...
async function clesMigrees (supabase, userId, provider = 'beds24') {
  ...
  if (error) {
    compteurs.echecs++
    const suivi = compteurDe(cle)
    suivi.echecsDepuisSucces++
    ...
    if (Date.now() - vuIncident >= INCIDENT_MS) {
      ...
      try {
        ...
        await reportIncident('cles_migrees_illisible', {
          ...
          // Par compte : c'est la granularite de l'incident lui-meme.
          echecs_depuis_dernier_succes: compteurDe(cle).echecsDepuisSucces,
          dernier_succes: compteurDe(cle).dernierSucces
            ? new Date(compteurDe(cle).dernierSucces).toISOString() : null,
          // Cumuls d'INSTANCE, tous comptes confondus — c'est leur sens.
  ...
  const suivi = compteurDe(cle)
  suivi.echecsDepuisSucces = 0
  suivi.dernierSucces = Date.now()
...
async function attendreFenetreDeCache ({
  ...
  // ⚠ ON AJOUTE UN CYCLE, RELEVE EN REVIEW. Attendre exactement CACHE_MS couvre
  // l'EXPIRATION du cache, pas le cycle deja EN VOL : une instance peut avoir lu
  // le cache a la derniere milliseconde de la fenetre, puis passer 40 a 56 s a
  // ecrire — c'est-a-dire pendant que le script deplace les lignes. Et sur ce
  // chemin `detectBookingChanges` ne consulte pas `automation_paused` : la pause
  // posee par le script ne le couvre pas. Un cycle de plus ferme le recouvrement.
  const MARGE_CYCLE_MS = 60 * 1000
  const total = CACHE_MS + MARGE_CYCLE_MS
  const secondes = Math.ceil(total / 1000)
  ...
  const PAS = 30 * 1000
  while (attendu < total) {
    const ce = Math.min(PAS, total - attendu)
    await dormir(ce)
    attendu += ce
    if (attendu < total) dire(`   ... ${Math.ceil((total - attendu) / 1000)} s`)
  }
...
// ⚠ LE RETRAIT — L'UNIQUE EXCEPTION LEGITIME A « ON NE SUPPRIME JAMAIS ».
// Le KB dit, et il a raison : on ne supprime JAMAIS une ligne de
// `provider_keys_migrated`, parce que le fait « cette cle est abandonnee » doit
// survivre a tout. L'exception est le TRANSFERT AVORTE, et elle nait du commit
// du 14 septembre 2026 : la cle est desormais enregistree AVANT que les lignes
// ne bougent. Si le transfert echoue ensuite, le bien reste marque migre SANS
// avoir ete transfere — et plus rien ne le touche : ni synchro, ni message, ni
// code d'acces, ni avis. Un bien mort-vivant, invisible, sans alerte.
// Annuler l'enregistrement est alors la seule remise en etat, et c'est le
// pendant exact de la pause qu'on rend. Toute autre suppression est un defaut.
async function retirerCleMigree (supabase, { userId, provider, propId }) {
  if (!userId || !provider || propId == null) {
    throw new Error('retirerCleMigree : userId, provider et propId requis')
  }
  const { error } = await supabase.from('provider_keys_migrated').delete()
    .eq('user_id', userId).eq('provider', provider).eq('provider_property_id', String(propId))
  if (error) throw new Error(`retirerCleMigree : ${error.message}`)
...
module.exports = {
  clesMigrees, motifNonSync, motifNonSyncPourBien, estCleMigree, concerneLeProvider,
  noterCleMigree, retirerCleMigree,
