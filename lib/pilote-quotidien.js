...
const { prixHoteDuBien, purgerPrixHotePasses } = require('./prix-hote')
const { relireMemoire } = require('./nuits-du-moteur')
...
async function piloterLeBien (supabase, bien, deps = {}) {
  ...
  // La main de l'hote (arbitrage A bis) : lue une fois, sautee par les deux
  // moteurs ; les nuits passees ne la portent plus.
  let prixHote
  try { await purgerPrixHotePasses(supabase, bien.id, auj); prixHote = await prixHoteDuBien(supabase, bien.id, auj, fin) }
  ...
  const ouverture = await ouvrirLaFenetreDuBien(supabase, bien, {
    aujourdHui: auj, appel: deps.appel, demander: deps.demander, prixCalcule, fermetures, vendues: venduesCtx, prixHote
  })
  ...
  if (ctx) {
    ...
    // ⚠ OUVERTURE INCONNUE : ON NE TARIFE RIEN, ET ON LE DIT (dette 25, lot
    // V2.0.3). Avant, `ouverts: null` partait vers `entretenirLesPrix`, dont la
    // garde ne tirait que sur `undefined` ; `calculerPrix` lit `null` comme
    // « pas de filtre » : toutes les nuits etaient tarifees et DEMANDEES au
    // calendrier, puis le bilan etait reecrit « aucun prix pose ». Des prix
    // partis sous un bilan qui mentait.
    if (ouverts == null) {
      prix = { ok: false, refus: 'ouverture_inconnue', message: `Capacité non calculable (${cap && cap.raison}) : aucun prix posé.`, fin }
    } else {
      prix = await entretenirLesPrix(supabase, bien, {
        aujourdHui: auj, appel: deps.appel, demander: deps.demander, ctx, fermetures, lignes, ouverts, prixHote,
...
// `relireMemoire` vit dans lib/nuits-du-moteur.js : la confirmation
// d'activation relit la memoire de la meme facon.
