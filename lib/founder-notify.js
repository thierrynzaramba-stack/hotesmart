...
const SANS_SMS = new Set(['reservation_remboursee'])
...
async function reportIncident(type, { userId = null, propertyId = null, propertyName = null, detail = null, threshold = 1, fenetreMs = 3600 * 1000 } = {}) {
  ...
  const since = new Date(Date.now() - fenetreMs).toISOString()
  ...
  const html = `<h3>HôteSmart — Alerte : ${esc(label)}</h3>`
    ...
    + (body ? `<p>${esc(body)}</p>` : '')
    + `<p style="color:#86868b;font-size:12px">Anti-spam : 1 par type et par bien toutes les ${Math.round(fenetreMs / 60000)} min.</p>`
