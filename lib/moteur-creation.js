...
async function alerterReveil (t, bien, detail) {
...
async function creerDepuisTentative (supabase, tentativeId) {
  ...
  try {
    ...
    let envoi = await canal.createBooking(bien.provider_property_id, resa)
    ...
    if (!envoi.status || envoi.status === 0) {
      await alerterReveil(t, bien,
        `POST CRS d'issue INCERTAINE (${(t.amount_cents / 100).toFixed(2)} ${t.currency}, ` +
        `${t.arrival} → ${t.departure}, ${t.guest_email}). Argent encaisse. ` +
        `NE PAS REJOUER : verifier chez le provider avant tout geste.`)
...
async function echecCertain (supabase, t, bien, raison, detail) {
  ...
  if (remb.ok) {
    ...
    try {
      await reportIncident('reservation_remboursee', {
        ...
        detail: `${detail} Voyageur rembourse (${(t.amount_cents / 100).toFixed(2)} ${t.currency}).`
      })
    } catch (e) { console.error('[moteur-creation] incident non enregistre', e.message) }
  ...
  await alerterReveil(t, bien,
    `${detail} REMBOURSEMENT ECHOUE (${remb.raison}). ` +
    `${(t.amount_cents / 100).toFixed(2)} ${t.currency} encaisses sans reservation. A traiter A LA MAIN.`)
...
async function rattraperBloquees (supabase, { maintenant, limite = 20 } = {}) {
  ...
  for (const t of aSignaler) {
    ...
    await alerterReveil(t, bien,
      `Tentative BLOQUEE en « paye » depuis plus de 10 min. ` +
      `${(t.amount_cents / 100).toFixed(2)} ${t.currency}, ${t.arrival} → ${t.departure}, ${t.guest_email}. ` +
      `Chercher le code ${codeReservation(t.id)} chez le provider AVANT tout geste : ` +
      `s'il existe, la reservation est creee ; sinon elle ne l'est pas. NE PAS REJOUER a l'aveugle.`)
    await supabase.from('booking_attempts')
...
module.exports = {
  ...
  payloadDepuisTentative, rembourser, creerDepuisTentative
}
