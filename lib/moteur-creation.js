...
async function prevenirVoyageur (supabase, t, bien, sorte, reference) {
  ...
  const commun = {
    // ⚠ LE COMPTE PROPRIETAIRE : c'est lui qui donne la cle Brevo et l'adresse
    // d'expedition. Sans `userId`, l'e-mail repart sous l'identite de la
    // plateforme — le defaut que l'etape 5 corrige.
    userId: t.user_id,
    // ⚠ LE PROPID PROVIDER, PAS L'UUID — comme partout ailleurs dans ce module
    // (lignes `propId` plus haut et plus bas). Cette valeur descend jusqu'a
    // `reportIncident`, dont l'anti-spam filtre sur `property_id` : deux
    // identites pour un meme bien, et une panne de quota Brevo alerte DEUX fois
    // par heure au lieu d'une. L'ecran d'incidents a venir ne les retrouverait
    // pas non plus par le propId. Constat de review.
    propertyId: bien ? String(bien.provider_property_id) : null,
    // L'identifiant de la reservation chez le provider : c'est lui que porte
    // l'adresse de reponse. Vide sur un remboursement — aucune reservation
    // n'existe, donc aucun fil a rejoindre, et le reply-to retombe sur l'hote.
    bookingId: reference || null,
