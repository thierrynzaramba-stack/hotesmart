...
async function syncStatusFromBookings(userId, propertyId, bookings, knowledge = {}) {
  ...
  // Statut canonique (lib/bookings-snapshot.js) : seul 'confirmed' occupe le
  // logement. Ecarte aussi 'request' (demande non confirmee), que le test en dur
  // sur 'cancelled'/'black' laissait passer. Bookings Beds24 bruts -> provider par
  // defaut ; un snapshot porte son propre champ provider, qui reste prioritaire.
  const confirmed = (bookings || []).filter(b => isActiveStatus(b, 'beds24'))
