...
// ─── 10. La neutralisation du futur, APRES le re-keying ──────────────────────
// Regle de Thierry : au demappage, les sejours a venir et en cours ne comptent
// plus, les passes restent. C'est ce qui rend inutile le dedoublonnage du
// carnet. On ANNULE, on ne supprime pas : un sejour annule n'occupe rien, fait
// annuler son menage tout seul par le sync, et laisse la trace.
//
// ⚠ APRES le deplacement, jamais avant : tant que le bien est encore chez
// Beds24, le cron de synchronisation reecrit ses snapshots et remettrait
// « confirme » au cycle suivant.
const TITRE_PURGE = 'Sejours a venir neutralises (demappage)'

async function etatPurgeFutur (supabase, bien) {
  const { etatPurge } = require('./migration-purge-futur')
  const r = await etatPurge(supabase, bien)
  return etape('purger_le_futur', TITRE_PURGE, r.etat, r.message, r.action || null)
}

// ─── 9. Le deplacement des donnees (re-keying) ───────────────────────────────
// Le dernier geste, et le seul irreversible sans effort : les 18 tables enfants
// et le bien passent chez le nouveau provider EN UNE TRANSACTION.
const TITRE_REKEY = 'Deplacement des donnees vers le nouveau provider'

// Sans comptage : il coute 42 `count(*)` par bien et appartient a l'apercu de
// l'action, pas a l'affichage de l'etat.
function etatDeplacement (bien) {
  const { etatRekeying } = require('./migration-rekeying')
  const r = etatRekeying(bien)
  return etape('re_keying', TITRE_REKEY, r.etat, r.message, r.action || null)
}
...
async function etatMigration (supabase, bien, opts = {}) {
  const etapes = [
    ...
    etatDeplacement(bien),
    await etatPurgeFutur(supabase, bien)
  ]
...
module.exports = { etatMigration, etatProvisionnement, etatPousseeAri, etatModePrix, etatDeplacement, etatPurgeFutur, CHAMPS_FICHE, HORIZON_PRIX_JOURS }
