...
async function materializeBeds24Properties(userId, properties, results) {
  ...
  // `clesMigrees` retombe volontairement OUVERT sur un echec de lecture, et ce
  // choix reste bon pour les actions reversibles. Mais materialiser CREE UN
  ...
  // pretendait justement les distinguer. Or `clesMigrees` retombe volontairement
  // OUVERT sur un echec : « 0 ignore » peut donc vouloir dire « la garde n'a
  // rien filtre parce qu'elle est aveugle ». On le dit.
  if (!migrees.size) {
  ...
  // « aucun bien migre » de « la lecture des cles migrees a echoue » — et
  // `clesMigrees` retombe volontairement OUVERT sur un echec de lecture.
  if (ignorees > 0) {
