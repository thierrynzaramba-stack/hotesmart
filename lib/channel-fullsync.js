...
async function runFullSync(bien, { dryRun = false } = {}) {
  ...
  // Dates ecartees parce que leur tarif tombe sous le plancher du bien.
  // Comptees a part de `fermeesSansPrix` : « pas de prix » et « prix absurde »
  // sont deux problemes differents, et l'hote doit pouvoir les distinguer.
  const sousPlancher = []
  // Le premier refus rencontre, pour pouvoir l'expliquer a l'hote : sans lui,
  // le message dirait « des nuits sous le plancher » sans dire lequel ni
  // combien — donc sans qu'il puisse agir.
  let detailPlancher = null
  ...
  for (let i = 0; i < 500; i++) {
    ...
    // ─── PRIX PLANCHER ────────────────────────────────────────────────────
    // ⚠ UN TARIF SOUS LE PLANCHER EST TRAITE COMME UNE ABSENCE DE PRIX.
    // Signale par Thierry le 12 septembre 2026 : des nuits a 0 ont ete
    // retrouvees. Rien ici ne refusait un tarif absurde — seule l'absence
    // TOTALE de prix fermait la date. Or `rate: 0` est le pire cas : Channex
    // ne l'applique pas, il garde le prix de la GRILLE, donc la nuit se vend
    // a un tarif que l'hote n'a jamais choisi, en silence.
    //
    // On FERME, on ne corrige pas : remonter au plancher inventerait un prix
    // et le vendrait au nom de l'hote.
    let refusPlancher = null
    if (prixEur !== null) {
      const verdict = tarifAcceptable(Math.round(prixEur * 100), bien)
      if (!verdict.ok) {
        refusPlancher = { raison: verdict.raison, cents: Math.round(prixEur * 100), plancher: verdict.plancher }
        if (!detailPlancher) detailPlancher = refusPlancher
        sousPlancher.push(iso)
      }
    }

    if (prixEur === null || refusPlancher) {
      ...
      obj.stop_sell = true
      if (!refusPlancher) fermeesSansPrix.push(iso)
    } else {
      ...
      // ⚠ NI FERMEE, NI DEJA VENDUE — releve en review.
      // L'index unique partiel autorise une ligne courante A COTE d'une ligne
      // vendue : sans ce test, repousser le tarif d'une plage englobant une
      // nuit vendue ouvrirait une courante neuve, et le moteur recompterait la
      // nuit comme « affichee, jamais vendue ». `vendues` porte deja les nuits
      // occupees, c'est le meme calcul qui reduit le stock.
      const dejaVendue = (vendues[iso] || []).length >= unites
      if (!obj.stop_sell && !dejaVendue) prixParNuitFs[iso] = rateCents
    }
  ...
  if (sousPlancher.length) {
    const d = detailPlancher || {}
    warnings.push(messageRefus(d.raison, d.cents, d.plancher, sousPlancher.length, 'poussee'))
    console.log(`[fullsync] bien ${bien.id} : ${sousPlancher.length} date(s) SOUS LE PLANCHER`
      + ` (${sousPlancher[0]} a ${sousPlancher[sousPlancher.length - 1]}), fermees`)
    if (!dryRun) {
      try {
        const { reportIncident } = require('./founder-notify')
        await reportIncident('prix_sous_plancher', {
          userId: bien.user_id || null,
          propertyId: String(propIdFs || bien.id),
          propertyName: bien.name || String(bien.id),
          threshold: 1,
          detail: `${sousPlancher.length} nuit(s) a un tarif sous le plancher`
            + ` (${(d.cents / 100).toFixed(2)} EUR contre ${(d.plancher / 100).toFixed(2)} EUR mini)`
            + ` sur ${bien.name}. Ces dates sont FERMEES, aucun prix n'est parti.`
            + ` ⚠ LE TARIF RESTE DANS LE CALENDRIER : tant qu'il n'est pas`
            + ` corrige, cette alerte reviendra a chaque synchronisation et ces`
            + ` nuits resteront invendables. Corriger le prix dans le calendrier.`
        })
      } catch (e) {
        console.error('[fullsync] alerte « prix sous plancher » non partie :', e.message)
      }
    }
  }
  ...
  if (dryRun) {
    ...
    return {
      ...
      pushed: false,
      warnings,
      // ⚠ LES DATES SOUS PLANCHER SONT DES DATES NON TARIFEES.
      // Releve en review : exclues de `fermeesSansPrix` (elles ont leur propre
      // compteur), elles etaient comptees comme tarifees — un bien dont le
      // prix de base passe sous le plancher affichait « 500 dates tarifees »
      // alors que ZERO tarif part. Et c'est l'apercu que l'operateur de
      // bascule lit avant de pousser pour de vrai.
      dates_tarifees: 500 - fermeesSansPrix.length - sousPlancher.length,
      ...
      dates_sous_plancher: sousPlancher.length,
...
// `channelCall` est exporte pour le cron (moteur d'ouverture, 4.6.3) : le
// writer attend un client canal fourni par l'appelant, et celui-ci existe deja.
module.exports = { runFullSync, JOURS_POUSSES, toLocalISO, fenetrePoussee, channelCall }
