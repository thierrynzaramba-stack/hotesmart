...
const { proprieteQuiRecoitLARI } = require('./rate-sync')
...
// `proprieteQuiRecoitLARI` (lib/rate-sync.js).
async function runFullSync(bien, { dryRun = false } = {}) {
  ...
  for (const col of ['provider', 'migration_target_property_id']) {
    if (bien && bien[col] === undefined) {
      throw new Error(`runFullSync : la colonne \`${col}\` n a pas ete selectionnee — impossible de savoir vers quelle propriete pousser`)
    }
  }
  const propIdFs = proprieteQuiRecoitLARI(bien)
  if (!propIdFs) {
...
module.exports = { runFullSync, JOURS_POUSSES }
