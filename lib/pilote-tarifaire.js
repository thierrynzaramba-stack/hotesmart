...
function fenetreDuBien (bien) {
  ...
  const valeur = Number(bien.pilote_fenetre_valeur)
  if (!FENETRE_TYPES.includes(type)) return null
  if (!Number.isInteger(valeur) || valeur <= 0) return null
  return { type, valeur }
}
...
function dateOuverture (bien, date, aujourdHui) {
  ...
  return f.type === 'jours' ? decalerJours(date, -f.valeur) : decalerMois(date, -f.valeur)
}
