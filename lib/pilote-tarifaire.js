...
// ─── LA VALIDATION D'UNE FENETRE DEMANDEE (activation, lot 4.6.3) ───────────
// Ce qu'un formulaire ou un endpoint transmet : { type, valeur }. On rend la
// paire propre, ou un refus en francais. Bornes : L'HORIZON DU CANAL — tout
// le produit pousse et reconcilie 500 jours (JOURS_POUSSES, fullsync) ; une
// fenetre au-dela ouvrirait des nuits qu'aucun full sync ne reconcilierait
// (releve en review). 500 jours, ou 16 mois (~487 jours au pire).
// `Number(true)` vaut 1 : on exige un entier, la valeur peut arriver en
// chaine depuis un formulaire.
const FENETRE_MAX = { jours: 500, mois: 16 }
function validerFenetre (entree) {
  if (!entree || typeof entree !== 'object') {
    return { ok: false, code: 'fenetre_requise', error: 'Choisissez jusqu\'où YieldFlow ouvre les dates : un nombre de jours ou de mois glissants.' }
  }
  const type = String(entree.type || '')
  if (!FENETRE_TYPES.includes(type)) {
    return { ok: false, code: 'fenetre_type_inconnu', error: 'La fenêtre se compte en jours ou en mois.' }
  }
  const brut = entree.valeur
  const valeur = (typeof brut === 'number' || (typeof brut === 'string' && /^\d+$/.test(brut.trim()))) ? Number(brut) : NaN
  if (!Number.isInteger(valeur) || valeur <= 0) {
    return { ok: false, code: 'fenetre_valeur_invalide', error: 'La fenêtre doit être un nombre entier de jours ou de mois, supérieur à zéro.' }
  }
  if (valeur > FENETRE_MAX[type]) {
    return { ok: false, code: 'fenetre_trop_longue', error: `Au plus ${FENETRE_MAX[type]} ${type} : c'est l'horizon que HôteSmart pousse et réconcilie vers les plateformes.` }
  }
  return { ok: true, fenetre: { type, valeur } }
}
