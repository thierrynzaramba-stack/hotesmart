...
// ⚠ CE QUI DIT SI UN CANAL RENDRA CE SEJOUR : SA SOURCE, PAS SON CODE.
// Premiere version : « il a un code OTA, donc il reviendra ». FAUX, et mesure en
// base : les reservations creees par HoteSmart en portent un aussi
// (`HS-1788765359046-J2XC0`, `HSM-98BCBDBC8A084978` — api/reservation-directe.js
// et lib/moteur-creation.js), et Channex les rend avec `ota_name: "Offline"`.
// Elles auraient donc ete neutralisees : nuits remises en vente, menage annule,
// voyageur oublie — exactement contre la regle de Thierry.
//
// Repartition reelle des 1 431 snapshots des quatre biens :
//   airbnb / AirBNB      927  toutes codees   -> un canal les rendra
//   booking / BookingCom 311  toutes codees   -> un canal les rendra
//   direct               199  aucune codee    -> personne ne les rendra
//   Offline                3  codees HS-/HSM- -> creees ici, personne ne les rend
//   iCal import 1          4  codees          -> autre connexion, pas ce canal
//
// ⚠ LISTE FERMEE DES OTA, ET LE DOUTE PROFITE A LA CONSERVATION. Ne pas
// neutraliser un sejour d'OTA produit un doublon : grave, mais visible et
// reparable. Neutraliser une reservation VIVANTE remet ses nuits en vente et
// annule son menage : pire, et silencieux. Donc tout ce qui n'est pas un OTA
// CONNU est conserve — et une source inconnue est SIGNALEE, pour qu'un humain
// tranche au lieu d'un defaut.
const SOURCES_OTA = new Set([
  'airbnb', 'booking', 'bookingcom', 'booking.com', 'bookingdotcom',
  'expedia', 'vrbo', 'homeaway', 'agoda', 'tripadvisor'
])
const SOURCES_CONSERVEES = new Set([
  'direct', 'offline', 'app', 'manual', 'hotesmart', 'hs', 'crs'
])

function natureDuSejour (snapshot) {
  const src = String((snapshot || {}).source || '').trim().toLowerCase()
  if (!src) return 'inconnue'
  if (SOURCES_OTA.has(src)) return 'ota'
  // « iCal import 1 », « iCal import 2 »… : une autre connexion, que ce canal
  // ne rendra pas.
  if (SOURCES_CONSERVEES.has(src) || src.startsWith('ical')) return 'conservee'
  return 'inconnue'
}
...
async function auditPurge (supabase, bien, { maintenant = new Date() } = {}) {
  ...
  // ⚠ ON NE NEUTRALISE QUE CE QUI SERA REIMPORTE — REGLE DE THIERRY
  // (10 septembre 2026) : « les reservations en direct doivent tout simplement
  // rester en base et se poursuivre ».
  //
  // Elle est juste, et elle donne le bon critere : la neutralisation existe pour
  // eviter qu'un sejour revienne SOUS UN AUTRE IDENTIFIANT. Un sejour sans code
  // OTA ne vient d'aucun canal, donc aucun canal ne le rendra : il ne peut pas
  // faire doublon, et le neutraliser aurait supprime une reservation VIVANTE du
  // planning — nuits remises en vente, menage annule, voyageur oublie.
  //
  // Et ses nuits restent fermees chez le nouveau provider : le stock est CALCULE
  // a la poussee (`lib/nuits-occupees.js` la compte comme occupee). C'est ce qui
  // rend inutile de la recreer a la main chez le canal (l'etape 2.10 du plan de
  // bascule tombe).
  const estReimportable = (r) => natureDuSejour(r.snapshot) === 'ota'
  ...
  const duFutur = sejours.filter(r => estReimportable(r)
    && estDuFutur(r.snapshot, aujourdhui)
  ...
  return {
    ...
    // Les directes a venir : elles RESTENT, et on le dit — pour que personne ne
    // les croie oubliees.
    directes_conservees: sejours.filter(r => !estReimportable(r)
      && estDuFutur(r.snapshot, aujourdhui)
      && readStatus(r.snapshot, bien.provider) === STATUS.CONFIRMED)
      .map(r => ({ booking_id: r.booking_id, arrivee: r.snapshot.arrival,
        depart: r.snapshot.departure, source: (r.snapshot || {}).source || null,
        nature: natureDuSejour(r.snapshot) })),
    // ⚠ CE QU'ON N'A PAS SU CLASSER. Une source inconnue est conservee — le
    // doute profite a la reservation vivante — mais elle doit se VOIR : si un
    // canal la rendait, elle ferait doublon, et personne ne l'aurait dit.
    sources_inconnues: [...new Set(sejours
      .filter(r => estDuFutur(r.snapshot, aujourdhui)
        && readStatus(r.snapshot, bien.provider) === STATUS.CONFIRMED
        && natureDuSejour(r.snapshot) === 'inconnue')
      .map(r => String((r.snapshot || {}).source || '(vide)')))],
    // Ce qui est du futur mais deja hors jeu : annule, demappe, en demande. Les
    // directes VIVANTES n'en font pas partie — elles sont comptees a part, sinon
    // l'apercu annoncait « 1 hors jeu » pour une reservation bien reelle.
    hors_jeu_au_futur: sejours.filter(r => estDuFutur(r.snapshot, aujourdhui)
      && ![STATUS.CONFIRMED, STATUS.BLOCKED].includes(readStatus(r.snapshot, bien.provider))).length,
...
// Ce qui reste vivant et qu'on ne touche pas, dit de la MEME facon partout :
// apercu, retour d'execution, ou constat qu'il n'y a rien a faire. Une seule
// branche qui le disait, c'etait une promesse a moitie tenue.
function resumeConserves (audit) {
  let out = ''
  if (audit.directes_conservees.length) {
    out += ` ${audit.directes_conservees.length} reservation(s) hors canal a venir se `
      + 'poursuivent (aucun canal ne les rendra) : '
      + audit.directes_conservees.map(d => `${d.arrivee}→${d.depart}`).join(', ')
      + '. Leurs nuits restent fermees chez le nouveau provider par le stock calcule.'
  }
  if (audit.sources_inconnues.length) {
    out += ` ⚠ Source(s) non classee(s) et donc CONSERVEE(S) : `
      + audit.sources_inconnues.join(', ')
      + '. A trancher : si un canal les rend, elles feront doublon.'
  }
  return out
}
...
async function purgerLeFutur (supabase, bien, { dryRun = true, maintenant = new Date() } = {}) {
  ...
  if (!audit.a_annuler) {
    return { ok: true, dry_run: !!dryRun, audit,
      directes_conservees: audit.directes_conservees,
      sources_inconnues: audit.sources_inconnues,
      // ⚠ « AUCUN SEJOUR A VENIR » AURAIT ETE FAUX. Apres un passage reussi,
      // `a_annuler` tombe a 0 alors que les directes du futur sont bien la : dire
      // « aucun sejour a venir » les aurait fait passer pour inexistantes.
      note: 'Aucun sejour d\'OTA a venir a neutraliser : rien ne peut faire doublon au '
        + `remapping. Les ${audit.passes} sejours passes restent intacts.`
        + resumeConserves(audit) }
  }
  ...
  if (dryRun) {
    return { ok: true, dry_run: true, audit,
      ...
      // Les directes a venir : elles RESTENT, et on le dit — pour que personne
      // ne les croie oubliees.
      directes_conservees: audit.directes_conservees,
      sources_inconnues: audit.sources_inconnues,
      va_faire: [
        ...
        `les ${audit.directes_conservees.length} reservation(s) hors canal a venir se poursuivent`
      ],
...
async function etatPurge (supabase, bien, { maintenant = new Date() } = {}) {
  ...
  if (!audit.a_annuler) {
    return { etat: 'fait',
      message: 'Aucun sejour d\'OTA a venir : rien ne peut faire doublon au remapping. '
        + `${audit.passes} sejour(s) passe(s) conserves.` + resumeConserves(audit) }
  }
