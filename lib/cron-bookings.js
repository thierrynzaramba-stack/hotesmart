...
async function detectBookingChanges(userId, beds24Key, property, tokens, results) {
  ...
  // partir des bookings. Deduit le bon etat et met a jour property_status.
  // Ne regresse jamais un statut 'ready' (seul markReady manuel peut y mener).
  try {
    await syncStatusFromBookings(userId, property.id, bookings)
  } catch (err) {
