...
const { motifNonSync } = require('./cles-migrees')
...
async function detectBookingChanges(userId, beds24Key, property, tokens, results) {
  ...
  // ⚠ ET LA GARDE EST FERMEE : « je ne sais pas » vaut « je n'y touche pas ».
  // Mesure du 14 septembre : lors de trois cycles isoles, cette garde est
  // passee AVEUGLE (table des cles migrees illisible) et l'ancienne cle du 23
  // s'est rouverte — 82 sejours arraches a la fiche Channex, puis SEIZE
  // menages annules par le writer cinq minutes plus tard. Voir [cles-migrees].
  const motif = await motifNonSync(supabase, userId, property.id, 'beds24')
  if (motif) {
    // Deux faits differents, deux phrases differentes : confondre « ce bien est
    // migre » (normal, definitif) avec « la garde est aveugle » (anormal,
    // passager) enverrait le diagnostic dans la mauvaise direction.
    console.log(motif === 'migree'
      ? `[cron-bookings] cle migree, snapshots ignores : ${property.id}`
      : `[cron-bookings] garde AVEUGLE, bien ${property.id} NON traite ce cycle (incident leve)`)
    return
  }
