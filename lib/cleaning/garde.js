...
//   ET disponibles (conges, puis exceptions, puis regles RRULE).
//
...
//     voulue par la spec (§12.1) et l'etat actuel de Regina. ⚠ Un TABLEAU VIDE,
//     lui, est conserve tel quel : c'est « aucun jour », un choix explicite de
//     l'hote, et `attitreeCeJour` les distingue.
...
// Attitree ce jour-la ?
//
// ⚠ NULL ET TABLEAU VIDE NE DISENT PAS LA MEME CHOSE (revise au lot 3.5) :
//   - `weekdays` ABSENT/NULL = tous les jours. C'est la retrocompatibilite du
//     §12.1 : Regina n'a pas de `weekdays`, et lire le vide comme « aucun jour »
//     aurait vide le planning au premier deploiement ;
//   - `weekdays: []` = AUCUN JOUR, un choix EXPLICITE de l'hote. Depuis que
//     l'ecran envoie ce tableau, decocher tous les jours produit `[]` — et le
//     lire comme « tous les jours » faisait exactement l'inverse du geste :
//     l'hote retirait quelqu'un de tous les jours et la laissait attitree sept
//     jours sur sept, sans le moindre signe.
// Convention : 0 = dimanche … 6 = samedi, celle de `getUTCDay()` (§12.7).
function attitreeCeJour (liaison, date) {
  if (liaison.weekdays === null || liaison.weekdays === undefined) return true
  if (!liaison.weekdays.length) return false
...
function contexteDispo (bien, providerId) {
  ...
  // ⚠ LES CONGES PASSENT PAR LA MEME PORTE. Les oublier ici aurait ete la faute
  // la plus silencieuse du lot : `estDisponible` sait les lire, mais il ne lit
  // que ce qu'on lui donne — la garde du jour aurait designe quelqu'un en
  // vacances, et rien a l'ecran ne l'aurait dit.
  return { regles: lire(bien.regles), exceptions: lire(bien.exceptions),
           conges: lire(bien.conges) }
}
 ...
 *
 * @param {{userId, propertyId, liaisons, regles?: Map, exceptions?: Map, conges?: Map}} bien
