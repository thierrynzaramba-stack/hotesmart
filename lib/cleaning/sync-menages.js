...
async function loadContext(userIds, propertyIds) {
  ...
  const { data: props, error: propsErr } = await supabase
    .from('properties')
    // ⚠ `base_price` SERT AU JOURNAL DES PRIX, PAS AU MENAGE.
    // Le consommateur 4 du dispatcher (reouverture apres annulation) retombe
    // dessus quand `calendar_inventory` ne porte aucun tarif pour la nuit —
    // meme regle que runFullSync, « un prix de base est un prix ». L'oublier
    // ici ne leverait aucune erreur : toutes les nuits seraient comptees
    // « sans prix » et jamais rouvertes, en silence.
    // ⚠ `cancellation_policy` SERT A LA NOTIFICATION DE L'HOTE, pas au menage.
    // Le consommateur « notif hote » du dispatcher l'affiche dans l'annonce de
    // nouvelle reservation. L'oublier ici n'aurait leve aucune erreur : la ligne
    // « Conditions » aurait simplement disparu de l'e-mail, en silence.
    .select('id, user_id, provider_property_id, name, provider, address, checkin_time, checkout_time, base_price, cancellation_policy')
    .in('user_id', userIds)
