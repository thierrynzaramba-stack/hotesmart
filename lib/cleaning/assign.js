...
function dansLaFenetreDeProposition (date, maintenant = Date.now(), jours = JOURS_PROPOSITION) {
  ...
  // ⚠ Le PASSE reste proposable : un menage d'hier peut encore etre a faire, et
  // c'est justement le moment ou l'hote cherche quelqu'un en urgence.
  return ecart <= jours
}
...
function deciderParGarde (bien, date, opts = {}) {
  ...
  const file = candidates
    .filter(c => c.requiresAck !== false)
    .filter(c => !exclus || !exclus.has(String(c.providerId)))

  const proposable = dansLaFenetreDeProposition(date, maintenant, fenetre)
  const proposee = proposable ? (file[0] || null) : null
  const differee = !proposable && !!file.length
