...
async function chargerLiaisons (sb, couples) {
  ...
  // ⚠ PAGINEE, COMME LES DEUX AUTRES LECTURES. Elle etait la seule sans borne :
  // PostgREST tronque a 1000 lignes SANS erreur, et l'appelant le plus large —
  // l'ecran de garde, jusqu'a 200 biens — perdait alors des liaisons en silence.
  // Les biens concernes devenaient « personne de garde » sept jours sur sept :
  // exactement l'ecran FAUX que le 503 sur panne existe pour empecher.
  // ⚠ L'ordre de pagination est `id`, pas `rang` : il doit etre STABLE et unique,
  // sinon deux pages peuvent rendre la meme ligne et en sauter une autre. Le tri
  // par rang se fait ci-dessous, par groupe.
  ...
  const data = await lireTout(() => sb.from('property_cleaning_providers')
    .select('id, user_id, property_id, provider_id, rang, weekdays, requires_ack, active')
    .in('user_id', userIds)
    .in('property_id', propIds)
    .eq('active', true), 'liaisons', 'id')
...
async function chargerDisponibilites (sb, userIds, { du = null, au = null } = {}) {
  const vide = { regles: new Map(), exceptions: new Map(), conges: new Map() }
  ...
  // ⚠ LES CONGES SE CHARGENT ICI AUSSI, sans quoi le moteur assignerait pendant
  // les vacances : `estDisponible` les lit, mais il ne peut lire que ce qu'on lui
  // donne. La borne de fenetre porte sur `fin` — un conge commence AVANT la
  // fenetre peut tres bien la couvrir, et le filtrer sur `debut` l'aurait perdu.
  const conges = await lireTout(() => {
    let q = sb.from('conges_plages')
      .select('user_id, provider_id, debut, fin, motif')
      .in('user_id', ids)
    if (du) q = q.gte('fin', du)
    if (au) q = q.lte('debut', au)
    return q
  }, 'conges', 'id')
