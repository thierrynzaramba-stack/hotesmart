...
// Le conge qui couvre ce jour, ou `null`.
//
// ⚠ UN CONGE EST UNE PLAGE, PAS UNE COLLECTION DE JOURS (table `conges_plages`,
// 15 septembre 2026). C'est ce qui permet de le supprimer d'un geste, et c'est
// pour ca que l'ecran verrouille les jours couverts au lieu de les laisser
// basculer : gratter un jour au milieu d'un conge laisserait une plage qui dit
// une chose et un calendrier qui en montre une autre.
//
// ⚠ BORNES INCLUSIVES DES DEUX COTES, comme `prestataire_periodes` et comme la
// contrainte SQL. Le dernier jour d'un conge est un jour de conge.
//
// ⚠ DEUX CONGES QUI SE CHEVAUCHENT NE SONT PAS UNE ANOMALIE : on rend le
// premier trouve, et supprimer l'un laisse l'autre verrouiller ses jours. C'est
// le comportement juste — prolonger un conge en en posant un second par-dessus
// est un geste legitime.
function congeCouvrant (date, conges) {
  const cle = cleJour(date)
  if (!cle) return null
  for (const c of (conges || [])) {
    const d = cleJour(c.debut), f = cleJour(c.fin)
    if (!d || !f) continue                       // une plage illisible ne couvre rien
    if (cle >= d && cle <= f) return c
  }
  return null
}
 ...
 *
 * L'ordre compte, et il est celui grave le 15 septembre 2026 :
 *   1. un CONGE couvre ce jour -> ABSENTE, et le jour est verrouille a l'ecran.
 *      Il l'emporte sur tout, y compris sur une exception « disponible » : on ne
 *      se declare pas presente au milieu de ses propres vacances par megarde ;
 *   2. une EXCEPTION pour ce jour tranche, dans les deux sens. C'est ce qui
 *      permet de dire « pas ce samedi-la » sans defaire sa recurrence ;
 *   3. AUCUNE REGLE active = DISPONIBLE. C'est le cas de Regina, et c'est ce qui
 *      rend le systeme sans effet tant que personne n'a rien declare ;
 *   4. sinon, disponible si au moins une regle couvre ce jour.
 *
 * ⚠ LES ETAGES 2, 3 ET 4 SONT INCHANGES. Le conge s'ajoute AU-DESSUS : un appel
 * qui ne passe pas `conges` se comporte exactement comme avant.
 *
 * @param {string|Date} date  jour de calendrier
 * @param {{regles?: Array, exceptions?: Array, conges?: Array}} contexte
 */
function estDisponible (date, { regles = [], exceptions = [], conges = [] } = {}) {
  const cle = cleJour(date)
  if (!cle) return false   // une date illisible n'est pas un jour ou l'on travaille

  if (congeCouvrant(cle, conges)) return false
...
// La LECTURE INVERSE : ce que l'ecran doit recocher pour une regle existante.
//
// ⚠ POURQUOI ELLE VIT ICI, A COTE DE `construireRrule`. Les deux sont un aller
// et un retour : separees, elles divergent au premier ajustement. Le test de
// symetrie (construire puis relire) les tient ensemble.
//
// ⚠ AUCUNE CHAINE RRULE NE REMONTE A L'ECRAN, ET CETTE FONCTION EST CE QUI LE
// PERMET. La regle gravee au §2 de la spec interdit qu'une RRULE descende vers
// le client ou en remonte ; sans lecture inverse cote serveur, l'ecran ne
// pouvait pas recocher les cases d'une recurrence deja posee — il ne recevait
// qu'un libelle. Le libelle est pour l'oeil ; ceci est pour les cases.
//
// ⚠ LES DEUX CONVENTIONS DE JOURS NE SONT PAS LA MEME, et c'est le piege exact.
// La bibliotheque `rrule` compte LUNDI = 0 … DIMANCHE = 6 (norme RFC 5545).
// Cette application compte DIMANCHE = 0 … SAMEDI = 6 (celle de `getUTCDay()`,
// et celle de `weekdays` sur les liaisons). Confondre les deux decale toute la
// semaine d'un cran, en silence : « le week-end » deviendrait « vendredi et
// lundi », et personne ne verrait l'erreur avant le jour J.
function lireRrule (rrule) {
  if (!rrule) return null
  try {
    const r = rrulestr(String(rrule))
    const brut = r.options && r.options.byweekday
    if (!Array.isArray(brut) || !brut.length) return null
    // `byweekday` rend des entiers, ou des objets `Weekday` selon la version.
    const jours = [...new Set(brut.map(w => {
      const n = (w && typeof w === 'object' && 'weekday' in w) ? w.weekday : w
      return Number.isInteger(n) ? (n + 1) % 7 : null      // lundi=0 -> dimanche=0
    }).filter(j => j !== null))].sort((a, b) => a - b)
    if (!jours.length) return null
    const d = r.options.dtstart
    return {
      jours,
      cadence: Number.isInteger(r.options.interval) && r.options.interval > 0 ? r.options.interval : 1,
      ancre: d instanceof Date && !Number.isNaN(d.getTime()) ? cleJour(d) : null
    }
  } catch (e) {
    // ⚠ Une regle illisible ne fait pas tomber l'ecran : il affichera son libelle
    // sans pouvoir recocher les cases. C'est degrade, pas casse — et coherent
    // avec `regleCouvre`, qui la traite comme une regle qui ne couvre rien.
    console.error('[availability] regle illisible a la relecture:', e.message)
    return null
  }
}
