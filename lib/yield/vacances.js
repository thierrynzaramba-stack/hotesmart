...
function couverture (periodes, debut, fin, zone = null) {
  ...
  // ⚠ L'ETENDUE SE MESURE SUR LA ZONE DU BIEN quand elle est demandee — releve
  // en review. Prendre le `max` toutes zones confondues annonçait un horizon
  // plus lointain que la realite des que la zone du bien s'arretait plus tot
  // que les autres : la banniere de dette se taisait sur des mois deja hors
  // couverture pour CE logement.
  const retenues = zone ? periodes.filter(p => p.zone === zone) : periodes
  const base = retenues.length ? retenues : periodes
  const min = base.reduce((a, p) => (a == null || p.date_debut < a) ? p.date_debut : a, null)
  const max = base.reduce((a, p) => (a == null || p.date_fin > a) ? p.date_fin : a, null)
