...
async function preparerContexte (supabase, bien, compte, options = {}) {
  ...
  const ventesParDate = new Map()
  for (const e of eclatements) {
    if (!e || !e.compte) continue
    for (const n of e.nuits || []) {
      if (n.prix == null || !Number.isFinite(n.prix) || n.prix <= 0) continue
      if (!ventesParDate.has(n.date)) ventesParDate.set(n.date, [])
      ventesParDate.get(n.date).push({ prix: n.prix, hors_reference: !!(n.hors_reference || e.long_sejour) })
    }
  }
...
function preuveN1 (ctx, date) {
  ...
  const valides = v.filter(x => !x.hors_reference)
  const liste = valides.length ? valides : v
  return {
    date: cmp.date,
    meme_segment: !!cmp.meme_segment,
    alignement: cmp.alignement,
    prix: liste.length ? Math.min(...liste.map(x => x.prix)) : null,
    hors_reference: !valides.length && v.length > 0,
    ventes: valides.length
  }
}

module.exports = { preparerContexte, prixDeLaNuit, preuveN1, ANS_REFERENCE, JOURS_COMPARABLES }
