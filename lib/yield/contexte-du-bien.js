...
// a son radar) sans consequence : la segmentation d'une nuit ne depend pas des
// bornes du contexte — verifie sur un pont, un week-end prolonge et un
// evenement qui les depassent, et sur un contexte de plus de 2000 jours
// (tests/parite-ecran-moteur.test.js). ⚠ Ce dernier cas etait FAUX jusqu'au
// lot V2.0.1 : au-dela de 2000 jours, ponts et week-ends prolonges
// disparaissaient en silence (`lib/yield/reference.js`, `parTranches`). Ce que
...
const { nuitComparable } = require('./comparable')
...
async function preparerContexte (supabase, bien, compte, options = {}) {
  ...
  // Ce que chaque nuit a rapporte, pour la PREUVE de la fourchette Exceptionnel
  // (lot V2.0.7) : le prix obtenu l'an dernier sur la nuit comparable. Une
  // nuit hors reference (exception, fermeture) ou d'un long sejour degressif
  // ne prouve pas ce qu'une nuit vaut : elle est gardee, marquee.
  const ventesParDate = ventesParDateDe(eclatements)

  return { auj, bien, grille, contexte, evenements, reglages, pressionParMois, pickupParMois,
    capacitesMois, ouverts, ouvertureConnue, ventesParDate,
...
function prixDeLaNuit (ctx, date, { ouverte = null, vendue = null } = {}) {
  ...
  return S.suggerer({
    ...
    // Le reglage de la nuit SOUS-JACENTE, quand un evenement sans historique
    // s'y replie (« vacances de Noel : +1 cran » vaut pour le reveillon).
    reglageDe: s => reglagePour(ctx.reglages, s),
    // La preuve N-1 (fourchette Exceptionnel ET plancher N-1 a tous les
    // niveaux) : une fonction, appelee au plus une fois par nuit — la cascade
    // ne tourne pas pour une nuit vendue, fermee ou sans prix.
    preuveN1: () => preuveN1(ctx, date),
    bien: ctx.bien
  })
}

/**
 * Ce que chaque nuit a rapporte. Une reservation qui ne COMPTE pas (annulee,
 * non confirmee) n'est pas une vente. ⚠ Une nuit vendue SANS prix exploitable
 * (reservation recreee a une migration de canal) reste une vente, `prix: null`
 * — la jeter ferait dire « non vendue » a une nuit vendue (review V2.0.7).
 */
function ventesParDateDe (eclatements) {
  const out = new Map()
  for (const e of eclatements || []) {
    if (!e || !e.compte) continue
    for (const n of e.nuits || []) {
      const valide = n.prix != null && Number.isFinite(n.prix) && n.prix > 0
      if (!out.has(n.date)) out.set(n.date, [])
      out.get(n.date).push({ prix: valide ? n.prix : null,
        hors_reference: !!(n.hors_reference || e.long_sejour) })
    }
  }
  return out
}
...
/**
 * LA PREUVE : la nuit comparable de l'an dernier (la cascade de l'ecran, une
 * seule regle) et ce qu'elle a rapporte. Plusieurs ventes le meme jour (bien a
 * plusieurs unites) : la plus BASSE — chaque unite s'est vendue au moins a ce
 * prix, c'est ce qui est prouve.
 */
function preuveN1 (ctx, date) {
  const cmp = nuitComparable(date, { contexte: ctx.contexte })
  if (!cmp || !cmp.date) return null
  // ⚠ UNE NUIT COMPARABLE A VENIR NE PROUVE RIEN (review V2.0.7) : au-dela
  // d'un an de fenetre, la cascade rend une date future ; reservee, son prix
  // est peut-etre celui que YieldFlow vient de poser — « prix obtenu » serait
  // faux ; libre, « non vendue » serait « je ne sais pas » dit « non ».
  const finRef = ctx.finRef || ctx.auj
  if (finRef && cmp.date > finRef) {
    return { date: cmp.date, meme_segment: !!cmp.meme_segment, alignement: cmp.alignement,
      prix: null, pas_encore_passee: true, hors_reference: false, ventes: 0 }
  }
  const v = (ctx.ventesParDate && ctx.ventesParDate.get(cmp.date)) || []
  const chiffrees = v.filter(x => x.prix != null)
  const valides = chiffrees.filter(x => !x.hors_reference)
  const liste = valides.length ? valides : chiffrees
  return {
    date: cmp.date,
    meme_segment: !!cmp.meme_segment,
    alignement: cmp.alignement,
    prix: liste.length ? Math.min(...liste.map(x => x.prix)) : null,
    prix_inconnu: v.length > 0 && !chiffrees.length,
    hors_reference: !valides.length && chiffrees.length > 0,
    ventes: valides.length
  }
}

module.exports = { preparerContexte, prixDeLaNuit, preuveN1, ventesParDateDe, ANS_REFERENCE, JOURS_COMPARABLES }
