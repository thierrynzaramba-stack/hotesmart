...
const SEGMENT_PARENT = { pont: 'ferie' }
...
function segmenterJour (iso, contexte = {}) {
  const { zoneBien = null, vacances = [], feries = null, ponts = null,
    fenetre = null, evenements = null } = contexte
  if (!estJourISO(iso)) return null
  ...
  if (evenements && evenements.length) {
    ...
    if (ici.length) {
      ...
      // A duree egale, le nom tranche : deterministe avant tout.
      const choisi = [...ici].sort((a, b) =>
        (duree(a) - duree(b)) || String(a.nom).localeCompare(String(b.nom)))[0]
...
function construireContexte (options = {}) {
  ...
  if (!estJourISO(debut) || !estJourISO(fin) || fin < debut) {
    return { zoneBien, vacances: [], evenements: [], feries: new Map(),
      ponts: new Map(), fenetre: null, non_calculable: 'fenetre_invalide' }
  }
  return {
    zoneBien,
    vacances: vacances || [],
