...
function nuitComparable (date, options = {}) {
  ...
  if (String(s.segment).startsWith('evenement:')) {
    const occsHote = (contexte.evenements || [])
      .filter(e => e && e.segment === s.segment)
    const ici = occsHote.find(e => date >= e.date_debut && date <= e.date_fin)
  ...
  if (commerciale) {
    ...
    // Un contexte qui ne couvre pas l'an dernier ne rend pas une date « vue ».
    const horsContexte = j => {
      const c = segmenterJour(j, contexte)
      return !c || c.non_calculable === 'hors_fenetre_du_contexte'
    }
    if (horsContexte(`${an}-${date.slice(5)}`)) return aucun(date, RAISONS.SEGMENT_INCONNU)
  ...
  if (s.segment === SEGMENTS.VACANCES_ZONE || s.segment === SEGMENTS.VACANCES_AUTRE) {
    ...
    //
    // ⚠ LA NUIT RETENUE N-1 N'EST JAMAIS UNE DATE COMMERCIALE — releve en
    // review (23 septembre 2026). Elles ont leur propre comparaison, date a
    // date (etage a) : elles ne sont pas une « position dans les vacances ».
    // Sans cette garde, le samedi 13 fevrier 2027 pouvait se comparer au
    // samedi 14 fevrier 2026 — la Saint-Valentin vendue 295 € — et la colonne
    // « l'an dernier » publiait ce prix comme le reel d'un samedi de vacances.
    // ⚠ LES RANGS SE COMPTENT SUR TOUTES LES NUITS, comme avant : retirer les
    // dates commerciales du compte decalait les AUTRES nuits (le dernier
    // dimanche des vacances 2027 se comparait a l'avant-dernier de 2026). Seule
    // la nuit retenue est remplacee, par la plus proche du meme jour de
    // semaine dans les memes vacances (la suivante a egalite) — et on le dit.
    // ⚠ LES FERIES, EUX, RESTENT CANDIDATS : arbitrage deja ecrit (test « un
    // ferie tombe DANS les vacances N-1 ») — la Toussaint un samedi de vacances
    // est bien le samedi de pointe comparable, et `meme_segment: false` le dit.
    const memeJour = j => jourDeSemaine(j) === s.jour_semaine
    const estCommerciale = j => {
      const c = segmenterJour(j, contexte)
      return !!(c && c.origine === 'commercial')
    }
    ...
    const iVise = rangIci < candidats.length ? rangIci : candidats.length - 1
    if (!estCommerciale(candidats[iVise])) {
      if (rangIci < candidats.length) {
        return decrire(date, candidats[iVise], contexte, ETAGES.EVENEMENT_MOBILE,
      ...
      return decrire(date, candidats[iVise], contexte,
        ETAGES.EVENEMENT_MOBILE, ALIGNEMENTS.JOUR_DANS_VACANCES, s,
        { rang: rangIci + 1, rang_n1: candidats.length })
    }
    for (let d = 1; d < candidats.length; d++) {
      for (const i of [iVise + d, iVise - d]) {
        if (i >= 0 && i < candidats.length && !estCommerciale(candidats[i])) {
          return decrire(date, candidats[i], contexte, ETAGES.EVENEMENT_MOBILE,
            ALIGNEMENTS.JOUR_DANS_VACANCES, s, { rang: rangIci + 1, rang_n1: i + 1 })
        }
      }
    }
    return aucun(date, RAISONS.JOUR_ABSENT_DES_VACANCES)
  }
