...
// ⚠ LES DEUX SEULES DATES AJOUTEES A LA MAIN, ET ELLES SONT NOMMEES.
// Le reveillon de Noel et celui du Nouvel An ne sont pas des jours feries : le
// moteur les voit comme des nuits de vacances de Noel ordinaires, et l'etage b
// les alignerait sur un 27 decembre. Ce sont les deux nuits de l'annee ou la
// DATE fait le prix. Toute autre date se compare par sa nature, pas par son
// numero.
const REVEILLONS = ['12-24', '12-31']
...
function nuitComparable (date, options = {}) {
  ...
  if (REVEILLONS.includes(date.slice(5))) {
    const t = `${Number(date.slice(0, 4)) - 1}-${date.slice(5)}`
    if (estJourISO(t)) {
      return decrire(date, t, contexte, ETAGES.DATE_FIXE, ALIGNEMENTS.REVEILLON, s)
    }
  }
