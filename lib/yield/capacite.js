...
const NON_CALCULABLE = {
  ...
  // Toute la periode est au-dela de la fenetre glissante d'un bien
  // auto-pilote : personne n'a rien decide, 0/0 — non calculable, PAS zero.
  HORS_FENETRE: 'periode_hors_fenetre'
}
...
async function joursOuverts (supabase, bien, debut, fin, options = {}) {
  ...
  // ⚠ `lignes` PERMET DE NE LIRE QU'UNE FOIS POUR PLUSIEURS PERIODES.
  // Le radar douze mois appelle cette fonction 24 fois — un mois et son N-1,
  // douze fois. Mesure sur La bulle : 2 673 ms de requetes pour une page qui
  // s'ouvre chaque matin. Avec les lignes fournies, c'est UNE lecture et
  // 24 classifications en memoire.
  //
  // ⚠ LA REGLE NE BOUGE PAS D'UN IOTA : seule la SOURCE des lignes change.
  // Dupliquer la classification dans l'appelant aurait fait deux verites sur
  // « qu'est-ce qu'un jour ouvert » — exactement ce que ce module existe pour
  // eviter. Un test compare les deux chemins sur les memes donnees.
  const { joursExclus = null, aujourdHui = null, estimerLePasse = true,
    lignes: fournies = null } = options
  ...
  const base = {
    ...
    jours_sans_ligne: 0,
    jours_sans_prix: 0,
    jours_hors_fenetre: 0,
    jours_attendus_sans_ligne: 0,
    detail: null,
    detail_hors_fenetre: null
  }
  if (jours === null) {
    return { ...base, raison: NON_CALCULABLE.PERIODE_TROP_LONGUE }
  }
  // ⚠ `supabase` N'EST EXIGE QUE SI L'ON DOIT LIRE. Quand l'appelant fournit
  // les lignes, il n'y a plus de lecture a faire — exiger le client rendait
  // « parametres_invalides » sur un appel parfaitement valide, et le radar
  // affichait « je ne sais pas » sur douze mois. Le reste de la garde ne bouge
  // pas : sans bien ni fenetre, il n'y a rien a classer.
  if ((!supabase && !fournies) || !bien?.id || !jours.length) {
  ...
  // ⚠ LA GARDE PORTE SUR LA PARTIE FUTURE, PAS SUR LE DEBUT DE LA PERIODE.
  // Releve en review, et le defaut etait grave : desarmer la garde des que
  // `debut < auj` laissait passer une periode A CHEVAL — le mois en cours.
  // Un bien Beds24 traversait alors la garde, on interrogeait sa memoire
  // inexistante, et tous ses jours FUTURS repartaient « fermes ». Mesure du
  // scenario : 11 jours estimes ouverts, 19 comptes fermes, 20 nuitees — un
  // taux d'occupation de 182 %, `calculable: true`. C'est exactement le
  // « 360 jours pour fermes » que cette garde existe pour empecher.
  //
  // Le passe, lui, reste estimable pour TOUS les providers : c'est justement
  // parce que la memoire n'existe pas qu'on estime.
  const contientDuFutur = fin >= auj
  if (!estRelieAuCanal(bien) && contientDuFutur) {
  ...
  const lignes = []
  if (fournies) {
    // ⚠ ON REFILTRE SUR LA FENETRE **ET SUR LE BIEN**. L'appelant fournit une
    // plage large ; sans le premier filtre, un mois compterait les jours
    // ouverts de toute l'annee.
    //
    // ⚠ ET SANS LE SECOND, LE COMMENTAIRE « seule la SOURCE change » ETAIT
    // FAUX — releve en review. Le chemin base fait `.eq('property_id',
    // bien.id)` ; celui-ci ne classait que sur la date. Des lignes d'un AUTRE
    // bien melees a l'appel auraient gonfle le denominateur du taux
    // d'occupation, en silence. L'appelant d'aujourd'hui filtre correctement,
    // donc aucun effet en production — mais le garde-fou annonce n'existait
    // pas, et le test cense le couvrir ne pouvait pas le voir.
    for (const l of fournies) {
      if (!l || l.date < debut || l.date > fin) continue
      // `property_id` peut etre absent de la projection de l'appelant : on ne
      // refuse que ce qui est EXPLICITEMENT d'un autre bien.
      if (l.property_id != null && String(l.property_id) !== String(bien.id)) continue
      lignes.push(l)
    }
  } else {
  ...
  // ⚠ LA FENETRE GLISSANTE D'UN BIEN AUTO-PILOTE (lot 4.6.0, spec §2 ter).
  // `null` pour un bien en mode calendrier, ou yieldflow sans fenetre reglee :
  // alors RIEN ci-dessous ne change. C'est la garantie du lot.
  // ⚠ CALCULEE ICI, AVANT LA GARDE « aucune ligne » — releve en review. Un
  // mois affiche entierement au-dela de la fenetre n'a aucune ligne : la
  // garde le rendait « futur sans memoire d'intention » (« son calendrier n'a
  // pas encore ete enregistre ») pendant que chaque ligne du meme mois disait
  // « pas encore ouverte ». Deux verites dans une seule page.
  const finFenetre = finDeFenetre(bien, auj)
  if (finFenetre != null && !lignes.length && jours.every(j => j > finFenetre)) {
    return { ...base, raison: NON_CALCULABLE.HORS_FENETRE,
      jours_hors_fenetre: jours.length, detail_hors_fenetre: jours.slice() }
  }

  // Cas 2 : memoire jamais amorcee sur cette periode.
  // ⚠ MEME ASYMETRIE QUE CI-DESSUS, RELEVEE EN REVIEW. Une memoire vide sur
  // une periode A CHEVAL rendait `calculable: true` avec tous les jours futurs
  // comptes fermes — et SANS drapeau, puisque `estimee` ne couvre que le passe.
  // Denominateur faux, credible, silencieux.
  // Le futur sans memoire n'est pas « ferme » : il est INCONNU.
  if (!lignes.length && (contientDuFutur || !estimerLePasse)) {
    return { ...base, raison: contientDuFutur ? NON_CALCULABLE.FUTUR_NON_AMORCE : NON_CALCULABLE.VIDE }
  }

  const parJour = new Map(lignes.map(l => [l.date, l]))
  const ouverts = []
  // ⚠ LA CLASSIFICATION PAR NUIT EST PORTEE PAR LE RESULTAT, comme `detail`
  // porte les ouverts — releve en review. L'endpoint la REDECIDAIT de son cote
  // (« sans ligne et au-dela ») : deux endroits encodaient « une ligne reelle
  // prime sur la fenetre », et le jour ou un appelant passe `joursExclus`, la
  // tuile compte une fermee pendant que la ligne dit « pas encore ouverte ».
  // Trois recopies, trois verites — le schema que ce lot dit vouloir eviter.
  const horsFenetreDates = []
  let fermes = 0
  let sansLigne = 0
  let horsFenetre = 0        // nuits au-dela de la fenetre : NI ouvertes NI fermees
  let attendusSansLigne = 0  // nuits DANS la fenetre sans ligne : le moteur aurait du ouvrir
  let sansPrix = 0
  let estimes = 0            // jours passes ouverts PAR CONVENTION
  let estimesFermes = 0      // jours passes fermes par une exception declaree
  for (const j of jours) {
    // ⚠ UNE FERMETURE DECLAREE PRIME SUR TOUT — releve en review.
    // Elle n'etait consultee que pour les jours SANS ligne : une exception
    // posee par l'hote sur un jour passe portant une ligne perimee (ecrite a
    // la migration, `avail: 1`) etait ignoree, et le jour comptait ouvert.
    // « L'hote sait mieux » ne souffre pas d'exception — c'est sa declaration.
    ...
    if (!l) {
      ...
      if (estimerLePasse && j < auj) { estimes++; ouverts.push(j); continue }
      // ⚠ « PAS ENCORE OUVERTE » N'EST PAS « FERMEE » — arbitrage C du §2 ter.
      // Au-dela de la fenetre, l'hote n'a rien decide : la nuit sort du
      // calcul, ni au numerateur ni au denominateur. La compter fermee ferait
      // s'effondrer le TO d'un bien a fenetre de 120 jours (245 nuits
      // « fermees » par an) et Yield conclurait qu'il faut baisser les prix.
      // Le TO lit `jours_ouverts` : elle n'y etait deja pas. Ce qui change,
      // c'est qu'elle n'est plus comptee dans `jours_fermes` — et le mot.
      if (finFenetre != null && j > finFenetre) { horsFenetre++; horsFenetreDates.push(j); continue }
      // ⚠ EN DECA DE LA FENETRE ET SANS LIGNE, C'EST UNE ANOMALIE : le moteur
      // aurait du ouvrir cette nuit. Elle reste comptee fermee — rien ne s'y
      // vend, c'est la verite — mais le compteur le DIT au lieu de le noyer
      // dans « sans ligne ». L'alarme viendra au 4.6.5 ; ici, la donnee.
      if (finFenetre != null && j >= auj) attendusSansLigne++
  ...
  return {
    ...
    // ⚠ SUR UNE PERIODE PASSEE, CE COMPTEUR TOMBE A ZERO : les jours sans
    // ligne partent desormais en `estimes`. Le signal « memoire d'intention
    // jamais completee » est donc porte par `jours_estimes_ouverts` pour le
    // passe, et par celui-ci pour le futur. Un appelant qui alerte doit lire
    // les DEUX.
    ...
    // Nuits au-dela de la fenetre glissante d'un bien auto-pilote : ni
    // ouvertes ni fermees, exclues du calcul. Toujours 0 pour un bien sans
    // fenetre — la ligne ci-dessus n'a alors jamais tire.
    jours_hors_fenetre: horsFenetre,
    // Nuits DANS la fenetre, futures, sans ligne : le moteur aurait du les
    // ouvrir. Comptees fermees (c'est vrai), mais nommees (c'est anormal).
    jours_attendus_sans_ligne: attendusSansLigne,
    // Les dates hors fenetre, pour que l'appelant n'ait pas a les redecider.
    detail_hors_fenetre: horsFenetreDates,
