...
function suggerer (options = {}) {
  ...
  if (indice === niveaux.length - 1 && plafond != null && plafond > niveau.prix) {
    ...
    const f = primeExceptionnel({ niveau, plafond, plafondBrut: grille.base.plafond_brut, preuve, pression,
      unites, typeDeNuit: typeDeNuit(preuve) })
    ...
    if (preuve && preuve.prix != null && borneN1 > niveau.prix) {
      ...
      } else if (f.motif_sans_prime === MOTIFS_SANS_PRIME.PRIME_RETIREE && f.prix_sans_pression > niveau.prix) {
...
function primeExceptionnel ({ niveau, plafond, plafondBrut = null, plafondEstLeNiveau = false,
  preuve, pression, unites = 1, typeDeNuit = null }) {
  const dateFr = iso => `${iso.slice(8, 10)}/${iso.slice(5, 7)}/${iso.slice(0, 4)}`
