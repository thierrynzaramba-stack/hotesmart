...
function suggerer (options = {}) {
  ...
  const mesure = parJour ? parJour.get(`${s.segment}|${s.jour_semaine}`) : null
  ...
  const cransHote = regl && regl.crans != null && Number.isFinite(regl.crans)
    ? Math.max(-AMPLITUDE_MAX * 2, Math.min(AMPLITUDE_MAX * 2, Math.round(regl.crans)))
    : null
  ...
  if (cransHote != null && indiceModele != null) {
    ...
    base.cle_reglage = regl.cle || null
    ...
    if (!ps) base.position_sous_jacente = SEGMENTS.HORS_VACANCES
  ...
  base.couches.push({
    ...
    detail: source === 'mesure'
      ? `mesure directe du couple (${nomLisible(s)}, ${s.jour_semaine}) :`
