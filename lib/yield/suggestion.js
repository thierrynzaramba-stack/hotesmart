...
function suggerer (options = {}) {
  ...
  const indiceStructure = j => {
    ...
    if (fiable(p)) return p.indice
    return fiable(structSeg) ? structSeg.indice : null
  }
  const typeDeNuit = pr => {
  ...
  base.fourchette = { min: niveaux[0].prix,
    max: base.fourchette_exceptionnel ? base.fourchette_exceptionnel.plafond : niveaux[niveaux.length - 1].prix }
  ...
  if (plafonne) {
    ...
    return {
      ...
      resume: plafondEstLeNiveau
        ? `Plafonné à ${cible} €, le haut de votre grille — le ${dateFr(preuve.date)} s’est vendu ${Math.floor(preuve.prix)} €, mais aucune deuxième réservation n’a payé plus sur ce logement. Vous pouvez poser un prix plus haut à la main (✎).`
        : seule
        ...
        : `Plafonné à ${cible} € — le prix rond sous les ${Math.floor(plafondBrut)} € atteints par deux réservations (${Math.floor(preuve.prix)} € le ${dateFr(preuve.date)}). Vous pouvez poser un prix plus haut à la main (✎).`,
