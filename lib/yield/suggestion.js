...
//
// ⚠ LE PLAFOND EST UNE BORNE DE SECURITE, PAS UN OBJECTIF. Il ne se retire pas
// « par simplification » : c'est lui qui borne le cliquet N-1 (un prix prouve
// devient le plancher de l'annee suivante, qui devient celui de la suivante —
// une annee exceptionnelle se transmettrait indefiniment sans lui).
//
 ...
 */
function primeExceptionnel ({ niveau, plafond, preuve, pression }) {
  ...
  if (preuve.prix <= niveau.prix) {
    return sans(MOTIFS_SANS_PRIME.PAS_PLUS_CHER,
      `${bas} — l’an dernier, la nuit comparable s’est vendue ${Math.round(preuve.prix)} € le ${dateFr(preuve.date)} : pas plus cher que ce niveau`,
  ...
  if (preuve.prix - niveau.prix < SEUIL_PREUVE) {
    return sans(MOTIFS_SANS_PRIME.ECART_FAIBLE,
      `${bas} — l’an dernier, ${Math.round(preuve.prix)} € le ${dateFr(preuve.date)} : moins de ${SEUIL_PREUVE} € au-dessus, on ne bouge pas`,
  ...
  return {
    ...
    resume: `Exceptionnel · ${cible} € — prix obtenu le ${dateFr(preuve.date)} sur la nuit comparable`
      + (amincie ? ' (référence amincie : une seule vente)' : ''),
