...
const { segmenterJour, segmenterSansEvenements, contexteSansEvenements, SEGMENTS,
  SEGMENT_PARENT, segmentParent, jourDeSemaine } = require('./reference')
...
// ─── LA FOURCHETTE EXCEPTIONNEL (lot V2.0.7, arbitrage de Thierry, 23 sept. 2026)
// ⚠ « EXCEPTIONNEL » EST UNE FOURCHETTE, PAS UN PRIX. Son bas est le prix du
// niveau (P92) ; son haut est le prix atteint par au moins DEUX reservations.
// Une nuit n'y monte QUE sur preuve : le prix obtenu l'an dernier sur sa nuit
// comparable. Sans preuve, pas de prime — jamais le plafond par defaut.
//
// ⚠ LE PLAFOND : LE PRIX ATTEINT PAR AU MOINS DEUX RESERVATIONS DISTINCTES
// (arbitrage de Thierry, 23 septembre 2026, option b). Une seule reservation ne
// fixe pas ce que la machine pose seule — c'est une barre PLUS BASSE que les
// 8 nuits et 3 reservations du reste du moteur, pas plus haute. Au-dela, l'hote
// pose lui-meme (✎, prioritaire) : le plafond ne limite que ce que la MACHINE
// fait seule.
// ⚠ CE N'EST PAS UNE GARANTIE CONTRE UNE VENTE ABERRANTE. Il la neutralise
// seulement parce qu'elle est SEULE : deux reservations aberrantes au meme prix
// passeraient. Limite connue, ecrite.
// ⚠ C'EST UNE DONNEE VIVANTE, PAS UNE CONSTANTE : recalculee a chaque lecture
// de la grille, elle monte seule des qu'une deuxieme reservation atteint un
// prix plus haut. Ne jamais la figer dans une constante ni un reglage.
// ⚠ MEME ENSEMBLE FILTRE QUE LA PREUVE : reservations qui comptent, hors
// reference et longs sejours ecartes, prix > 0 (`construireGrille` pour l'un,
// `ventesParDateDe` pour l'autre ; un test tient les deux d'accord).
//
// ⚠ LA PRESSION DECIDE, ELLE N'AJOUTE AUCUN EURO. Chaque prix affiche reste un
// prix deja obtenu : inventer des euros par cran contredirait tout le moteur.
// Un mois qui se vend nettement moins bien que l'an dernier (le seuil de la
// couche pression) RETIRE la prime — la demande de cette annee contredit la
// preuve de la precedente. C'est une falaise (−24 % garde toute la prime,
// −26 % la perd) : acceptable parce que la nuit retombe sur un niveau lui aussi
// prouve, a condition de le DIRE — jamais un prix qui chute sans phrase.
const SEUIL_PRESSION = 0.25
// ⚠ LE MEME SEUIL QUE LE PLANCHER N-1 (point B) : relever un prix sur la preuve
// de l'an dernier est UN geste, il n'a qu'un seuil. Sous deux pas d'arrondi,
// on ne bouge pas (168 € prouves ne font pas passer 165 € a 170 €).
const SEUIL_PREUVE = 2 * 5

const MOTIFS_SANS_PRIME = {
  PAS_DE_COMPARABLE: 'pas_de_nuit_comparable',
  // ⚠ « JE NE SAIS PAS » N'EST PAS « NON » — releve en review : une nuit
  // comparable encore A VENIR (fenetre de plus d'un an) ou vendue sans prix
  // exploitable (reservation recreee a une migration) n'est pas « non vendue ».
  PAS_ENCORE_PASSEE: 'nuit_comparable_pas_encore_passee',
  PRIX_INCONNU: 'nuit_comparable_vendue_prix_inconnu',
  NON_VENDUE: 'nuit_comparable_non_vendue',
  AUTRE_SEGMENT: 'nuit_comparable_d_un_autre_segment',
  HORS_REFERENCE: 'nuit_comparable_hors_reference',
  AUTRE_TYPE_DE_NUIT: 'nuit_comparable_d_un_autre_type_de_jour',
  PAS_PLUS_CHER: 'nuit_comparable_pas_plus_chere',
  ECART_FAIBLE: 'preuve_a_moins_de_deux_pas',
  PRIME_RETIREE: 'prime_retiree_par_la_pression',
  RETIREE_DELAI: 'releve_retire_par_le_delai',
  REGLAGE_HOTE: 'releve_non_repris_reglage_de_l_hote'
}
...
function grilleDeBase (prix, options = {}) {
  const { seuil = SEUIL_NUITS, reservations = 0, seuilResas = SEUIL_RESERVATIONS,
    prixParReservation = null } = options
  ...
  // ⚠ LE PLAFOND DE LA FOURCHETTE EXCEPTIONNEL : le prix atteint par au moins
  // DEUX reservations distinctes (la 2e reservation la plus chere, chacune
  // comptee a sa nuit la plus chere), arrondi VERS LE BAS au pas — jamais un
  // euro au-dessus de ce que deux reservations ont paye. Sans deux
  // reservations, ou sans place au-dessus du niveau : pas de fourchette (`null`).
  const parResa = Array.isArray(prixParReservation)
    ? prixParReservation.filter(v => Number.isFinite(v) && v > 0).sort((a, b) => b - a) : []
  const plafondBrut = parResa.length >= 2 ? parResa[1] : null
  const plafond = plafondBrut != null ? Math.floor(plafondBrut / PAS_ARRONDI) * PAS_ARRONDI : null
  return {
    fiable: true,
    ...
    max: arrondi(max),
    niveaux,
    plafond: plafond != null && plafond > niveaux[niveaux.length - 1].prix ? plafond : null,
    plafond_brut: plafondBrut != null ? arrondi(plafondBrut) : null,
    non_calculable: null
  }
}
...
function construireGrille (eclatements, options = {}) {
  ...
  for (const e of eclatements || []) {
    ...
    for (const n of e.nuits || []) {
      ...
      toutes.push(n.prix)
      resasBien.add(id)
      maxParResa.set(id, Math.max(maxParResa.get(id) || 0, n.prix))
  ...
  const base = grilleDeBase(toutes,
    { seuil, reservations: resasBien.size, seuilResas, prixParReservation: [...maxParResa.values()] })
...
function suggerer (options = {}) {
  ...
  if (pression && pression.ecart != null && Number.isFinite(pression.ecart)) {
    ...
    else if (pression.ecart <= -SEUIL_PRESSION) dPression = -1
    else if (pression.ecart >= SEUIL_PRESSION) dPression = 1
    base.couches.push({
  ...
  // Le niveau AFFICHE : celui du pipeline, sauf si la preuve N-1 releve le
  // prix au-dela d'un niveau superieur (point B) — il suit alors le prix.
  let niveauAffiche = niveau
  ...
  // ─── LE TYPE DE NUIT (point B, option b′, arbitrage de Thierry,
  // 23 septembre 2026) ─────────────────────────────────────────────────────
  // ⚠ LE PLANCHER EST UN PLANCHER. Une preuve venue d'un jour qui se vend
  // MOINS BIEN est prudente — elle sous-estime, sans danger ; venue d'un jour
  // qui se vend MIEUX, elle est fausse — elle surestime. La regle « niveau
  // EGAL OU INFERIEUR » n'est pas un assouplissement du « meme type de nuit »,
  // c'est sa forme correcte. Egal accepte, strictement superieur refuse.
  // Le niveau d'un jour se lit dans la STRUCTURE DU BIEN hors vacances (le
  // niveau de chaque jour de semaine chez cet hote), jamais dans un calendrier
  // invente. La bulle : mercredi et jeudi Base — le reveillon du jeudi
  // 31/12/2026 se prouve par le mercredi 31/12/2025 ; samedi Tres haut —
  // le samedi 14/02/2026 a 295 € ne prouve rien pour le dimanche 14/02/2027.
  // Coeur de vie 23 : jeudi Base, vendredi Moyen — Noel 2025 (jeudi, 190 €)
  // prouve un minimum pour Noel 2026 (vendredi).
  // ⚠ HYPOTHESE, ECRITE COMME TELLE : l'ordre des jours vient de la structure
  // HORS vacances, et il est applique a des nuits DE vacances (Noel, jour de
  // l'an). On suppose que le relief semaine / week-end garde son ordre pendant
  // les vacances — coherent avec la regle « un contexte pousse semaine et
  // week-end du meme cran », mais non verifie (KB suggestion-yield §3 ter).
  // Vaut pour A (fourchette) comme pour B : c'est le meme geste.
  // ⚠ LE NIVEAU PROPRE DU JOUR, OU RIEN — releve en review : le repli sur la
  // position GLOBALE hors vacances donnait le meme indice a deux jours non
  // mesures, et « samedi et dimanche se vendent au meme niveau » s'affirmait
  // sans avoir ete mesure (la b′ redevenait l'option a sur tout bien maigre).
  // « Je ne sais pas » n'est pas « oui » : un jour non mesure refuse la preuve.
  const indiceStructure = j => {
    const p = parJour ? parJour.get(`${SEGMENTS.HORS_VACANCES}|${j}`) : null
    return fiable(p) ? p.indice : null
  }
  const typeDeNuit = pr => {
    if (!pr || !pr.date) return null
    const jP = jourDeSemaine(pr.date)
    if (jP === s.jour_semaine) return { ok: true }
    const a = indiceStructure(s.jour_semaine)
    const b = indiceStructure(jP)
    return { ok: a != null && b != null && b <= a, prudent: a != null && b != null && b < a,
      jour: s.jour_semaine, jour_preuve: jP,
      niveau_jour: a != null ? niveaux[a].nom : null, niveau_preuve: b != null ? niveaux[b].nom : null }
  }
  const lirePreuve = () => typeof options.preuveN1 === 'function' ? options.preuveN1() : (options.preuveN1 || null)
  const unites = Math.max(1, Number(bien && bien.inventory_units) || 1)
  // Le delai a −1 (14 jours ou moins) et le reglage de l'hote A LA BAISSE
  // retirent la prime / le relevement (arbitrages de Thierry, options a).
  // ⚠ Un reglage qui MONTE ne gene rien : la preuve n'est qu'un MINIMUM, elle
  // ne releve que si elle depasse le prix du niveau — jamais elle ne baisse.
  const delaiBaisse = dDelai === -1
  const reglageBaisse = source === 'reglage' && cransHote != null && cransHote < (cransMesures ?? 0)
    ? `${cransHote > 0 ? '+' : ''}${cransHote} cran${Math.abs(cransHote) > 1 ? 's' : ''}${base.cle_reglage ? ` sur « ${base.cle_reglage} »` : ''}`
    : null

  // ─── 3 bis. LA FOURCHETTE EXCEPTIONNEL : UNE PRIME SUR PREUVE ─────────────
  const plafond = grille.base.plafond
  if (indice === niveaux.length - 1 && plafond != null && plafond > niveau.prix) {
    const preuve = lirePreuve()
    const f = primeExceptionnel({ niveau, plafond, plafondBrut: grille.base.plafond_brut, preuve, pression,
      unites, typeDeNuit: typeDeNuit(preuve), delaiBaisse, reglageBaisse })
    base.fourchette_exceptionnel = { bas: niveau.prix, plafond }
    base.prime_exceptionnel = f
    if (f.prix != null) {
      prix = f.prix
      // ⚠ « LE DEPLACEMENT A ETE LIMITE » ET UN PRIX AU-DESSUS DU NIVEAU SUR LA
      // MEME NUIT se contredisent sans une phrase (releve en review).
      const borne = base.couches.find(c => c.nom === 'borne')
      if (borne) borne.resume += ' — la preuve de l’an dernier, elle, place la nuit dans la fourchette'
    }
    base.couches.push({ nom: 'fourchette', agit: true, deplacement: 0,
      prix: f.prix != null ? f.prix : niveau.prix,
      resume: f.resume, detail: f.detail })
  } else if (indice < niveaux.length - 1) {
    // ─── 3 ter. LE PLANCHER N-1 (point B) : AU MOINS LE PRIX DE L'AN DERNIER ─
    // ⚠ LA MEME DECISION QUE LA FOURCHETTE (`primeExceptionnel`) : meme
    // segment, nuit passee, hors reference ecartee, meme type de nuit, deux pas
    // d'ecart au moins, arrondi au pas superieur, retrait par la pression.
    // ⚠ CE PLANCHER EST UN CLIQUET, ET LE PLAFOND EST CE QUI LE BORNE (reserve
    // ecrite par Thierry, 23 septembre 2026) : il ne peut que monter, et une
    // annee exceptionnelle se transmettrait indefiniment sans borne. NE PAS
    // RETIRER CETTE BORNE PAR SIMPLIFICATION. Sans plafond (pas de deuxieme
    // reservation au-dessus du niveau le plus haut), la borne est le prix du
    // niveau le plus haut.
    // ⚠ PAS DE PHRASE QUAND RIEN NE BOUGE : sur une nuit sans preuve, B est
    // muet (la fourchette, elle, dit toujours pourquoi — c'est son niveau).
    const haut = niveaux[niveaux.length - 1]
    const borneN1 = plafond != null ? plafond : haut.prix
    const preuve = lirePreuve()
    if (preuve && preuve.prix != null && borneN1 > niveau.prix) {
      const f = primeExceptionnel({ niveau, plafond: borneN1,
        plafondBrut: plafond != null ? grille.base.plafond_brut : null,
        plafondEstLeNiveau: plafond == null, preuve, pression, unites, typeDeNuit: typeDeNuit(preuve),
        delaiBaisse, reglageBaisse })
      const dateFr = iso => `${iso.slice(8, 10)}/${iso.slice(5, 7)}/${iso.slice(0, 4)}`
      if (f.prix != null && f.prix > niveau.prix) {
        // LE NIVEAU AFFICHE SUIT LE PRIX : « le niveau annonce ne peut plus
        // mentir » — une nuit Haut relevee a 195 € n'est plus « Haut ».
        const iApres = niveaux.reduce((k, n, i) => (n.prix <= f.prix ? i : k), indice)
        niveauAffiche = niveaux[iApres]
        prix = f.prix
        const change = iApres !== indice
        base.releve_n1 = { ...f, niveau_avant: niveau.nom, niveau_apres: niveauAffiche.nom,
          resume: f.plafonne ? f.resume
            : `Relevé à ${f.prix} € — prix obtenu le ${f.date_lisible || dateFr(f.preuve_date)} sur la nuit comparable`
              + (change ? ` (${niveau.nom} → ${niveauAffiche.nom})` : '')
              + (f.reference_amincie && !f.amincie_systematique ? ' (référence amincie : une seule vente)' : '')
              + (f.suffixe_jour || '') }
        // `niveau` reste celui du PIPELINE (l'API l'expose en `niveau_choisi`) ;
        // le niveau effectif et le deplacement effectif suivent le prix
        // (review : « depart Haut, niveau Exceptionnel, deplacement −1 » mentait).
        base.niveau_effectif = niveauAffiche.nom
        base.deplacement_effectif = iApres - indiceDepart
        base.couches.push({ nom: 'releve_n1', agit: true, deplacement: iApres - indice, prix: f.prix,
          resume: base.releve_n1.resume, detail: `${f.detail} · niveau ${niveau.nom} (${niveau.prix} €) → ${niveauAffiche.nom}`
            + (plafond == null ? ` · borne = haut de la grille (${borneN1} €, pas de plafond sur ce logement)` : '') })
      } else if ([MOTIFS_SANS_PRIME.PRIME_RETIREE, MOTIFS_SANS_PRIME.RETIREE_DELAI, MOTIFS_SANS_PRIME.REGLAGE_HOTE]
        .includes(f.motif_sans_prime) && f.prix_sans_pression > niveau.prix) {
        // Le retrait se dit, comme dans la fourchette : jamais un prix qui
        // aurait monte et ne monte pas, sans phrase.
        base.releve_n1 = { ...f, niveau_avant: niveau.nom, niveau_apres: niveau.nom,
          resume: f.resume.replace(/^Prime retirée/, 'Relèvement retiré') }
        base.couches.push({ nom: 'releve_n1', agit: true, deplacement: 0, prix: niveau.prix,
          resume: base.releve_n1.resume, detail: f.detail })
      }
    }
  }
  ...
  base.etiquette = base.affine_par_le_jour
    ? `${niveauAffiche.nom} · ${s.jour_semaine}` : niveauAffiche.nom
  base.etiquette_corrigee = !!base.affine_par_le_jour
  // Le haut de l'etendue est le plafond quand la fourchette existe : un contrat
  // `max: 165` a cote d'une suggestion a 195 € serait faux (releve en review).
  // ⚠ ET JAMAIS SOUS LE PRIX SUGGERE : le plancher N-1 peut monter un niveau
  // inferieur jusqu'au plafond (review du point B, meme defaut qu'au V2.0.7).
  base.fourchette = { min: niveaux[0].prix,
    max: Math.max(prix, base.fourchette_exceptionnel ? base.fourchette_exceptionnel.plafond : niveaux[niveaux.length - 1].prix) }
...
/**
 * LA PRIME DANS LA FOURCHETTE EXCEPTIONNEL — la preuve, ou la raison de son
 * absence. Toujours une phrase : une nuit a 165 € sans prime dit pourquoi.
 * @param preuve { date, prix, ventes, hors_reference, meme_segment } | null
 *   la nuit comparable de l'an dernier et ce qu'elle a rapporte
 */
function primeExceptionnel ({ niveau, plafond, plafondBrut = null, plafondEstLeNiveau = false,
  preuve, pression, unites = 1, typeDeNuit = null, delaiBaisse = false, reglageBaisse = null }) {
  const dateFr = iso => `${iso.slice(8, 10)}/${iso.slice(5, 7)}/${iso.slice(0, 4)}`
  const sans = (motif, resume, detail) => ({ prix: null, motif_sans_prime: motif, resume, detail,
    preuve_date: preuve && preuve.date ? preuve.date : null,
    preuve_prix: preuve && preuve.prix != null ? preuve.prix : null })
  const bas = `Exceptionnel · ${niveau.prix} € sans prime`
  if (!preuve || !preuve.date) {
    return sans(MOTIFS_SANS_PRIME.PAS_DE_COMPARABLE,
      `${bas} — pas de nuit comparable l’an dernier`,
      `aucune nuit comparable N-1 : pas de preuve, pas de prime (plafond ${plafond} €)`)
  }
  if (preuve.pas_encore_passee) {
    return sans(MOTIFS_SANS_PRIME.PAS_ENCORE_PASSEE,
      `${bas} — la nuit comparable (${dateFr(preuve.date)}) n’est pas encore passée : rien n’est encore prouvé`,
      `nuit comparable ${preuve.date} a venir : pas de preuve (une reservation a venir n'est pas un prix obtenu)`)
  }
  if (preuve.prix == null && preuve.prix_inconnu) {
    return sans(MOTIFS_SANS_PRIME.PRIX_INCONNU,
      `${bas} — la nuit comparable (${dateFr(preuve.date)}) a été vendue, mais son prix n’est pas exploitable`,
      `nuit comparable ${preuve.date} vendue sans prix exploitable : pas de preuve`)
  }
  if (preuve.prix == null) {
    return sans(MOTIFS_SANS_PRIME.NON_VENDUE,
      `${bas} — la nuit comparable (${dateFr(preuve.date)}) n’a pas été vendue`,
      `nuit comparable ${preuve.date} non vendue : pas de preuve`)
  }
  if (preuve.meme_segment === false) {
    return sans(MOTIFS_SANS_PRIME.AUTRE_SEGMENT,
      `${bas} — la nuit comparable (${dateFr(preuve.date)}) était d’une autre nature`,
      `nuit comparable ${preuve.date} d'un autre segment : son prix ne prouve rien pour celle-ci`)
  }
  if (preuve.hors_reference) {
    return sans(MOTIFS_SANS_PRIME.HORS_REFERENCE,
      `${bas} — la vente du ${dateFr(preuve.date)} est écartée de votre historique`,
      `nuit comparable ${preuve.date} hors reference (exception, fermeture ou long sejour)`)
  }
  // ⚠ DEUX PHRASES, PAS UNE — vu sur apercu reel (12/02/2027) : « moins de
  // 10 € d'ecart » se lisait sur une nuit vendue 160 €, c'est-a-dire MOINS
  // cher que le niveau. Ce n'est pas un petit ecart, c'est aucune preuve.
  if (typeDeNuit && typeDeNuit.ok === false) {
    return sans(MOTIFS_SANS_PRIME.AUTRE_TYPE_DE_NUIT,
      `${bas} — la nuit comparable (${dateFr(preuve.date)}) était un ${typeDeNuit.jour_preuve}`
        + (typeDeNuit.niveau_preuve && typeDeNuit.niveau_jour
          ? `, qui se vend ${typeDeNuit.niveau_preuve} chez vous, contre ${typeDeNuit.niveau_jour} pour un ${typeDeNuit.jour} : son prix surestimerait cette nuit`
          : `, dont le niveau chez vous est inconnu : son prix ne prouve rien pour un ${typeDeNuit.jour}`),
      `nuit comparable ${preuve.date} (${typeDeNuit.jour_preuve}, structure ${typeDeNuit.niveau_preuve}) d'un autre type que ${typeDeNuit.jour} (${typeDeNuit.niveau_jour})`)
  }
  if (preuve.prix <= niveau.prix) {
    return sans(MOTIFS_SANS_PRIME.PAS_PLUS_CHER,
      `${bas} — l’an dernier, la nuit comparable s’est vendue ${Math.floor(preuve.prix)} € le ${dateFr(preuve.date)} : pas plus cher que ce niveau`,
      `preuve ${preuve.prix} € <= niveau ${niveau.prix} € : rien a reprendre`)
  }
  if (preuve.prix - niveau.prix < SEUIL_PREUVE) {
    return sans(MOTIFS_SANS_PRIME.ECART_FAIBLE,
      `${bas} — l’an dernier, ${Math.floor(preuve.prix)} € le ${dateFr(preuve.date)} : moins de ${SEUIL_PREUVE} € au-dessus, on ne bouge pas`,
      `preuve ${preuve.prix} € a moins de deux pas d'arrondi au-dessus du niveau (${niveau.prix} €)`)
  }
  const cible = Math.min(plafond, Math.ceil(preuve.prix / PAS_ARRONDI) * PAS_ARRONDI)
  if (pression && pression.fiable !== false && pression.ecart != null &&
    Number.isFinite(pression.ecart) && pression.ecart <= -SEUIL_PRESSION) {
    const pct = Math.round(-pression.ecart * 100)
    return { ...sans(MOTIFS_SANS_PRIME.PRIME_RETIREE,
      `Prime retirée — ce mois se vend ${pct} % moins bien que l’an dernier : ${niveau.prix} € au lieu de ${cible} € (prix obtenu le ${dateFr(preuve.date)})`,
      `preuve ${preuve.prix} € le ${preuve.date}, prime ${cible} € retiree : portefeuille ${Math.round(pression.ecart * 100)} % vs N-1 au meme delai (seuil −${SEUIL_PRESSION * 100} %)`),
    prix_sans_pression: cible, ecart_pression: pression.ecart }
  }
  // ⚠ LE DELAI RETIRE LA PRIME (arbitrage de Thierry, 23 septembre 2026,
  // option a). LE PRIX DE L'AN DERNIER A ETE OBTENU A UN DELAI INCONNU : s'il
  // s'est vendu six mois a l'avance, il ne prouve rien sur ce que la nuit vaut
  // a 9 jours. Le delai porte une information que le prix N-1 ne porte pas ;
  // les deux ne parlent pas de la meme situation. La couche delai a −1
  // (14 jours ou moins) retire donc la prime ou le relevement, et le dit.
  // ⚠ RAFFINEMENT DATE (dettes-v1, n° 28) : la regle exacte serait « le
  // plancher s'applique a 14 jours ou moins SEULEMENT si la nuit comparable
  // etait elle aussi encore invendue a ce delai » — la date de vente existe.
  if (delaiBaisse) {
    return { ...sans(MOTIFS_SANS_PRIME.RETIREE_DELAI,
      `Prime retirée — la nuit approche : ${niveau.prix} € au lieu de ${cible} € (prix obtenu le ${dateFr(preuve.date)}, à un délai inconnu)`,
      `preuve ${preuve.prix} € le ${preuve.date}, prime ${cible} € retiree : couche delai a −1 (le prix N-1 ne dit pas a quel delai il a ete obtenu)`),
    prix_sans_pression: cible }
  }
  // ⚠ LA MAIN DE L'HOTE PRIME (arbitrage de Thierry, option a) : un hote qui
  // BAISSE un segment a une raison — travaux, concurrent, strategie — et le
  // moteur n'a pas a la defaire en silence. Le detail montre le prix de l'an
  // dernier ET dit qu'il n'est pas repris : l'hote peut changer d'avis en
  // connaissance de cause. Meme traitement que le ✎.
  if (reglageBaisse) {
    return { ...sans(MOTIFS_SANS_PRIME.REGLAGE_HOTE,
      `Prix de l’an dernier : ${Math.floor(preuve.prix)} € le ${dateFr(preuve.date)} — non repris : votre réglage (${reglageBaisse}) fixe ce niveau`,
      `preuve ${preuve.prix} € le ${preuve.date} non reprise : reglage de l'hote a la baisse (${reglageBaisse})`),
    prix_sans_pression: cible }
  }
  // ⚠ UNE PRIME QUI REPOSE SUR UNE SEULE VENTE LE DIT — « référence amincie »,
  // le vocabulaire du reste du moteur. Ailleurs une mesure devient fiable a
  // 8 nuits et 3 reservations ; ici, le prix d'UNE nuit devient un plancher.
  // ⚠ SUR UN BIEN A UNE SEULE UNITE, C'EST TOUJOURS VRAI — releve en review :
  // un drapeau systematique ne dit rien sur la ligne, il passe en legende
  // (restitution-yield : « un drapeau systematique passe en legende »). La
  // donnee le garde (`reference_amincie`), la phrase ne le repete que la ou il
  // distingue quelque chose : un bien a plusieurs unites.
  const amincie = !(preuve.ventes >= 2)
  const amincieDite = amincie && unites > 1
  // ⚠ UNE PREUVE VENUE D'UN AUTRE JOUR LE DIT (Thierry) : « prouve le
  // 25/12/2025 » seul laisserait croire a une comparaison a l'identique.
  const autreJour = typeDeNuit && typeDeNuit.jour_preuve ? typeDeNuit : null
  const dateLisible = autreJour ? `${autreJour.jour_preuve} ${dateFr(preuve.date)}` : dateFr(preuve.date)
  const suffixeJour = !autreJour ? ''
    : (autreJour.prudent
      ? ` ; un ${autreJour.jour_preuve} se vend au niveau ${autreJour.niveau_preuve}, un ${autreJour.jour} au niveau ${autreJour.niveau_jour} : ce prix est donc un minimum prudent`
      : ` ; ${autreJour.jour_preuve} et ${autreJour.jour} se vendent au même niveau chez vous (${autreJour.niveau_jour})`)
  const jourDeLaPreuve = autreJour ? { jour_preuve: autreJour.jour_preuve, niveau_preuve: autreJour.niveau_preuve,
    jour: autreJour.jour, niveau_jour: autreJour.niveau_jour, prudent: !!autreJour.prudent } : null
  // ⚠ UNE NUIT PLAFONNEE LE DIT, ET DIT LA SORTIE (arbitrage de Thierry) : un
  // plafond muet serait vecu comme une limite arbitraire. Deux cas : la preuve
  // depasse ce que deux reservations ont paye (« une seule fois »), ou elle
  // n'est coupee que par l'arrondi vers le bas du plafond.
  const plafonne = Math.ceil(preuve.prix / PAS_ARRONDI) * PAS_ARRONDI > plafond
  if (plafonne) {
    const seule = !plafondEstLeNiveau && (plafondBrut == null || preuve.prix > plafondBrut)
    return {
      prix: cible, motif_sans_prime: null, preuve_date: preuve.date, preuve_prix: preuve.prix,
      reference_amincie: amincie, amincie_systematique: amincie && !(unites > 1), plafonne: true,
      preuve_autre_jour: jourDeLaPreuve, date_lisible: dateLisible, suffixe_jour: suffixeJour,
      resume: plafondEstLeNiveau
        ? `Plafonné à ${cible} €, le haut de votre grille — le ${dateLisible} s’est vendu ${Math.floor(preuve.prix)} €, mais aucune deuxième réservation n’a payé plus sur ce logement. Vous pouvez poser un prix plus haut à la main (✎).`
        : seule
        ? `Plafonné à ${cible} € — le ${dateLisible} s’est vendu ${Math.floor(preuve.prix)} €, mais une seule réservation a atteint ce prix. Vous pouvez poser un prix plus haut à la main (✎).`
        : `Plafonné à ${cible} € — le prix rond sous les ${Math.floor(plafondBrut)} € atteints par deux réservations (${Math.floor(preuve.prix)} € le ${dateLisible}). Vous pouvez poser un prix plus haut à la main (✎).`,
      detail: `preuve ${preuve.prix} € le ${preuve.date} > plafond ${plafond} € (2e reservation la plus chere : ${plafondBrut} €)`
        + ` · fourchette ${niveau.prix}–${plafond} €`
    }
  }
  return {
    prix: cible,
    motif_sans_prime: null,
    preuve_date: preuve.date,
    preuve_prix: preuve.prix,
    reference_amincie: amincie,
    amincie_systematique: amincie && !(unites > 1),
    plafonne: false,
    preuve_autre_jour: jourDeLaPreuve,
    date_lisible: dateLisible,
    suffixe_jour: suffixeJour,
    resume: `Exceptionnel · ${cible} € — prix obtenu le ${dateLisible} sur la nuit comparable`
      + (amincieDite ? ' (référence amincie : une seule vente)' : '') + suffixeJour,
    detail: `preuve ${preuve.prix} € le ${preuve.date}, arrondie au pas superieur : ${cible} €`
      + ` · fourchette ${niveau.prix}–${plafond} € (plafond = prix atteint par au moins deux reservations)`
      + (amincie ? ' · reference amincie : 1 vente' : ` · ${preuve.ventes} ventes`)
  }
}
