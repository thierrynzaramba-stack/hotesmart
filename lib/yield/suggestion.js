...
function suggerer (options = {}) {
  ...
  if (indice === niveaux.length - 1 && plafond != null && plafond > niveau.prix) {
    const preuve = typeof options.preuveN1 === 'function' ? options.preuveN1() : (options.preuveN1 || null)
    const f = primeExceptionnel({ niveau, plafond, plafondBrut: grille.base.plafond_brut, preuve, pression,
      unites: Math.max(1, Number(bien && bien.inventory_units) || 1) })
 ...
 */
function primeExceptionnel ({ niveau, plafond, plafondBrut = null, preuve, pression, unites = 1 }) {
  ...
  if (plafonne) {
    const seule = plafondBrut == null || preuve.prix > plafondBrut
    return {
      ...
      resume: seule
        ? `Plafonné à ${cible} € — le ${dateFr(preuve.date)} s’est vendu ${Math.floor(preuve.prix)} €, mais une seule réservation a atteint ce prix. Vous pouvez poser un prix plus haut à la main (✎).`
  ...
  return {
    ...
    plafonne: false,
    resume: `Exceptionnel · ${cible} € — prix obtenu le ${dateFr(preuve.date)} sur la nuit comparable`
      + (amincieDite ? ' (référence amincie : une seule vente)' : ''),
