...
// niveau (P92) ; son haut est le prix le plus eleve DEJA OBTENU sur le bien.
...
//
// ⚠ LE PLAFOND EST UNE BORNE, PAS UN OBJECTIF — ET IL NE PROTEGE PAS DE TOUT.
// Il ne se retire pas « par simplification ». MAIS (review V2.0.7) : la preuve
// est tiree du MEME historique que le plafond, donc une preuve passee est
// toujours <= au plafond ; il ne rogne que l'arrondi et ne borne pas un prix
// ABERRANT (une vente a 900 € par erreur deviendrait plafond ET preuve). Le
// cliquet N-1, lui, n'escalade pas : 195 prouves redonnent 195. La definition
// d'un plafond robuste est une decision de Thierry, en attente (KB §3 ter).
//
...
function grilleDeBase (prix, options = {}) {
  ...
  // ⚠ LE PLAFOND DE LA FOURCHETTE EXCEPTIONNEL : le prix le plus eleve deja
  // obtenu, arrondi VERS LE BAS au pas — jamais un euro au-dessus du vendu.
  // Sans place au-dessus du niveau, pas de fourchette (et on le dit : `null`).
  const plafond = Math.floor(max / PAS_ARRONDI) * PAS_ARRONDI
  return {
    fiable: true,
    ...
    max: arrondi(max),
    niveaux,
    plafond: plafond > niveaux[niveaux.length - 1].prix ? plafond : null,
    non_calculable: null
  }
}
...
function suggerer (options = {}) {
  ...
  if (indice === niveaux.length - 1 && plafond != null && plafond > niveau.prix) {
    ...
    const f = primeExceptionnel({ niveau, plafond, preuve, pression,
 ...
 */
function primeExceptionnel ({ niveau, plafond, preuve, pression, unites = 1 }) {
  ...
  return {
    ...
    plafonne: cible === plafond && Math.ceil(preuve.prix / PAS_ARRONDI) * PAS_ARRONDI > plafond,
    ...
    detail: `preuve ${preuve.prix} € le ${preuve.date}, arrondie au pas superieur : ${cible} €`
      + ` · fourchette ${niveau.prix}–${plafond} € (plafond = prix le plus eleve deja obtenu, borne de securite)`
