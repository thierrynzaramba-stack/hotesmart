...
function suggerer (options = {}) {
  ...
  if (s.origine && !fiable(psPropre) && !hotePose) {
    ...
    if (t && t.segment) {
      ...
      reglPos = typeof options.reglageDe === 'function' ? (options.reglageDe(t) || null) : null
  ...
  if (s.origine && sPos === s && source !== 'reglage' && !mesureFiablePropre &&
    ...
    if (t && t.segment) {
      const dessous = suggerer({ date, grille, contexte: contexteSansEvenements(contexte, exclus),
        ...
        reglage: typeof options.reglageDe === 'function' ? (options.reglageDe(t) || null) : null,
  ...
  base.couches.push({
    ...
    // observation.
    resume: sPos !== s && source !== 'reglage'
      ? `${nomLisible(s)} : pas encore d’influence mesurée`
        + ` (${base.historique_propre.echantillon} nuit(s) vendue(s), il en faut ${SEUIL_NUITS})`
        + ` — cette nuit garde le niveau qu’elle aurait sans cette date,`
        + ` ${nomLisible(sPos)} un ${s.jour_semaine} : ${niveaux[indiceDepart].nom}`
      : source === 'mesure'
