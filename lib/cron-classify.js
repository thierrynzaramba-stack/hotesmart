...
async function processChannelPropertyMessages(userId, property, results) {
  ...
  for (const [bookingId, threadMsgs] of Object.entries(byBooking)) {
    try {
      ...
      // basee sur les taches — seulement lastReplyAt (reponses envoyees). Les
      // threads info_unknown/sympathy-test ne creent jamais de reponse, donc
      // ils etaient reclassifies par l'IA a CHAQUE tick (~3500 appels/jour
      ...
      // chemin Beds24 : tache ou conversation creee apres le dernier message
      // guest -> thread deja traite, skip AVANT tout appel IA.
      if (lastGuestTime && await hasNewerTaskOrConv(userId, property.id, bookingId, new Date(lastGuestTime))) continue

      // Skip si le dernier message du thread est du host (rien a traiter
      // jusqu'a la prochaine reponse guest) — meme regle que le chemin Beds24.
...
// ou une conversation a-t-elle ete creee APRES le dernier message guest du thread ?
// Si oui -> le thread est deja traite, on skip AVANT tout appel IA (economie tokens).
