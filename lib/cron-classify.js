...
async function processProperty(userId, beds24Key, property, results) {
  ...
  for (const [bookingId, msgs] of Object.entries(byBooking)) {
    try {
      // Bloque uniquement si une tâche active (pending / pending_validation)
      // existe déjà. Les tâches 'done' ou 'ignored' n'empêchent pas la création
      // d'une nouvelle tâche si un nouveau message arrive.
      ...
      if (existingActive && existingActive.length > 0) continue

      // Si une conversation existe déjà pour ce booking, vérifier que le dernier
      // message guest est postérieur à la dernière réponse envoyée. Sinon skip.
      if (existingConv && existingConv.length > 0) {
        const lastConvTime = new Date(existingConv[0].created_at)
        const lastGuestTime = new Date(
          [...msgs].filter(m => m.source === 'guest')
            .sort((a, b) => new Date(b.time) - new Date(a.time))[0]?.time || 0
        )
        if (lastGuestTime <= lastConvTime) continue
      }
