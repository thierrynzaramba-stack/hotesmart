...
// ⚠ ASYMETRIE ASSUMEE, ET ELLE NE PORTE PAS SUR L'OTA MAIS SUR LE PROVIDER.
//
// Le seuil de note s'applique quand l'echelle est VERIFIEE coherente :
//   - Airbnb via Channex : oui, echelle sur 10 connue ;
//   - Booking via BEDS24 : oui. La sonde du 3 septembre 2026 sur 93 avis reels
//     montre `review_score` et les categories toutes sur 10, et la moyenne des
//     categories qui colle au score global (9 pour 9,2 ; 10 pour 10) ;
//   - Booking via CHANNEX : NON. La sonde du 2 septembre a montre des echelles
//     qui ne coincident pas — un overall_score de 1 avec toutes les categories a
//     2.5, un overall de 10 avec des categories a 7.5.
//
// Autrement dit : l'incoherence vient de CHANNEX, pas de Booking.com. Le meme
// avis Booking.com est coherent quand Beds24 le rend et incoherent quand Channex
// le rend. C'est pourquoi la condition regarde le provider et non la seule OTA —
// « harmoniser » sur l'OTA reintroduirait le pari qu'on refuse.
//
// On stocke brut dans tous les cas (docs/kb/avis-voyageurs.md) : ce seuil est une
// lecture, pas une conversion.
const SEUIL_NOTE = 6

// Vrai quand l'echelle du score de proprete est fiable pour ce couple.
function echelleFiable (avis) {
  if (avis?.provider === 'beds24') return true            // mesure sur 93 avis
  // ⚠ Ligne REDONDANTE aujourd'hui : le repli ci-dessous rend deja false pour
  // toute OTA autre qu'airbnb, et une mutation qui la supprime ne fait echouer
  // aucun test — c'est verifie, ce n'est pas une supposition. On la garde
  // comme garde-fou du jour ou le repli s'elargira (une OTA de plus jugee
  // fiable) : l'exclusion Channex/Booking disparaitrait alors en silence.
  if (avis?.provider === 'channex' && avis?.ota === 'booking') return false
  return avis?.ota === 'airbnb'
}
...
function classerParRegle (avis) {
  ...
  if (echelleFiable(avis) && avis.score_clean != null && Number(avis.score_clean) <= SEUIL_NOTE) {
...
- Le verdict s'applique QUELLE QUE SOIT LA LANGUE de l'avis. L'extrait est cite
  mot pour mot DANS LA LANGUE D'ORIGINE, jamais traduit.
...
function lireReponse (brut, texte) {
  ...
  let extrait = extraitVerifie(texte, json?.extrait)
  ...
  // prestataire. `extraitVerifie` rend la portion du TEXTE D'ORIGINE et ne
  // tolere que les ecarts d'ESPACES — ce qui ferme la dette notee au KB (un
  // avis contenant « tres propre » perdait sa citation pour un ecart de forme).
...
// ─── Classification d'UN avis, ecriture comprise ────────────────────────────
// Partagee par le passage du cron et par la saisie manuelle (api/avis.js), qui
// classe au fil de l'eau : l'hote qui vient de saisir un avis doit voir son
// verdict tout de suite, pas au prochain cycle.
//
// Renvoie le verdict ecrit, ou null si l'avis n'a pas pu etre classe — auquel
// cas ai_analyzed_at N'EST PAS pose et l'avis repassera par la file.
async function classerUnAvis (sb, avis, deps = {}) {
  ...
  if (regle) {
    resultat = { verdict: regle.verdict, extrait: null, voie: 'regle' }
  } else {
    ...
    if (!texte) {
      ...
      resultat = { verdict: 'rien_signale', extrait: null, voie: 'sans_texte' }
    } else {
      const ia = await classerParIA(texte, deps)
      if (ia) resultat = { ...ia, voie: 'ia' }
    }
  }
  ...
  // ⚠ `.neq('verdict_source','humain')` SUR L'ECRITURE, pas seulement sur la
  // lecture de la file. Entre les deux il y a un appel au modele, soit plusieurs
  // secondes : l'hote peut requalifier pendant ce temps. Sans cette condition,
  // le verdict automatique ecrasait la correction humaine — et la ligne restait
  // estampillee `humain`, donc gelee : exclue de la file, protegee par le
  // trigger, irrattrapable sans SQL. Ecriture conditionnelle, pas filtre de
  // lecture.
  const { error } = await sb.from('ota_reviews').update({
    ai_clean_verdict: resultat.verdict,
    ai_clean_excerpt: resultat.extrait,
    ai_analyzed_at:   new Date(maintenant()).toISOString()
  }).eq('id', avis.id).eq('user_id', avis.user_id).neq('verdict_source', 'humain')

  if (error) {
    console.error('[reviews-classify] update echec:', error.message)
    return null
  }
  return resultat
}
...
async function classerAvis (results, deps = {}) {
  ...
  const { data: file, error } = await sb
    .from('ota_reviews')
    // ⚠ `provider` EST indispensable : echelleFiable() en depend. Son absence a
    // fait passer trois avis Beds24 notes 2.5 et 5 en proprete pour
    // « rien_signale » — le seuil ne s'appliquait jamais, faute de savoir d'ou
    // venait la ligne. Defaut invisible en test, revele par le rattrapage reel.
    .select('id, user_id, provider, ota, tags, score_clean, content, content_public, content_private, verdict_source')
    .is('ai_analyzed_at', null)
    // ⚠ Un verdict pose par un HUMAIN n'est jamais repris par le modele. Meme
    // logique que le DO NOTHING des detections : ce que le modele dirait d'une
    // seconde lecture n'a aucune valeur face a une correction deja faite.
    // Le trigger de reanalyse porte la meme garde cote base.
    .neq('verdict_source', 'humain')
  ...
  for (const avis of file) {
    ...
    let resultat = null
    try {
      resultat = await classerUnAvis(sb, avis, deps)
    } catch (e) {
      console.error('[reviews-classify] appel IA echec:', e.message)
    }

    if (!resultat) {
      // Non classe : ai_analyzed_at n'a pas ete pose, l'avis repassera. Le
      // poser sur un echec le sortirait de la file pour toujours.
      bilan.erreurs++
      continue
    }
    if (resultat.voie === 'regle')      bilan.regle++
    else if (resultat.voie === 'ia')    bilan.ia++
    else if (resultat.voie === 'sans_texte') bilan.sans_texte++
    bilan[resultat.verdict]++
  }
  ...
  // Reste-t-il des avis en file ? Ce compte est le seul signal qui rendrait
  // visible une boucle de reanalyse : si le trigger remettait des avis en file
  // aussi vite qu'on les traite, le bilan afficherait un travail normal, mais ce
  // compte ne descendrait jamais. Une boucle se lit alors a l'oeil nu en
  // comparant deux cycles. Aucune alerte automatique : dette assumee, KB §7.
  const { count: enFile } = await sb
    .from('ota_reviews')
    ...
    // Le trigger de reanalyse porte la meme garde cote base.
    .neq('verdict_source', 'humain')
  bilan.reste_en_file = enFile ?? null
