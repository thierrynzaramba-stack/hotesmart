...
// Tags Airbnb releves sur les avis reels. On matche des RACINES, pas des tags
// entiers : Airbnb en ajoute regulierement, et un tag inconnu contenant
// "spotless" reste un tag de proprete.
const TAG_POSITIF = /squeaky_clean|pristine|spotless|free_of_clutter|_clean\b|\bclean_/i
const TAG_NEGATIF = /dirty|unclean|not_clean|stain|dusty|smell|grimy|mold/i
// `cleanliness_other` dit "proprete" sans dire quoi : il ne tranche RIEN, il
// envoie a l'etage 2 pour que le texte decide.
const TAG_AMBIGU  = /cleanliness_other/i
...
function classerParRegle (avis) {
  ...
  if (tags.some(t => TAG_NEGATIF.test(t))) {
    return { verdict: 'remarque', raison: 'tag_negatif' }
  }
  // Un tag ambigu doit passer AVANT le positif : `cleanliness_other` ne dit pas
  // si c'est un compliment ou un reproche.
  if (tags.some(t => TAG_AMBIGU.test(t))) return null

  if (tags.some(t => TAG_POSITIF.test(t))) {
...
function texteVoyageur (avis) {
  return [avis?.content_public, avis?.content_private, avis?.content]
