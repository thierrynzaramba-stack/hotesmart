...
//  2. Statut canonique normalisé à l'écriture. La liste fait foi dans
//     lib/bookings-snapshot-status.js (ALL_STATUSES) — NE PAS la recopier ici :
//     cette ligne a porté `confirmed|cancelled|blocked|request` après l'entrée
//     de `demapped` au canon, et un audit s'y est fié pour déclarer « hors
//     canon » 5 lignes parfaitement valides.
...
const EMPTY_SNAPSHOT = {
  ...
  currency:           undefined,
  // Adresse du voyageur. Elle est le CANAL DE SORTIE des reservations qui n'ont
  // pas de messagerie OTA derriere (Channex `ota_name: "Offline"`, saisie directe
  // Beds24) : spec docs/specs/spec-canal-email-resa-directe.md.
  //
  // ⚠ ELLE VIT DANS LE COEUR, PAS DANS `booking_attempts`. Le payload brut la
  // portait deja (`raw.customer.mail` cote Channex, `raw.email` cote Beds24), mais
  // le lecteur de GuestFlow ne selectionne que `booking_id, snapshot` — rapatrier
  // `raw` pour une adresse couterait 6 Ko par ligne, toutes les 5 minutes.
  // `booking_attempts.guest_email` ne couvre lui que les reservations du moteur,
  // et se cle sur `properties.id` quand le snapshot se cle sur le propId provider :
  // le retenir ouvrirait un second chemin, ce que le coeur de donnees interdit.
  //
  // ⚠ ELLE NE DECIDE PAS DU ROUTAGE. Booking.com sert un alias
  // `@guest.booking.com` dans ce meme champ : router sur « une adresse existe »
  // detournerait vers l'e-mail des reservations qui ont une messagerie OTA. C'est
  // `source` qui tranche, jamais la presence de cette valeur.
  guestEmail:         undefined,
  // Telephone du voyageur, meme regime que l'adresse (jamais `null`, cf.
  // `emailOuRien`). Il n'ouvre AUCUN canal : les SMS au voyageur ne sont pas du
  // perimetre. Il sert a la notification de nouvelle reservation envoyee A
  // L'HOTE, qui doit pouvoir joindre son client sans ouvrir trois ecrans — et il
  // dormait dans `raw.customer.phone` comme l'adresse y dormait.
...
// ⚠ JAMAIS `null`, ET C'EST UNE EXCEPTION RAISONNEE A LA REGLE 3 du merge.
// Ailleurs, un champ fourni a null est une information (« ce provider sait qu'il
// n'y a pas de valeur ») et ecrase l'existant. Ici, non : le feed Channex sert des
// revisions AMPUTEES — l'annulation a `rooms: []` et dates nulles decrite dans
// lib/booking-changes.js en est la preuve deja payee — et une revision qui rendrait
// `customer.mail: null` effacerait l'adresse a laquelle on doit justement ecrire.
//
// Le produit ne perd rien a ne pas distinguer « jamais fourni » de « efface » :
// les deux se traitent pareil (aucun envoi, badge « pas d'e-mail » sur la fiche).
// Un ecrasement, lui, coute un message non delivre.
//
// ⚠ ON NE VALIDE PAS LA FORME ICI. Le coeur enregistre ce que le provider a dit ;
// c'est la couche d'envoi qui refusera une adresse inexploitable, et qui le DIRA.
// Filtrer en silence ici rendrait « adresse absente » et « adresse invalide »
// indistinguables pour l'hote.
//
// ⚠ SERT AUSSI AU TELEPHONE — le nom est reste, la regle est la meme : une chaine
// non vide, trimee, ou rien. La renommer toucherait ses appelants pour un gain
// de lecture ; c'est note, pas fait.
function emailOuRien(valeur) {
...
function fromBeds24(booking) {
  ...
  return {
    ...
    guestEmail:         emailOuRien(b.email),
    // Beds24 sert deux champs : `phone` (fixe) et `mobile`. Le mobile d'abord —
    // c'est celui qu'on compose le jour d'une arrivee.
    guestPhone:         emailOuRien(b.mobile) || emailOuRien(b.phone)
  }
}
...
function fromChannex(booking) {
  ...
  return {
    ...
    // `customer.mail`, pas `customer.email` : Channex n'a jamais servi le second
    // (verifie sur les 8 reservations Offline reelles et sur les temoins OTA).
    guestEmail:         emailOuRien(customer.mail),
    guestPhone:         emailOuRien(customer.phone)
  }
}
...
async function saveBookingSnapshot(supabase, { userId, bookingId, propertyId, provider, booking, snapshot, existing, existingPropertyId, existingRawHash, budgetRaw, initialImport } = {}) {
  try {
    ...
    // ⚠ UN SEJOUR DEJA CONNU N'EST PAS UNE NOUVEAUTE : C'EST UN RE-IMPORT.
    ...
    // Channex a livre trois sejours existant deja sous la cle Beds24. Les trois
    // evenements ont ete ecrits `processed_at = NULL`, donc distribues, et le
    // message de BIENVENUE est reparti aux trois voyageurs — qui l'avaient recu
    // les 5, 7 et 9 septembre. C'est exactement le scenario que le commentaire
    // de `recordChangeEvent` decrit depuis toujours.
    //
    // LA REGLE : le code de reservation de l'OTA est le seul identifiant qui
    // traverse un changement de channel manager. S'il est DEJA connu de ce
    // compte sous un AUTRE `booking_id`, alors ce n'est pas une reservation
    // nouvelle — c'est la meme, qui arrive par un autre chemin. On materialise
    // sans distribuer : ni message, ni code sur la serrure, ni notification
    // menage.
    //
    // ⚠ SEULEMENT SUR `new`, et seulement s'il y a un code OTA. Une reservation
    // directe n'en a pas de l'OTA, et un `modified` / `cancelled` doit etre
    // distribue meme sur un sejour connu — c'est tout l'objet du suivi.
    //
    // ⚠ FAIL-SAFE : une lecture en echec ne bloque RIEN. On distribue comme
    // avant et on hurle. Bloquer ici perdrait des notifications legitimes pour
    // se proteger d'un doublon.
    let reimport = false
    if (change && change.type === 'new' && !initialImport) {
      const codeOta = codeOtaBrut(merged)
      if (codeOta) {
        try {
          const { data: dejaVu, error: eVu } = await supabase
            .from('bookings_snapshot')
            .select('booking_id')
            .eq('user_id', userId)
            .eq('snapshot->>otaReservationCode', codeOta)
            .neq('booking_id', String(bookingId))
            .limit(1)
          if (eVu) {
            console.error('[bookings-snapshot] recherche de re-import impossible, on distribue :', eVu.message)
          } else if (dejaVu && dejaVu.length) {
            reimport = true
            console.log(`[bookings-snapshot] sejour ${codeOta} deja connu sous ${dejaVu[0].booking_id} `
              + `— ${bookingId} materialise SANS distribution (re-import)`)
          }
        } catch (e) {
          console.error('[bookings-snapshot] exception recherche de re-import, on distribue :', e.message)
        }
      }
    }
    ...
    if (change) {
      const journalise = await recordChangeEvent(supabase, {
        ...
        change,
        initialImport: initialImport || reimport
      })
      if (!journalise) {
...
// ⚠ LE BUDGET DOIT ETRE PARTAGE PAR TOUT LE CYCLE, PAS PAR APPEL.
// Constat de review, 8 septembre 2026 : `budgetRaw` n'etait fourni par AUCUN
// appelant, donc chaque appel repartait a 60. Or `bookings_snapshot` a deux
// alimentateurs Beds24 par bien dans le meme cycle */5 (cron-bookings et
// cron-classify), plus Channex : le plafond effectif etait 60 x call sites x
// biens d'UPDATE SEQUENTIELS. Le jour de l'enrichissement du `raw`, ou toutes
// les lignes changent d'empreinte d'un coup, c'est exactement le depassement
// des 60 s que ce budget pretend empecher — et ce qui saute en bout de cycle,
// c'est `dispatch_changements` : menages, codes d'acces, messages.
//
// Le budget vit donc ICI, au niveau du module, et non chez l'appelant :
// `processUser` habite `api/cron.js`, que CLAUDE.md interdit de patcher
// partiellement. Une instance Vercel chaude reutilise le module entre deux
// invocations, d'ou la remise a zero par INACTIVITE : dans un cycle, deux appels
// sont espaces de moins de 60 s (plafond de la fonction) ; entre deux cycles,
// de plus de 240 s (periode de 5 min moins la duree). Le seuil de 120 s separe
// les deux sans ambiguite, et se tromper ne fait que rendre le budget neuf —
// le comportement d'avant ce correctif, jamais pire.
const INACTIVITE_NOUVEAU_CYCLE_MS = 120000
let budgetPartage = null
let dernierAppelBudget = 0

function budgetDuCycle (maintenant = Date.now()) {
  if (!budgetPartage || maintenant - dernierAppelBudget > INACTIVITE_NOUVEAU_CYCLE_MS) {
    budgetPartage = { restant: RAW_PAR_CYCLE }
  }
  dernierAppelBudget = maintenant
  return budgetPartage
}

// Pour les tests : repartir d'un cycle vierge sans attendre 120 s.
function _reinitialiserBudget () { budgetPartage = null; dernierAppelBudget = 0 }
...
async function saveBookingSnapshots(supabase, { userId, propertyId, provider, bookings, initialImport, budgetRaw } = {}) {
  ...
  try {
    ...
    // Budget partage par tout le CYCLE (voir budgetDuCycle ci-dessus).
    // L'appelant peut toujours en imposer un — le backfill s'en sert pour ne pas
    // etre borne par le budget du cron.
    const budget = budgetRaw || budgetDuCycle()
...
// ⚠ LE CODE OTA D'UN BOOKING BRUT — point unique.
// `fromBeds24` le lit dans `apiReference`, `fromChannex` dans
// `ota_reservation_code`. Les consommateurs qui travaillent sur des bookings
// BRUTS (le cron de messages, par exemple) n'ont pas de snapshot sous la main :
// sans ce helper, chacun redecouvrirait la regle, et l'un des deux finirait par
// l'oublier.
//
// C'est le seul identifiant d'un sejour qui traverse un changement de channel
// manager — le plan de bascule s'appuie dessus pour la reconciliation, et les
// avis voyageurs pour leur rattachement.
function codeOtaBrut(booking) {
  const b = booking || {}
  const code = b.apiReference || b.ota_reservation_code || b.otaReservationCode || null
  if (!code) return null
  const propre = String(code).trim()
  return propre ? propre.toUpperCase() : null
}
