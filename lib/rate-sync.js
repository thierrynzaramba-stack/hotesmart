...
// ⚠ CE BIEN EST-IL EN COURS DE DEMENAGEMENT ? POINT UNIQUE.
// « Il a une propriete cible » ne suffit pas : apres le re-keying (phase 2.8),
// la cible A ETE PROMUE dans `provider_property_id` et la colonne n'en garde
// plus que la memoire. Un bien est en migration tant que les deux different.
//
// POURQUOI CE PREDICAT EXISTE (9 septembre 2026, trouve en review) : sans lui,
// l'etape « pousser l'ARI » acceptait n'importe quel bien — y compris un bien
// VIVANT chez Channex, avec ses canaux Booking et Airbnb mappes. Elle aurait
// pousse 500 jours vers les OTA, `stop_sell` sur chaque date sans prix, hors du
// cooldown 24 h et hors de la file serialisee — en affirmant dans sa reponse
// qu'aucun canal n'existait sur la cible.
function estEnMigration(property) {
  if (!property || !property.migration_target_property_id) return false
  return String(property.provider_property_id) !== String(property.migration_target_property_id)
}

// ⚠ QUELLE PROPRIETE, CHEZ LE PROVIDER, EST CELLE DE CE BIEN ? POINT UNIQUE.
// Vaut pour TOUT ce qu'on lui adresse — l'ARI, les canaux, les mappings, les
// activations — et pas seulement pour la poussee de calendrier.
// Tant que la bascule n'a pas eu lieu, c'est la propriete CIBLE : elle seule
// existe chez le nouveau provider, et `provider_property_id` porte encore la cle
// du provider SOURCE (« 209413 » chez Beds24). Apres le re-keying, cette cle a
// ete promue et redevient la seule verite — la cible n'est plus consultee, meme
// si la colonne la garde en memoire.
//
// Un bien qui n'est ni chez le canal ni provisionne rend `null` : il n'y a
// nulle part ou pousser, et l'appelant doit le DIRE plutot que d'inventer une
// destination.
function proprieteChezLeProvider(property) {
  if (!property) return null
  if (estEnMigration(property)) return property.migration_target_property_id
  if (estRelieAuCanal(property)) return property.provider_property_id || null
  return null
}
...
module.exports = { canPushRates, RATE_PUSH_BLOCKED, estRelieAuCanal, CHANNEL_NOT_CONNECTED, proprieteChezLeProvider, estEnMigration }
