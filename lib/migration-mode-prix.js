...
const { peutCouperLaPousseeDesPrix } = require('./pilote-tarifaire')
...
async function changerModeDePrix (supabase, bien, mode, { dryRun = true, appel = null } = {}) {
  ...
  // ⚠ LE SECOND ECRIVAIN DE `rate_sync_mode` — RELEVE EN REVIEW (lot 4.5).
  // La porte inverse de B bis etait branchee dans `api/channel-property.js`
  // seulement. Ici, l'assistant de migration ecrit `keep` en direct : un bien
  // pilote par YieldFlow heurtait alors la contrainte `properties_pilote_keep_ck`
  // et l'hote lisait « new row violates check constraint ... » — du jargon
  // postgres, exactement ce que l'autre chemin prend soin d'eviter.
  if (mode === 'keep') {
    const verrou = peutCouperLaPousseeDesPrix(bien)
    if (!verrou.ok) return { ok: false, raison: verrou.code, message: verrou.error }
  }
