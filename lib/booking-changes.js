...
// ⚠ SANS DATE, LA GARDE TIENT QUAND MEME.
// L'ancienne version renvoyait `false` des que `departure` manquait — « date
// inconnue : on ne bloque pas » — et laissait donc passer un evenement pour une
// reservation qu'aucun prestataire ne peut planifier.
//
// Ce n'est pas theorique : Channex sert exactement ce cas. Sur Colomiers, la
// reservation a3f88358 (annulee, Booking.com, 97,00 EUR) porte
// `arrival_date: null`, `departure_date: null`, `rooms: []`. Le mapper la traduit
// en dates nulles, et l'ancienne garde la laissait produire un `new`.
//
// Regle : on se replie sur l'arrivee quand le depart manque — une arrivee vieille
// de plus de 7 jours est un sejour termine, quelle que soit la duree. Et sans
// AUCUNE date, on bloque : un menage sans date est ininterpretable, on ne
// derange personne pour ca. Un faux negatif ici coute une notification perdue
// sur une donnee deja incomplete ; un faux positif envoie une prestataire sur
// place sans lui dire quel jour.
//
// ⚠ `precedent` N'EST PAS FACULTATIF QUAND ON EN A UN.
// Premiere version de cette garde : elle ne regardait que le snapshot entrant et
// bloquait des qu'il n'avait aucune date. Or c'est exactement la forme que prend
// l'annulation Channex decrite plus haut — dates nulles, `rooms: []` — et
// `mergeSnapshot` ECRASE les dates connues avec ces nulls (un null fourni est une
// information, cf. docs/kb/bookings-snapshot.md §5). L'annulation d'un sejour
// pourtant connu et date ne produisait donc plus AUCUN evenement : ni menage
// annule, ni code revoque, ni message stoppe — la panne meme que ce correctif
// ferme cote Beds24, rouverte cote Channex.
// On se replie donc sur les dates de la ligne existante avant de conclure.
function sejourTermine(snapshot, aujourdhui, precedent) {
  const snap = snapshot || {}
  const prev = precedent || {}
  const reference = snap.departure || snap.arrival || prev.departure || prev.arrival
  if (!reference) return true                  // aucune date exploitable, nulle part
  const limite = new Date(aujourdhui || Date.now())
  limite.setDate(limite.getDate() - JOURS_DE_GRACE)
  return String(reference) < limite.toISOString().split('T')[0]
}
...
function detectChange(previous, incoming, defaultProvider, aujourdhui) {
  ...
  // `previous` sert de repli quand l'entrant n'a plus de dates (annulation
  // Channex a charge utile vide) : le sejour reste celui qu'on connait.
  if (sejourTermine(next, aujourdhui, previous)) return null
