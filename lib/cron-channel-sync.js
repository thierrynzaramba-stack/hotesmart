...
async function processSyncQueue(results) {
  ...
  try {
    ...
    const { data: bienRows, error: bienErr } = await supabase
      .from('properties')
      .select('id, user_id, name, provider, migration_target_property_id, inventory_units, base_price, prix_minimum, capacity, included_guests, extra_guest_fee, provider_property_id, provider_room_type_id, provider_rate_plan_id, rate_sync_mode')
    ...
    // ⚠ RELECTURE DU PROVIDER A L'EXECUTION, comme le mode juste en dessous.
    // Un bien en cours de migration a les trois ids (ceux de sa propriete CIBLE)
    // et garde la cle de son provider SOURCE : le push partirait vers Channex sur
    // « 209413 ». Meme si l'enfilage le refuse deja, le worker ne suppose pas que
    // l'appelant a bien garde — c'est lui qui ecrit chez le provider.
    if (!estRelieAuCanal(bien)) {
      throw new Error('Bien non gere par le canal (provider ' + bien.provider + ') — aucun ARI pousse')
    }
