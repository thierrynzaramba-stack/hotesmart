...
async function processSyncQueue(results) {
  ...
  try {
    ...
    const { data: bienRows, error: bienErr } = await supabase
      .from('properties')
      .select('id, user_id, name, provider, base_price, capacity, included_guests, extra_guest_fee, provider_property_id, provider_room_type_id, provider_rate_plan_id, rate_sync_mode')
