...
async function pollBeds24Reviews (results, deps = {}) {
  ...
  for (const cle of (cles || [])) {
    ...
    for (const bien of (biens || [])) {
      ...
      // ⚠ SEPTIEME PORTE DE LA FAMILLE, RELEVEE EN REVIEW LE 14 SEPTEMBRE 2026.
      // Cette boucle lit `properties` et non la liste live du provider, donc
      // elle paraissait hors de portee : aujourd'hui aucune fiche n'y porte
      // `provider = 'beds24'`. Mais c'est exactement la « protection
      // accidentelle » que `processArrivalCodes` avait deja payee le
      // 10 septembre : le jour ou une fiche migree se recree (le defaut meme
      // que `materializeBeds24Properties` garde), ce poll irait chercher les
      // avis d'une cle abandonnee et les ecrirait dans le coeur.
      const motif = await motifNonSync(sb, bien.user_id, bien.provider_property_id, 'beds24')
      if (motif) {
        console.log(motif === 'migree'
          ? `[beds24-reviews] cle migree, avis ignores : ${bien.provider_property_id}`
          : `[beds24-reviews] garde AVEUGLE, bien ${bien.provider_property_id} NON traite ce cycle`)
        continue
      }
