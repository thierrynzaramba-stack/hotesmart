...
function bornerParDate (q, { debut, fin }) {
  const branche = (op, val) =>
    `and(stay_end.not.is.null,stay_end.${op}.${val}),` +
    `and(stay_end.is.null,received_at.${op}.${val})`
  if (fin) q = q.or(branche('lte', fin))
  if (debut) q = q.or(branche('gte', debut))
...
async function filtresAttribution (sb, { userId, prestataireId } = {}) {
  ...
  const SELECT_MENAGE = 'id, menage_events!inner(token)'
  if (jeton) {
  ...
  for (const [ref, intervalles] of fusionnerIntervalles(periodes)) {
    for (const iv of intervalles) {
      const surLaPeriode = q => bornerParDate(q.eq('property_id_ref', ref), iv)
