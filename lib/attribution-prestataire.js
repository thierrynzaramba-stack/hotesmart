...
async function avisDuPrestataire (sb, { userId, prestataireId, contexte = null } = {}) {
  ...
  const retenus = new Map()   // id -> { voie: 'menage' | 'periode', date }
  let tronqueVoie1 = false
  ...
  if (profil.pwa_token) {
    // ⚠ PLAFONNEE SUR LES AVIS, PLUS SUR LES EVENEMENTS — constat de review.
    // La version precedente lisait d'abord les 150 `menage_events` les plus
    // RECENTS, puis les avis qui s'y rattachent. Deux defauts, et le second
    // annulait le correctif du commit precedent :
    //   - la troncature des evenements ne levait AUCUN `tronque`, alors que la
    //     voie 2 le fait. Le compteur devenu exact annoncait « 577 avis pris en
    //     compte » au-dessus d'une liste amputee presentee comme complete ;
    //   - trier les EVENEMENTS du plus recent choisit ceux dont les avis ne sont
    //     pas encore arrives. La voie privilegiait donc les lignes les MOINS
    //     susceptibles de porter un avis : sur 62 evenements porteurs
    //     aujourd'hui, la liste pouvait rendre zero sous un total non nul.
    // La cle etrangere posee le 14 septembre rend cette double lecture inutile :
    // on interroge `ota_reviews` directement, borne par les AVIS et triee par
    // leur date. `menage_events` n'est plus lue du tout sur ce chemin.
    // ⚠ `menage_events.user_id` : meme defense en profondeur qu'avant — un jeton
    // n'a aucune unicite garantie entre comptes.
    const { data: avis, error: errAvis } = await sb.from('ota_reviews')
      .select('id, stay_end, received_at, menage_events!inner(token, user_id)')
      .eq('user_id', userId).eq('statut', 'confirme')
      .eq('menage_events.token', profil.pwa_token)
    ...
    const liste = avis || []
    if (liste.length > MAX_IDS) tronqueVoie1 = true
    for (const a of liste.slice(0, MAX_IDS)) {
      retenus.set(a.id, { voie: 'menage', recu: a.received_at })
    }
  }
  ...
  for (const p of (periodes || [])) {
    ...
    for (const a of liste.slice(0, MAX_IDS)) {
      ...
      // ⚠ DEUX DATES, DEUX ROLES, ET IL NE FAUT PAS LES CONFONDRE.
      // `dateDeRattachement` (stay_end sinon received_at) dit si l'avis tombe
      // DANS la periode. `received_at` dit quand il est ARRIVE, et c'est lui qui
      // ordonne : c'est la seule colonne sur laquelle les deux voies peuvent
      // trier cote base, et c'est aussi la date que la PWA affiche (`recuLe`).
      if (dansLaPeriode(dateDeRattachement(a), p)) {
        retenus.set(a.id, { voie: 'periode', recu: a.received_at })
      }
    }
  }
  ...
  // ⚠ ET TRIEE PAR DATE, PLUS PAR ORDRE D'INSERTION — constat de review.
  // La `Map` est remplie voie 1 puis voie 2 : `slice(0, MAX_IDS)` sur ses cles
  // prenait donc TOUTE la voie 1 avant de regarder la voie 2. Si la premiere
  // fournissait 150 identifiants, aucun avis attribue par periode n'entrait dans
  // la liste, meme plus recent. Les `order` poses dans chaque voie n'ordonnent
  // qu'A L'INTERIEUR d'une voie, jamais entre elles : la promesse « les plus
  // recents » n'etait pas tenue des qu'il y en avait deux.
  // ⚠ ET LA MEME CLE QUE LES REQUETES, sinon la promesse ne tient toujours pas.
  // Constat de review : le tri global se faisait sur `dateDeRattachement`
  // pendant que les deux voies trient et TRONQUENT sur `received_at`. Au-dela de
  // la borne, les 150 gardes par une voie n'etaient donc pas les 150 premiers du
  // tri final — la meme famille de defaut que ce tri venait corriger. On ordonne
  // sur `received_at` de bout en bout : c'est la seule colonne triable cote base
  // pour les deux voies, et c'est la date que la PWA affiche.
  // ⚠ Une date nulle part en DERNIER, pas en premier : un avis qu'on ne sait pas
  // dater ne doit pas evincer un avis date.
  const toutes = [...retenus.entries()]
    .sort((a, b) => String(b[1].recu || '').localeCompare(String(a[1].recu || '')))
  const ids = toutes.slice(0, MAX_IDS).map(([id]) => id)
  if (toutes.length > MAX_IDS) tronque = true
  let parMenage = 0, parPeriode = 0
  for (const v of retenus.values()) { if (v.voie === 'menage') parMenage++; else parPeriode++ }
