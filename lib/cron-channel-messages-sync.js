...
// pendant 125 cycles. Douze cycles de cron, soit environ une heure — la meme
// maille que l'anti-spam de `reportIncident`, qui bornera de toute facon l'envoi.
const RAPPEL_TOUS_LES = 12
...
async function lireEtat (supabase, cle) {
  ...
  const reprise = brut && brut.fil ? { fil: String(brut.fil), page: Number(brut.page) || 1 } : null
...
async function ecrireEtat (supabase, cle, { depuis, abstentions, reprise = null, motif = null }) {
  const { error } = await supabase.from('cron_logs').upsert({
    ...
    errors: (reprise && reprise.fil) || motif
      ? [{ ...(reprise && reprise.fil ? { fil: String(reprise.fil), page: Number(reprise.page) || 1 } : {}),
...
async function sAbstenir (supabase, { cle, etat, bien, motif, results, reprise = null }) {
  ...
  await ecrireEtat(supabase, cle, { depuis: etat.depuis || null, abstentions,
                                    reprise: reprise || etat.reprise || null, motif })
