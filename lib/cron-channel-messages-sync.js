...
const BUDGET_PARC_DEDIE_MS = 45000
...
async function sAbstenir (supabase, { cle, etat, bien, motif, results, reprise = null }) {
  ...
  console.warn(`[msg-sync] abstention (${motif}) sur ${bien.provider_property_id}, `
    + `${abstentions} d'affilee`
    + (etat.dernierMotif && etat.dernierMotif !== motif ? ` — precedent : ${etat.dernierMotif}` : '')
    + (etat.reprise ? ` — reprise gardee : fil ${etat.reprise.fil} page ${etat.reprise.page}` : ''))
