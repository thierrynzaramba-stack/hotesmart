...
// ⚠ MIROIR DE `rekeying_tables_sans_compte()` (migrations/2026-09-10-rekeying.sql).
// Les deux SEULES tables ou une ligne peut n'avoir aucun compte — mesure du
// 10 septembre 2026 : `access_codes` 96/119, `automation_incidents` 11/28.
// Si la liste SQL change, celle-ci doit suivre : sinon le rapport annonce
// deplacees des lignes qui restent, ou l'inverse.
const TABLES_TOLERANTES = ['access_codes', 'automation_incidents']
...
async function auditRekeying (supabase, bien) {
  ...
  return {
    ...
    // ⚠ BORNE AUX TABLES OU LA TOLERANCE S'APPLIQUE. Compter partout laissait
    // annoncer « dont N lignes deplacees » pour des lignes qui ne bougeraient
    // PAS : sur toute autre table, une ligne sans compte n'est ni retenue par le
    // predicat ni vue par le recomptage d'apres — donc abandonnee sous une cle
    // morte, avec un `conforme: true` par-dessus.
    lignes_sans_compte: tables
      .filter(t => TABLES_TOLERANTES.includes(t.nom_table))
      .reduce((n, t) => n + Number(t.sans_compte || 0), 0),
    // Et celles qui resteront derriere, s'il en apparaissait un jour ailleurs.
    sans_compte_non_deplacees: tables
      .filter(t => !TABLES_TOLERANTES.includes(t.nom_table))
      .reduce((n, t) => n + Number(t.sans_compte || 0), 0),
    ...
    par_table: tables.map(t => ({
      ...
      // Les lignes qui n'ont AUCUN compte : deplacees quand meme sur les deux
      // tables ou c'est mesure (`access_codes`, `automation_incidents`), parce
      // que les writers de serrures ne posent pas `user_id`. Elles se voient,
      // au lieu d'etre fondues dans le total.
      sans_compte: Number(t.sans_compte || 0)
    }))
  }
}
...
async function deplacerLeBien (supabase, bien, { dryRun = true } = {}) {
  ...
  if (dryRun) {
    return {
      ...
      lignes_sans_compte: audit.lignes_sans_compte,
      va_faire: [
        `sauvegarder les ${audit.lignes_a_deplacer} ligne(s) concernee(s) dans rekeying_backup`,
        ...(audit.lignes_sans_compte
          ? [`dont ${audit.lignes_sans_compte} ligne(s) sans compte proprietaire (codes d'acces, `
            + 'incidents) : deplacees car les writers de serrures ne posent pas `user_id`']
          : []),
        ...(audit.sans_compte_non_deplacees
          ? [`⚠ ${audit.sans_compte_non_deplacees} ligne(s) sans compte sur d'autres tables : `
            + 'elles ne seront PAS deplacees et resteront sous l\'ancienne cle. A regarder '
            + 'avant de reprendre l\'automatisation.']
          : []),
...
module.exports = { deplacerLeBien, auditRekeying, etatRekeying, raisonDeNePasDeplacer,
  PROVIDER_CIBLE, TABLES_TOLERANTES }
