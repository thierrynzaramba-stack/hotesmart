...
async function seamFetch(path, { method = 'GET', body, apiKey } = {}) {
  ...
  if (!r.ok) {
    // Un compte Seam suspendu coupe la pose des codes d'acces : le voyageur
    // arrive devant une porte qui ne s'ouvre pas. Ca doit reveiller.
    const { signalerSiPanneFacturation } = require('../incident-facturation')
    await signalerSiPanneFacturation('Seam (serrures)', { status: r.status, message: d.error?.message || '' })
    throw new Error(d.error?.message || `Seam ${r.status} sur ${path}`)
  }
  return d
}
