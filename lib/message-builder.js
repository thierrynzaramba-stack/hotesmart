...
// Sentinel posé par buildMessage quand le code Seam est indisponible (PIN null).
const UNRESOLVED_CODE = '[CODE À INSÉRER]'
...
function buildMessage(template, booking, guestName, seamCode, knowledge = {}) {
  ...
  return text
    ...
    .replace(/{code_acces}/g,     seamCode || UNRESOLVED_CODE)
...
// Un message ne doit JAMAIS partir au voyageur avec un code non résolu. Couvre
// les deux formes : le placeholder brut {code_acces} (template non rendu) et le
// sentinel UNRESOLVED_CODE posé par buildMessage quand le PIN Seam est null.
function hasUnresolvedCode(message) {
  if (!message) return false
  return message.includes(UNRESOLVED_CODE) || /\{code_acces\}/.test(message)
}

module.exports = { buildMessage, formatDate, hasUnresolvedCode, UNRESOLVED_CODE }
