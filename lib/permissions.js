...
// ⚠ CES PRESETS NE REMPLISSENT PLUS AUCUN FORMULAIRE. Les boutons « Employé » et
// « Propriétaire » ont ete retires de /settings a la demande du product owner
// (simplification maximale du panneau de droits), et les prestataires ne s'y
// gerent plus du tout.
//
// Ils restent ici comme REFERENCE DOCUMENTAIRE : ce a quoi ressemble un jeu de
// droits coherent pour chaque usage. C'est ce que lit quelqu'un qui se demande
// « qu'est-ce qu'on donne a un employe ? », et ce que verifient
// tests/permissions.test.js.
//
// Rien dans le code ne doit tester « est-ce un proprietaire » — seuls les
// niveaux comptent. Un preset n'a jamais ete un role, et l'est encore moins
// maintenant.
const PRESETS = {
  employe: {
    property_scope: 'all',
...
// est toujours dans le perimetre. Cette liste, elle, ne parle que des biens — le
// sort des lignes a property_id NULL est une decision de l'appelant, prise dans
// filtrePerimetreSql juste en dessous (exclues par defaut, et pourquoi).
...
// ou ne contient aucune reference exploitable — dans les deux cas l'appelant doit
// echouer FERME (ne rien renvoyer), jamais elargir.
//
// ⚠ `inclureSansBien` est FAUX par defaut, et c'est une DIVERGENCE ASSUMEE
// d'avec dansPerimetre / in_scope, qui considerent qu'une donnee sans bien est
// toujours dans le perimetre. Cette regle vaut pour une donnee de compte, pas
// pour une COLLECTION de donnees voyageur : `messages.property_id` peut etre NULL
// par defaut de rattachement, et inclure ce cas montrerait a un membre limite au
// bien A les conversations des biens B et C. L'alignement propagerait ici une
// permissivite, il ne la justifie pas. Un appelant dont les lignes sans bien sont
// legitimement communes au compte passe `true` explicitement.
//
// ⚠ SECONDE LIMITE : dansPerimetre TRADUIT une reference dans l'autre sens en
// passant par la ligne `properties` (un UUID vaut pour le provider_property_id du
// meme bien). Un filtre SQL ne peut pas : il compare la valeur BRUTE de la colonne
// a la liste. L'equivalence tient donc uniquement parce que `property_refs` est
// maintenu par trigger a partir de `property_ids` — refsDuPerimetre fusionne les
// deux, et la colonne trouve sa forme dans la liste. Si ce trigger disparaissait,
// un perimetre enregistre en UUID seul ne matcherait plus `bookings_snapshot`
// (dont property_id est toujours le provider_property_id TEXT).
//
// Les valeurs viennent de profile_permissions, pas du client, mais elles sont
// interpolees dans l'expression : une virgule ou une parenthese y injecterait des
// filtres.
const REF_SQL_SURE = /^[A-Za-z0-9_-]{1,64}$/
function filtrePerimetreSql(refs, colonne = 'property_id', inclureSansBien = false) {
  if (refs == null) return null
  if (!Array.isArray(refs) || refs.length === 0) return ''
  // ⚠ On ECARTE les references au format refuse, on ne jette pas l'expression
  // entiere : une seule reference mal formee (un point, un espace dans un
  // provider_property_id) videeait sinon TOUT le perimetre du membre, alors que
  // dansPerimetre aurait accepte ses autres biens. Ecarter retrecit — donc
  // echoue ferme — sans punir les biens sains.
  const sures = refs.map(String).filter(r => REF_SQL_SURE.test(r))
  if (sures.length !== refs.length) {
    console.warn(`[permissions] ${refs.length - sures.length} reference(s) de perimetre au format refuse, ecartee(s)`)
  }
  if (!sures.length) return ''
  const liste = `${colonne}.in.(${sures.join(',')})`
  return inclureSansBien ? `${colonne}.is.null,${liste}` : liste
}
