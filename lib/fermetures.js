...
async function scinderAutour (supabase, { userId, propertyId, nuitsRouvertes }) {
  ...
  for (const f of couvrantes) {
    ...
    // ⚠ ON RACCOURCIT L'ANCIENNE (UPDATE), PUIS ON INSERE LE RESTE — trouve par
    // le scenario reel sur staging du 21 septembre 2026. « Inserer les morceaux
    // avant de retirer l'ancienne » (review) heurtait la contrainte d'exclusion
    // (les morceaux chevauchent l'ancienne tant qu'elle existe) : 23P01, la
    // reouverture tombait en 503. Un UPDATE qui retrecit la periode ne se
    // heurte jamais a lui-meme ; le second morceau, insere ensuite, ne
    // chevauche plus rien. Sans transaction, l'echec le moins grave : un
    // insert rate laisse l'ancienne raccourcie et le second morceau sans objet
    // — nuits toujours fermees en memoire, rien ne s'ouvre a tort, et jamais
    // deux objets sur la meme nuit. Zero morceau (fermeture d'un jour rouverte)
    // = un DELETE.
    const apres = []
    if (!morceaux.length) {
      ...
      const { data: maj, error: eMaj } = await supabase.from('fermetures')
        .update({ date_debut: premier.date_debut, date_fin: premier.date_fin, updated_at: new Date().toISOString() })
        .eq('id', f.id).eq('user_id', userId).eq('property_id', propertyId)
        .select('id, date_debut, date_fin, raison').single()
      if (eMaj) return { ok: false, raison: 'ecriture_impossible', message: `Scission impossible : ${eMaj.message}` }
      apres.push(maj || { id: f.id, date_debut: premier.date_debut, date_fin: premier.date_fin, raison: f.raison })
    ...
    touchees.push({ avant: { id: f.id, date_debut: f.date_debut, date_fin: f.date_fin, raison: f.raison }, apres, rouvertes: dedans })
  }
