...
const estJour = v => JOUR_RE.test(String(v || ''))
...
async function fermeturesDuBien (supabase, propertyId, debut, fin) {
  ...
  const { data, error } = await supabase.from('fermetures')
    .select('id, user_id, property_id, date_debut, date_fin, raison, created_at')
...
function nuitsFermees (fermetures, debut, fin) {
  ...
  for (const f of fermetures || []) {
    ...
    for (let n = 0; j <= stop && n < 5000; n++) { out.add(j); j = decaler(j, 1) }
  }
  return out
}
...
async function supprimerFermeture (supabase, { userId, propertyId, id }) {
  ...
  const f = data[0]
  return { ok: true, fermeture: f, segments: [{ date_from: f.date_debut, date_to: f.date_fin, stop_sell: false }] }
}
...
function scinder (fermeture, nuitsRouvertes) {
  ...
  for (let n = 0; j <= fermeture.date_fin && n < 5000; n++) {
    if (rouvertes.has(j)) {
...
async function scinderAutour (supabase, { userId, propertyId, nuitsRouvertes }) {
  ...
  for (const f of couvrantes) {
    ...
    // ⚠ UN SEUL CHEMIN : on retire l'ancienne, on recree les morceaux. Deux
    // requetes de plus qu'un UPDATE astucieux, mais une seule facon d'ecrire.
