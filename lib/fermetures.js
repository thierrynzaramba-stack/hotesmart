...
async function modifierFermeture (supabase, { userId, propertyId, id, debut, fin, raison }) {
  ...
  // Chevauchement avec une AUTRE fermeture : refuse ici pour le message, tenu
  // par la contrainte d'exclusion sous concurrence.
  let voisines
  try { voisines = (await fermeturesDuBien(supabase, propertyId, debut, fin)).filter(f => f.id !== id) }
  ...
  if (retirees.length) {
    try {
      const restantes = (await fermeturesDuBien(supabase, propertyId, avant.date_debut, avant.date_fin)).filter(f => f.id !== id)
      const couvertes = nuitsFermees(restantes, avant.date_debut, avant.date_fin)
      ...
      retirees = []
      out.avertissement = `Dates modifiées, mais les nuits retirées n'ont pas été rouvertes (fermetures illisibles : ${e.message}). Rouvrez-les depuis le calendrier.`
    }
  }
  out.segments = [
