...
async function sendPlatformSms(to, message) {
  ...
  try {
    ...
    if (!r.ok) {
      // Un quota Brevo epuise coupe SMS et e-mails — y compris les alertes.
      // On le signale par un autre canal que celui qui vient d'echouer.
      const { signalerSiPanneFacturation } = require('./incident-facturation')
      await signalerSiPanneFacturation('Brevo (SMS/e-mails)', { status: r.status, message: data.message || '' })
      return { ok: false, error: data.message || `Brevo ${r.status}` }
    }
