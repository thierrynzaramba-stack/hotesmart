...
function calculerPrix ({ aujourdHui, fin, lignes, fermetures, prix }) {
  const comptes = { calculees: 0, changees: 0, inchangees: 0, non_calculables: 0, sous_plancher: 0,
    fermees: 0, fermees_par_l_hote: 0, vendues: 0, sans_ligne: 0 }
  const motifs = {}
  const changements = []
...
async function entretenirLesPrix (supabase, bien, deps = {}) {
  ...
  const decision = calculerPrix({ aujourdHui: auj, fin, lignes, fermetures,
  ...
  if (r.ecrit && r.ecrit.pushFailed) {
    return { ok: false, refus: 'poussee_refusee', fin, comptes: decision.comptes, motifs: decision.motifs, ignorees: r.ignorees, ecrit: r.ecrit,
      message: `Prix mémorisés mais non poussés au canal (${(r.ecrit.warnings || []).join(' · ') || 'poussée refusée'}).` }
  }
