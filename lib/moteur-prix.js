...
const { sautDuMoteur } = require('./nuits-du-moteur')
...
//   `null` = PAS DE FILTRE : toutes les nuits a ligne sont tarifables. Ce
//   n'est PAS « capacite non calculable » : c'est l'APPELANT qui refuse de
//   tarifer quand l'ouverture est inconnue (`entretenirLesPrix` sur le
//   contexte, `piloterLeBien` sur la memoire relue — dette 25). Le docstring
//   disait l'inverse du code.
...
function calculerPrix ({ aujourdHui, fin, lignes, fermetures, prix, ouverts = null, prixHote = null }) {
  ...
  for (let j = aujourdHui, n = 0; j <= fin && n < 5000; j = decaler(j, 1), n++) {
    // ⚠ LA REGLE DU SAUT VIT DANS lib/nuits-du-moteur.js : la confirmation
    // d'activation compte avec elle ce que ce moteur va recalculer.
    const saut = sautDuMoteur(j, { parLHote, prixHote, parDate, ouverts })
    if (saut) { comptes[saut]++; continue }
