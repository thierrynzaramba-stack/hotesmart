...
// firstName, lastName, arrival, departure, source et guestEmail (le routage de
// `lib/canal-voyageur.js`), otaReservationCode (la garde d'empreinte).
function bookingDepuisSnapshot(event, snapshot) {
  ...
  return {
    ...
    // ⚠ ET LE MEME PIEGE, UNE SECONDE FOIS, SUR LE MEME OBJET.
    // Ce booking est reconstruit champ par champ : tout ce qui n'est pas cite ici
    // n'existe pas pour les consommateurs. `canalPour` (lib/canal-voyageur.js) lit
    // `guestEmail` pour decider si une reservation Offline part par e-mail —
    // sans cette ligne, elle rendrait `pas_d_email` pour TOUTES, y compris celles
    // dont le coeur porte l'adresse, et le message de bienvenue ne partirait
    // jamais. La garde d'empreinte a deja ete inerte sur ce chemin pour la meme
    // raison (voir juste au-dessus) : un objet ampute rend muet un mecanisme sain.
...
// ─── Consommateur 3 bis : prevenir l'hote d'une nouvelle reservation ─────────
// Une reservation Airbnb ou Booking, l'hote l'apprend par la plateforme. Une
// reservation DIRECTE, personne ne la lui annoncait : il devait ouvrir HoteSmart
// pour savoir qu'il avait vendu.
//
// ⚠ SUR `new` SEULEMENT. Un `modified` ou un `cancelled` a sa propre histoire —
// et annoncer « nouvelle reservation » sur une modification serait faux.
//
// ⚠ ET LE MODULE FILTRE LUI-MEME SUR LA SOURCE. On pourrait tester `Offline`
// ici ; on ne le fait pas, pour que la regle « OTA non, direct oui » vive a UN
// seul endroit. Deux gardes d'accord aujourd'hui, c'est une garde oubliee demain.
async function consommateurNotifHote (event, ctx, snapshot) {
  if (event.type !== 'new') return

  // Meme re-verification que pour les templates : entre la detection et l'effet,
  // la reservation a pu etre annulee. Annoncer une vente defaite serait pire que
  // ne rien annoncer.
  if (snapshot && !isActiveStatus(snapshot, event.provider)) return

  const k = cle(event.user_id, event.property_id)
  const p = ctx.propsByKey[k] || {}

  // ⚠ CHARGEMENT PARESSEUX, et la cause n'est pas celle qu'on croit.
  // Ce module-ci utilise le client partage de `cron-shared`, que les tests
  // stubent deja. C'est sa dependance `lib/email-guestflow.js` qui cree SON
  // PROPRE client a l'import : le requerir en tete faisait echouer le simple
  // CHARGEMENT du dispatcher des que l'environnement n'etait pas complet — 20
  // tests tombes d'un coup sur « supabaseUrl is required ».
  // Un consommateur ne doit pas imposer ses dependances au moment ou on lit le
  // module qui l'appelle. (Faire passer `email-guestflow` au client partage
  // rendrait ce paresseux inutile : note, pas fait — il a ses propres tests.)
  const { notifierNouvelleResa } = require('./notif-hote-resa')

  const r = await notifierNouvelleResa({
    userId: event.user_id,
    bookingId: event.booking_id,
    propertyId: event.property_id,
    bien: p.name || null,
    snapshot: snapshot || {},
    politique: p.cancellation_policy || null
  })

  if (r.ok) {
    if (!r.ignore && !r.deja) {
      console.log(`[dispatch] hote prevenu de la reservation ${event.booking_id}`)
    }
    return
  }

  // ⚠ JAMAIS AVALE. L'hote croit etre prevenu de ses ventes : s'il ne l'est pas,
  // il doit l'apprendre autrement que par un client qui se presente a sa porte.
  console.error(`[dispatch] notification hote NON envoyee ${event.booking_id} : ${r.raison}`)
  // ⚠ PROTEGE, comme les quatre autres appels de ce fichier. Si l'alerting lui-meme
  // leve, le `throw` ci-dessous n'est jamais atteint et `processing_errors`
  // enregistre l'erreur d'ALERTING a la place de la vraie raison — on perdrait le
  // diagnostic au moment precis ou il sert. Constat de review.
  try {
    await reportIncident('notif_hote_non_envoyee', {
      userId: event.user_id, propertyId: String(event.property_id),
      propertyName: p.name || null, threshold: 1,
      detail: `Nouvelle reservation ${event.booking_id} non annoncee : ${r.raison}. `
        + 'Elle est bien enregistree — c\'est l\'e-mail qui n\'est pas parti.'
    })
  } catch (e) { console.error('[dispatch] incident notif hote non enregistre', e.message) }
  throw new Error(`notif hote : ${r.raison}`)
}
...
async function dispatchBookingChanges(results) {
  ...
  for (const event of events) {
    ...
    // 3 bis. Prevenir l'HOTE d'une nouvelle reservation directe.
    // ⚠ ICI, ET PAS DANS LE MOTEUR NI DANS LA SAISIE MANUELLE. Les deux creent
    // des reservations `Offline` et passent toutes deux par le feed, donc par ce
    // dispatcher : le brancher une fois ici couvre les deux sources, et la
    // prochaine. Le brancher deux fois en amont aurait donne deux
    // implementations a maintenir — et un jour, une seule des deux corrigee.
    //
    // Son echec ne doit emporter personne : il est trace comme les autres et
    // n'empeche ni le menage, ni le code d'acces, ni le message de bienvenue.
    try { await consommateurNotifHote(event, ctx, snapshot) }
    catch (e) { erreurs.push({ consommateur: 'notif_hote', erreur: e.message }) }
