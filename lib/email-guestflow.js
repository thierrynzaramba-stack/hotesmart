...
//
// Tant que l'ecran de l'etape 4 n'existe pas, on prend le PREMIER expediteur
// ACTIF du compte Brevo de l'hote — c'est-a-dire une adresse dont Brevo garantit
// deja qu'elle peut envoyer. Ce repli n'est pas provisoire : il reste le defaut
// propre que la spec demande quand l'hote n'a rien choisi.
//
// Le cache evite un aller-retour Brevo par message. Il vit dans le process (une
// instance Vercel chaude), pas au-dela : un expediteur ajoute chez Brevo est vu
// au prochain demarrage, et une invalidation fine ne vaudrait pas sa complexite.
...
async function configDuCompte (userId) {
  ...
  if (enCache && Date.now() - enCache.pose < CACHE_MS) return enCache.valeur
  ...
  const expediteur = await premierExpediteurActif(data.brevo_api_key)
  if (!expediteur.ok) return expediteur

  const valeur = { ok: true, cle: data.brevo_api_key, email: expediteur.email, nom: expediteur.nom }
  cacheExpediteur.set(String(userId), { pose: Date.now(), valeur })
  return valeur
}
  ...
  try {
    r = await fetch(BREVO_EMAIL, {
      ...
      body: JSON.stringify({
        ...
        subject: sujet,
        htmlContent: enveloppe(texteVersHtml(texte))
      })
    })
  } catch (e) {
    return { ok: false, raison: `brevo_injoignable: ${e.message}`, permanent: false }
  }
