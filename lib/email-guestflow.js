...
const { adresseDeReponse } = require('./jeton-reponse')
...
// ⚠ `bookingId` TRAVERSE, ET IL A FAILLI NE PAS LE FAIRE.
// Cette fonction ne le declarait pas : les deux appelants qui ecrivent au
// voyageur (les templates GuestFlow et l'envoi manuel de l'hote) le passaient
// consciencieusement, et il tombait ICI. `adresseDeReponse(undefined)` rend
// `null`, donc le reply-to retombait sur l'hote — pour DEUX des trois chemins
// d'envoi. On livrait le reply-to jeton, l'encart client qui l'annonce, et pas
// le comportement. Constat de review.
async function envoyerEmailVoyageur ({ userId, destinataire, sujet, texte, propertyId, propertyName, bookingId }) {
  if (!texte) return { ok: false, raison: 'texte_vide', permanent: true }
  return await envoyerHtml({
    userId, destinataire, sujet, propertyId, propertyName, bookingId,
...
async function envoyerHtml ({ userId, destinataire, sujet, html, propertyId, propertyName, bookingId }) {
  ...
  // Calcule une fois, lisible, et trace : on doit pouvoir dire ou une reponse
  // est censee atterrir sans relire trois expressions imbriquees.
  const repondreA = adresseDeReponse(bookingId) || config.email
  ...
  try {
    r = await fetch(BREVO_EMAIL, {
      ...
      body: JSON.stringify({
        ...
        // ⚠ REPLY-TO : L'ADRESSE DE REPONSE HOTESMART, QUAND ON EN A UNE.
        // Jusqu'au 18 septembre 2026, c'etait l'adresse d'expedition de l'hote —
        // faute d'ingestion, un reply-to pointant ailleurs aurait fait disparaitre
        // les reponses. Cette ingestion existe (`api/inbound-email.js`) : les
        // reponses entrent desormais dans le fil HoteSmart.
        //
        // ⚠ ET ON RETOMBE SUR L'HOTE AU MOINDRE DOUTE. Sans `bookingId`, sans
        // secret de jeton, la reponse du voyageur doit arriver QUELQUE PART :
        // mieux vaut la boite de l'hote que le vide. Un reply-to casse ne se
        // remarque que le jour ou un voyageur attend une reponse a une question
        // qu'on n'a jamais lue.
        replyTo: { email: repondreA, name: config.nom },
  ...
  return { ok: true, id: data.messageId || null, expediteur: config.email, repondreA }
}
