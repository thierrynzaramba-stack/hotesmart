...
const { hasUnresolvedCode } = require('./message-builder')
...
async function checkPendingMessages(results) {
  ...
  // Récupère les clés Beds24 par user pour envoi (hôtes ayant un vrai token)
  ...
  for (const log of pending) {
    try {
      ...
      // GARDE : ne pas envoyer un message avec un code non résolu. Marquer en
      // erreur (stoppe le retry de cette file) et alerter l'hôte.
      if (hasUnresolvedCode(pl.message)) {
        await alertMissingAccessCode({
          userId: log.user_id,
          propertyId: pl.property_id,
          booking: { id: log.booking_id, firstName: pl.guest_name }
        })
        await supabase.from('message_sent_log').update({ status: 'error' }).eq('id', log.id)
        console.warn(`[Access] Pending booking ${log.booking_id} : code non résolu, message bloqué, hôte alerté`)
        continue
      }
      ...
      // DOUBLE ECRITURE (etape 2) : message auto sortant (menage_done) dans
      // `messages`, sans toucher a l'INSERT conversations ci-dessus.
      // Chemin Beds24-only (envoi sendViaBeds24) -> provider 'beds24' en dur.
      // body = texte REEL (pl.message), pas la sentinelle [AUTO:].
      // ota null -> recordMessage resout via bookings_snapshot. providerMsgId
      // null -> dedup logique. Inconditionnel, comme l'INSERT conversations.
      await recordMessage({
        userId:        log.user_id,
        provider:      'beds24',
        propertyId:    pl.property_id,
