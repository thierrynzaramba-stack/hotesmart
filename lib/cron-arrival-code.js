...
async function processArrivalCodes(userId, beds24Key, property, bookings, results) {
  // 1. Chercher un booking avec arrival = today sur cette propriete
  //    (on le fait en premier pour pouvoir ensuite verifier le menage)
  ...
  // 2. Verifier que le menage du booking precedent a bien ete valide.
  //    On ne se base PAS sur property_status.status ('ready') car celui-ci
  //    est transitoire : des qu'un nouveau voyageur arrive, il bascule en
  //    'occupied', meme si le cron n'a pas encore eu le temps d'envoyer
  //    le code. On se base donc sur last_menage_at (timestamp persistant).
  //
  //    Regles :
  //    - Pas de booking anterieur : OK (premier voyageur, pas de menage a valider)
  //    - Booking anterieur existant : last_menage_at doit etre > departure
  //      du dernier booking anterieur
  ...
  const previousDeparture = (pastSnapshots || [])
    ...
    .filter(s => s.departure <= today) // depart deja passe ou du jour
  ...
  if (previousDeparture) {
    ...
    if (!propStatus?.last_menage_at) {
      console.log(`[ArrivalCode] ${property.id} : menage jamais valide, code non envoye pour booking ${todayArrival.id}`)
      return
    }
    ...
    if (menageMoment <= prevDepartureDate) {
      console.log(`[ArrivalCode] ${property.id} : last_menage_at (${propStatus.last_menage_at}) <= departure precedent (${previousDeparture.departure}), code non envoye`)
      return
    }
  }
  ...
  for (const template of templates) {
    ...
    try {
      ...
      // NB : la table n'a que 5 colonnes (id, user_id, booking_id,
      // template_id, sent_at). Les champs status/payload n'existent pas.
