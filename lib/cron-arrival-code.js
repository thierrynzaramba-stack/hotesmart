...
async function isMenageValidated(userId, propertyId, todayArrival) {
  ...
  const previousDeparture = (pastSnapshots || [])
    .map(r => r.snapshot)
    .filter(s => s && s.departure && isActiveStatus(s))
...
async function processArrivalCodes(userId, beds24Key, property, bookings, results) {
  ...
  const todayArrival = (bookings || []).find(b => {
    ...
    return belongs &&
      ...
      // Source mixte : bookings Beds24 bruts (sans champ provider -> defaut
      // 'beds24') et snapshots channel (qui portent leur provider, prioritaire).
      isActiveStatus(b, 'beds24')
  })
