...
async function importMessages(ctx) {
  ...
  // ⚠ REPRISE A L'INTERIEUR D'UN FIL — LE DEFAUT QUI BLOQUAIT LA PRODUCTION.
  // Le marqueur ne bougeait QUE si la passe allait au bout, et le budget est de
  // 2,5 s. Un bien a UN SEUL fil de 56 messages (La bulle, mesure le
  // 15 septembre 2026) : ~3 aller-retours Supabase par message, donc plus de
  // 2,5 s, donc passe tronquee, donc marqueur immobile, donc RE-IMPORT COMPLET
  // au cycle suivant. 125 abstentions d'affilee sur quatre biens, et l'annonce
  // d'ecriture de masse repartait a chaque cycle avec le meme compte. Le fil
  // n'entrait jamais en base, et le seul signal qui le disait — l'incident
  // « import suspendu » — ne partait qu'UNE fois, 125 cycles plus tot.
  //
  // ⚠ REPRENDRE EST SUR ICI, ET LA RAISON COMPTE : on reprend le MEME fil a la
  // MEME page. Les pages d'un fil sont ordonnees et stables ; ce qui arrive
  // apres coup fait bouger `updated_at` du fil, qui repassera par le marqueur
  // d'anteriorite. On ne saute donc rien — on ne recommence simplement pas.
  // ⚠ L'ORDRE DES PAGES EST UNE HYPOTHESE, PAS UNE MESURE — ET CE FICHIER
  // MESURE L'INVERSE A COTE. `getReservations` note que Channex « plafonne ou
  // ignore `pagination[limit]` ». Si les messages sortent du plus RECENT au plus
  // ancien, les messages arrives entre deux cycles se placent en TETE : ils
  // passent sous l'offset de reprise, ne sont jamais lus, et le fil est declare
  // complet — la perte definitive que le marqueur existe pour empecher.
  //
  // ⚠ ON NE POSTULE DONC PAS, ON VERIFIE (REVIEW.md regle 13). Le point de
  // reprise retient le `message_count` du fil au moment de la coupure — une
  // valeur que le listing porte deja, sans aucun appel supplementaire. Si le
  // compte a bouge, le fil a change : on repart de sa page 1. Le pire cas
  // redevient « on recommence », jamais « on perd ».
  const reprise = ctx.reprise && ctx.reprise.fil ? ctx.reprise : null
  let repriseUtilisee = false
  let repriseSuivante = null
  ...
  while (true) {
    ...
    for (const th of threads) {
      ...
      // ⚠ UNE REPRISE NE SE RE-ANNONCE PAS. Le preavis d'ecriture de masse
      // prepare l'alerte de croissance ; le lot est le MEME, et le re-annoncer a
      // chaque cycle est precisement ce qui a noye le signal pendant 125 cycles.
      // On annonce quand on COMMENCE, pas quand on continue.
      if (!annonceFaite && !reprise && typeof ctx.avantEcriture === 'function') {
        annonceFaite = true
      ...
      // prime.
      let filComplet = true
      // ⚠ ON NE REPREND QUE LE FIL QU'ON AVAIT QUITTE, et une seule fois.
      // Appliquer l'offset a un autre fil sauterait ses premieres pages — la
      // perte definitive que le marqueur existe pour empecher.
      let msgPage = 1
      const compteFil = Number(attr.message_count)
      if (reprise && !repriseUtilisee && String(th.id) === String(reprise.fil)) {
        repriseUtilisee = true
        const compteConnu = Number(reprise.count)
        const filInchange = Number.isFinite(compteConnu) && Number.isFinite(compteFil)
          ? compteConnu === compteFil
          : false                      // compte inconnu : on ne prend aucun risque
        if (filInchange) msgPage = Math.max(1, Number(reprise.page) || 1)
        else console.warn('[channel] reprise ecartee, le fil a bouge :', th.id,
                          compteConnu, '->', compteFil)
      }
      const premierePage = msgPage
      while (true) {
        if (depasse()) {
          interrompu = 'budget'; filComplet = false
          // ⚠ ON NOTE OU ON S'ARRETE. Sans ca, le cycle suivant recommence ce
          // fil depuis sa page 1 : sur un fil plus long que le budget, on ne
          // depasse JAMAIS la premiere page, et l'import ne finit jamais.
          repriseSuivante = { fil: String(th.id), page: msgPage,
                              count: Number.isFinite(compteFil) ? compteFil : null }
          break
        }
        ...
        if (!mr.ok) {
          ...
          filComplet = false
          // ⚠ MEME FAMILLE QUE LE BUDGET : sans point de reprise, ce fil sera
          // relu depuis sa page 1 au cycle suivant, et un 500 recurrent sur la
          // page 2 l'y bloquerait indefiniment.
          repriseSuivante = { fil: String(th.id), page: msgPage,
                              count: Number.isFinite(compteFil) ? compteFil : null }
          break
        }
        ...
        if (msgs.length === 0) {
          // ⚠ UNE PAGE VIDE A L'ENTREE D'UNE REPRISE N'EST PAS UNE PREUVE DE
          // COMPLETUDE. Elle veut dire que l'offset designe au-dela de la fin —
          // parce que le fil a change de taille, ou parce que la limite de
          // pagination n'est plus la meme. Sortir avec `filComplet` ferait
          // entrer l'instant de ce fil dans le marqueur alors que RIEN n'a ete
          // lu : le fil serait saute pour toujours.
          if (msgPage > premierePage) break
          filComplet = false
          repriseSuivante = null       // on repartira de la page 1
          break
        }

        for (const m of msgs) {
          // ⚠ LE BUDGET SE CONSULTE ICI AUSSI — MESURE EN REEL LE 16 SEPTEMBRE
          // 2026. Il n'etait teste qu'entre deux PAGES : une page de 100
          // messages, c'est ~300 aller-retours Supabase sans un seul controle.
          // Le cron dedie est mort en `FUNCTION_INVOCATION_TIMEOUT` a 60 s, donc
          // sans rendre son bilan ET sans ecrire l'etat du bien en cours : la
          // passe ne laissait aucune trace, et le cycle suivant recommencait.
          // Une garde entre les pages ne borne rien quand le travail est DANS la
          // page.
          // ⚠ ON REPREND A LA MEME PAGE, pas a la suivante : les messages deja
          // ecrits sont absorbes par la deduplication de `recordMessage`, et le
          // cout du rejeu est borne a UNE page. Une reprise au message pres
          // demanderait un curseur que l'API ne donne pas.
          if (depasse()) {
            interrompu = 'budget'; filComplet = false
            repriseSuivante = { fil: String(th.id), page: msgPage,
                                count: Number.isFinite(compteFil) ? compteFil : null }
        ...
        // ⚠ Sorti du lot par le budget : on ne passe pas a la page suivante.
        if (interrompu) break
  ...
  // ⚠ UNE REPRISE NE REGRESSE PAS. Sur un bien a plusieurs fils, le cycle
  // suivant relit les fils deja finis — le marqueur d'anteriorite n'a pas
  // bouge — et peut mourir DEDANS : le nouveau point de reprise designerait
  // alors un fil ANTERIEUR a celui qu'on avait atteint, et on n'arriverait
  // jamais au bout. On garde le point le plus AVANCE des deux.
  if (repriseSuivante && reprise && String(repriseSuivante.fil) !== String(reprise.fil) &&
      !repriseUtilisee) {
    repriseSuivante = reprise
  }
  ...
  return {
    ...
    // ⚠ OU REPRENDRE AU PROCHAIN CYCLE. `null` quand la passe est allee au bout :
    // le point de reprise doit alors etre EFFACE, sinon on rejouerait a jamais
    // un fil deja importe.
    reprise: repriseSuivante,
    _debug: debug
  }
}
