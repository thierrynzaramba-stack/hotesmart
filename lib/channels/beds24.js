...
async function syncMessages({ userId, propertyId, messages, bookingsMap }) {
  try {
    // LOG TEMPORAIRE (diagnostic etape 3) : a retirer une fois la cause identifiee.
    console.log('[syncMessages] beds24 prop', propertyId, '| recus:', messages?.length, '| guest:', (messages || []).filter(m => m.source === 'guest').length)
...
module.exports = { getReservations, getPropertyMessages, sendMessage, updateAvailability, refreshToken, syncMessages }
