...
const { saveBookingSnapshot } = require('../bookings-snapshot')
...
async function syncBookings({ userId, propertyId, bookings }) {
  try {
    ...
    // Pre-chargement des snapshots existants en UN seul select : sans lui,
    // saveBookingSnapshot relit la ligne booking par booking (2N requetes
    // Supabase par cycle cron). Le merge non destructif est conserve.
    const existingByBooking = {}
    const { data: rows } = await supabase
      .from('bookings_snapshot')
      .select('booking_id, snapshot')
      .eq('user_id', userId)
      .in('booking_id', list.map(b => String(b.id)))
    ;(rows || []).forEach(r => { existingByBooking[String(r.booking_id)] = r.snapshot || null })
