...
// `ttlMs` : duree de tenue. Le defaut reste INTENTION_TTL_MS — le comportement
// de la saisie manuelle (phase 2) est INCHANGE.
// Le moteur de reservation direct, lui, tient plus longtemps : une Checkout
// Session Stripe ne peut pas expirer avant 30 minutes, et une tenue plus courte
// que la session laisserait un voyageur payer des nuits qu'on a deja reliberees.
// `token` : a QUI appartient la tenue. Sans lui, la liberation d'une tentative
// A efface les nuits reposees entre-temps par une tentative B en cours de
// paiement — et les nuits de B redeviennent vendables pendant que B est sur la
// page Stripe. Constat de review du 7 septembre : c'est exactement la
// surreservation que la tenue existe pour empecher.
async function poserIntentions (supabase, { userId, propertyId, nuits: liste, ttlMs, token }) {
  const maintenant = Date.now()
  const expire = new Date(maintenant + (Number(ttlMs) > 0 ? Number(ttlMs) : INTENTION_TTL_MS)).toISOString()
  ...
  const lignes = liste.map(n => ({
    key: cleIntention(userId, propertyId, n), expire_at: expire,
    // `token` est deja la colonne de propriete du verrou (voir `poserVerrou`) :
    // on la reutilise pour la tenue, meme role, meme table.
    token: token || null
  }))
  ...
  // ⚠ L'ECHEC EST RENDU, pas seulement journalise. L'appelant decide : la saisie
  // manuelle (phase 2) continuait sans, le moteur de reservation refuse — chez
  // lui, une tenue non posee veut dire un voyageur qui part payer des nuits
  // restees vendables a tout le monde.
  if (error) {
    console.error('[resa-directe] intentions non posees', error.message)
    return { ok: false, raison: error.message }
  }
  return { ok: true }
}

// Rend les nuits AVANT l'expiration. Sans cela, un voyageur qui abandonne son
// paiement bloque ses dates jusqu'au bout du TTL — une demi-heure pendant
// laquelle personne d'autre ne peut reserver, alors que plus rien n'est en cours.
// Appelee sur `checkout.session.expired` et sur un paiement refuse.
//
// ⚠ Ne supprime QUE les nuits demandees, et seulement pour ce bien et cet hote.
// Le prefixe de cle porte les deux : une suppression plus large libererait les
// tenues d'autres ventes en cours.
async function libererIntentions (supabase, { userId, propertyId, nuits: liste, token }) {
  const cles = (liste || []).map(n => cleIntention(userId, propertyId, n))
  if (!cles.length) return
  let q = supabase.from('write_locks').delete().in('key', cles)
  // ⚠ ON NE LIBERE QUE LES SIENNES. Une tenue reposee entre-temps par une AUTRE
  // tentative porte un autre `token` : l'upsert de `poserIntentions` l'a
  // remplacee, et ce filtre l'epargne. Sans lui, liberer A rendait vendables des
  // nuits que B etait en train de payer.
  if (token) q = q.eq('token', token)
  const { error } = await q
  if (error) console.error('[resa-directe] intentions non liberees', error.message)
}

// Nuits deja vendues par nous et pas encore visibles dans le cœur.
//
// `ignorerToken` : ne pas compter les intentions posees par CETTE reservation-la.
// Sans ce filtre, MODIFIER une reservation qu'on vient de creer est refuse par
// les marqueurs de sa propre creation : l'hote saisit un sejour 12->15 (nuits
// 12, 13 et 14 tenues 20 minutes), corrige aussitot en 12->16, et la
// verification lit ses propres nuits comme occupees. C'est le pendant, cote
// intentions, de ce que `exclure` fait deja cote cœur — le marqueur porte donc
// l'identifiant de la reservation en `token` depuis sa creation.
async function intentionsEnCours (supabase, { userId, propertyId, nuits: liste, ignorerToken }) {
  const cles = liste.map(n => cleIntention(userId, propertyId, n))
  const { data, error } = await supabase
    .from('write_locks').select('key, token')
    .in('key', cles)
  ...
  const par = {}
  for (const l of data || []) {
    // ⚠ `ignorerToken` doit etre NON VIDE pour filtrer quoi que ce soit. Un
    // `undefined === undefined` ecarterait TOUTES les intentions — c'est-a-dire
    // le verrou desarme sur le chemin ordinaire, qui n'en passe aucun.
    if (ignorerToken && String(l.token || '') === String(ignorerToken)) continue
...
async function verifierDisponibilite (supabase, { userId, propertyId, arrival, departure, exclure, ignorerToken }) {
  ...
  const attente = await intentionsEnCours(supabase, { userId, propertyId, nuits: voulues, ignorerToken })
...
async function creerReservationDirecte (supabase, { userId, propertyId, resa }) {
  ...
  try {
    ...
    //
    // `token` = l'identifiant de la reservation. Il ne sert PAS a la tenue
    // elle-meme (n'importe quelle autre saisie doit voir ces nuits occupees),
    // mais a laisser la reservation se MODIFIER sans se heurter a ses propres
    // marqueurs — cf. `ignorerToken` dans `intentionsEnCours`.
    await poserIntentions(supabase, { userId, propertyId, nuits: dispo.nuits, token: String(envoi.id) })
...
// ─── Modification d'une reservation directe ──────────────────────────────────
// Meme sequence que la creation : verrou -> verification -> ecriture CRS ->
// liberation. Elle en partage les regles, et ce n'est pas un hasard : deplacer
// un sejour sur d'autres nuits, c'est vendre ces nuits-la. Un chemin de
// modification qui ne verifierait pas la disponibilite rouvrirait exactement la
// surreservation que la creation ferme — il suffirait de creer sur des dates
// libres, puis de deplacer sur des dates prises.
//
// DEUX DIFFERENCES, toutes deux necessaires :
//
// 1. `exclure: bookingId` — la reservation ne doit pas se compter elle-meme.
//    Sans lui, prolonger un sejour 12->15 en 12->16 serait refuse par ses
//    propres nuits 12, 13 et 14, deja dans le cœur.
// 2. `ignorerToken: bookingId` — meme raison, cote intentions : les marqueurs
//    poses par sa propre creation ne doivent pas bloquer sa correction.
//
// ⚠ LE CŒUR N'EST PAS ECRIT ICI NON PLUS. La modification revient par le feed,
// comme la creation et comme toute reservation OTA.
//
// ⚠ CHANNEX REVALIDE TOUT LE PAYLOAD A CHAQUE ECRITURE : `payloadCRS` exige un
// payload COMPLET (dates, `days` nuit par nuit, montant, client, occupation).
// Une modification partielle est rejetee en 422 — c'est pourquoi l'appelant
// reconstruit le payload entier depuis le cœur, et n'y remplace que ce que
// l'hote a change.
async function modifierReservationDirecte (supabase, { userId, propertyId, bookingId, resa }) {
  const verrou = await poserVerrou(supabase, { userId, propertyId })
  if (!verrou.ok) {
    return verrou.raison === 'occupe'
      ? { ok: false, raison: 'verrou_occupe', message: 'Une autre saisie est en cours sur ce bien.' }
      : { ok: false, raison: 'verrou_indisponible', erreur: verrou.erreur,
          message: "Le verrou de securite est indisponible. Rien n'a ete modifie." }
  }

  try {
    const dispo = await verifierDisponibilite(supabase, {
      userId, propertyId, arrival: resa.arrival, departure: resa.departure,
      exclure: String(bookingId), ignorerToken: String(bookingId)
    })
    if (!dispo.ok) {
      return {
        ok: false, raison: dispo.raison, conflits: dispo.conflits, unites: dispo.unites,
    ...
    // Meme routage que la creation : par le provider du bien, jamais « channex »
    // en dur. L'ecriture CRS n'existe pas cote Beds24.
    if (dispo.provider !== 'channex') {
      return { ok: false, raison: 'provider_sans_ecriture',
               message: `La modification directe n'est pas disponible sur ce bien (${dispo.provider}).` }
    }
    const { getProvider } = require('./channels')
    const canal = getProvider(dispo.provider)
    let envoi = await canal.updateBooking(bookingId, propertyId, resa)

    // 403 = app booking_crs absente (parc anterieur au 6 septembre 2026). On
    // installe et on rejoue UNE fois. Sur un PUT le rejeu est doublement sur :
    // un 403 signifie que rien n'a ete modifie, et l'ecriture est idempotente.
    if (!envoi.ok && envoi.status === 403) {
      console.warn('[resa-directe] app booking_crs absente sur', propertyId, '— installation a la volee')
      const inst = await canal.installerCRS(propertyId)
      if (inst.ok) envoi = await canal.updateBooking(bookingId, propertyId, resa)
    }

    if (!envoi.ok) {
      return { ok: false, raison: 'echec_crs', status: envoi.status, erreurs: envoi.erreurs,
               message: "La modification n'a pas pu etre transmise. Rien n'a ete modifie." }
    }

    // Les NOUVELLES nuits sont tenues jusqu'a ce que le feed les remonte.
    // Les anciennes ne sont PAS liberees ici : tant que le cœur porte encore
    // l'ancien sejour, liberer ses nuits les afficherait libres alors qu'elles
    // sont encore comptees comme occupees par le snapshot d'avant. Elles
    // expirent seules (TTL 20 min), et la purge de `poserIntentions` les
    // ramasse — un marqueur de trop ne cree jamais de surreservation, alors
    // qu'un marqueur de moins, si.
    await poserIntentions(supabase, { userId, propertyId, nuits: dispo.nuits, token: String(bookingId) })
    return { ok: true, bookingId: String(bookingId), nuits: dispo.nuits }
  } finally {
    await libererVerrou(supabase, verrou.cle, verrou.jeton)
  }
}
