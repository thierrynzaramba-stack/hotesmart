...
// ⚠ ON N'ANALYSE PAS UN MESSAGE VIEUX DE PLUS DE 30 JOURS — DECISION DE
// THIERRY, 14 SEPTEMBRE 2026.
//
// LE CURSEUR EST SUR `created_at` (voir plus bas) PRECISEMENT POUR VOIR LES
// MESSAGES IMPORTES TARDIVEMENT. C'est le bon choix pour un message recent
// qu'un canal nous livre en retard. Mais le rattrapage des fils OTA change
// l'echelle : l'import d'un bien ramene d'un coup des mois de conversation, et
// chacune de ces lignes, inseree aujourd'hui, passait chez le modele — avec des
// detections de proprete sur des sejours clos depuis longtemps, a valider a la
// main une par une.
//
// Mesure : le rattrapage d'Ofuro Futari a insere 35 messages sortants et
// 9 entrants reellement nouveaux d'un seul coup ; celui de Colomiers en portera
// ~107. Et l'import devient RECURRENT : ce cas cessera d'etre exceptionnel.
//
// CE QUE CETTE BORNE COUTE, ET C'EST ASSUME : si un import ramene un message de
// trois mois portant une vraie plainte de proprete, on ne la detectera pas. Le
// sejour est clos, l'avis est deja ecrit ou ne le sera pas. Le module le dit de
// lui-meme : « un signalement pendant le sejour a de la valeur PENDANT le
// sejour » — c'est la raison d'etre de sa periode horaire.
//
// ⚠ LE FILTRE EST DANS LA BOUCLE, PAS DANS LA REQUETE. En SQL, un lot dont TOUS
// les messages sont anciens ne rendrait aucune ligne : `dernierTraite` resterait
// nul, le curseur ne bougerait pas, et la file se bloquerait sur eux a chaque
// passage — indefiniment. Ici on les compte, on les saute, ET on avance.
const AGE_MAX_MS = 30 * 24 * 60 * 60 * 1000
...
async function classerMessages (results, deps = {}) {
  ...
  const bilan = { lus: 0, detectes: 0, ecrits: 0, sans_bien: 0, erreurs: 0, trop_anciens: 0 }
  ...
  for (const msg of file) {
    ...
    // Trop vieux : aucun appel au modele, et le curseur AVANCE (cf. AGE_MAX_MS).
    // ⚠ `sent_at` ABSENT = ON ANALYSE. « Je ne sais pas quand ce message a ete
    // ecrit » n'est pas « il est vieux » : se taire par defaut ferait rater un
    // signalement reel pour une donnee manquante.
    if (msg.sent_at && (maintenant() - new Date(msg.sent_at).getTime()) > AGE_MAX_MS) {
      bilan.trop_anciens++
  ...
  // ⚠ ON PARLE AUSSI QUAND ON N A RIEN ANALYSE — CORRECTIF DE REVIEW.
  // Un saut par anciennete n incremente pas `lus` : un lot entierement ancien
  // ne produisait donc AUCUNE ligne de journal. Le rattrapage de Colomiers
  // insere ~107 messages anciens — on aurait vu passer plusieurs cycles muets,
  // impossibles a distinguer de « il n y avait rien ». C est la lecon deja
  // payee : une erreur avalee derriere un cron a 200.
  if (bilan.lus > 0 || bilan.trop_anciens > 0) {
    console.log('[messages-classify] bilan', JSON.stringify(bilan))
  }
