...
const { trouverBienParIdProvider } = require('./bien-du-provider')
...
// ⚠ MEME RESOLUTION QUE LE WEBHOOK, ET POUR LA MEME RAISON.
// Ce poll lit LE MEME feed que `api/channel-webhook.js`. Resoudre le bien sur la
// seule cle source y etait bien plus grave : quand aucun bien n'est trouve, ce
// cron ACKE la revision — elle disparait DEFINITIVEMENT du feed. Pendant une
// bascule, une reservation arrivee sur la propriete cible aurait donc ete purgee
// par le filet de securite lui-meme. C'est le critere de rollback du plan (« une
// reservation OTA qui n'arrive pas dans le coeur sous 30 min »).
...
async function saveRevision(rev, revisionId) {
  ...
  // Ambigu = on n'attribue pas au hasard, et surtout on N'ACKE PAS : la revision
  // doit rester dans le feed jusqu'a ce qu'un humain tranche.
  if (owner.ambigu) {
    console.error('[cron-feed] identifiant AMBIGU, revision laissee dans le feed', providerPropertyId)
    return { saved: false, reason: 'ambiguous_property', owner: null }
  }
  ...
  const res = await saveBookingSnapshot(supabase, {
    ...
    bookingId,
    // ⚠ La cle du COEUR, pas l'identifiant recu : tous les lecteurs
    // (calendrier, menage, `nuitsOccupees`) interrogent par `provider_property_id`.
    propertyId: owner.provider_property_id,
    provider:   'channex',
    snapshot,
    // Le payload brut : sans lui, seul le polling conserverait un raw et le flux
    // temps reel Channex (le chemin nominal) n'en aurait aucun. `snapshot` reste
    // fourni : le writer ne remappe pas, il ne fait que stocker la source a cote.
    //
    // ⚠ `id` est FORCE au booking id, pour que la colonne `raw` porte toujours la
    // meme forme. api/channel-events.js y ecrit `{ id: <booking id>, ...attributs }`
    // (via getReservations) tandis qu'ici `rev` est une booking_revision dont l'id
    // est celui de la REVISION. Sans cette normalisation, `raw.id` designerait
    // tantot une revision tantot une reservation selon le dernier chemin ayant
    // ecrit — et les deux formes se chasseraient l'une l'autre a chaque passage,
    // declenchant un UPDATE du raw sans qu'aucun contenu n'ait bouge.
    // `revision_id` reste present dans les attributs Channex : rien n'est perdu.
...
async function pollChannelFeed(results) {
  ...
  while (page <= MAX_PAGES) {
    ...
    for (const item of list) {
      ...
      // db_error et ambiguous_property : on N'ACK PAS -> repris au prochain poll
    }
