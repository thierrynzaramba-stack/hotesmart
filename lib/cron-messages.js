...
const { envoyerEmailVoyageur, sujetPour, compterEchecs, noterEchecEmail,
        MAX_TENTATIVES_EMAIL } = require('./email-guestflow')
...
async function sendGuestMessage(beds24Key, property, booking, message, options = {}) {
  ...
  if (decision.canal === CANAL.EMAIL) {
    if (!ENVOI_EMAIL_BRANCHE) {
      // Garde de survie : l'interrupteur est ouvert depuis l'etape 3, mais le
      // refermer doit rester un geste sans danger, pas une panne.
      return { ok: false, differe: true, canal: decision.canal, motif: decision.motif,
               destinataire: decision.destinataire, error: 'canal e-mail coupe' }
    }

    // ⚠ LE COMPTE PROPRIETAIRE DU BIEN, PAS L'APPELANT. C'est sa cle Brevo, ses
    // credits, son domaine d'expedition. Sans `userId`, on n'envoie pas : deviner
    // le compte ici ferait partir un message sur la cle de quelqu'un d'autre.
    const userId = options.userId || property?.user_id || null
    if (!userId) {
      return { ok: false, canal: decision.canal, motif: decision.motif, permanent: true,
               error: 'compte proprietaire inconnu — envoi e-mail impossible' }
    }

    const envoi = await envoyerEmailVoyageur({
      userId,
      destinataire: decision.destinataire,
      sujet: options.sujet || sujetPour(options.eventType, property?.name),
      texte: message,
      // Pour qu'un quota Brevo epuise s'annonce AVEC le bien concerne.
      propertyId: property?.id, propertyName: property?.name,
      // ⚠ SANS LUI, PAS D'ADRESSE DE REPONSE : le voyageur repondrait a l'hote
      // dans sa boite, et sa reponse n'entrerait jamais dans le fil.
      bookingId
    })
    return {
      ok: envoi.ok === true,
      ...
      destinataire: decision.destinataire,
      expediteur: envoi.expediteur || null,
      // `permanent` commande l'abandon cote appelant : une adresse invalide ne
      // deviendra pas valide en reessayant toutes les cinq minutes.
      permanent: envoi.ok ? false : envoi.permanent === true,
      // ⚠ `quota` DOIT TRAVERSER. Il etait perdu ici, donc la branche « quota »
      // de `faut_il_abandonner_email` etait morte : un forfait Brevo journalier
      // epuise se faisait COMPTER comme un echec ordinaire et le message etait
      // abandonne au bout de 25 minutes, alors que le quota repart a minuit.
      // Constat de review — le test du 429 n'exercait que le module d'envoi,
      // jamais le chemin qui le traverse.
      quota: envoi.ok ? false : envoi.quota === true,
      error: envoi.ok ? null : envoi.raison
    }
  }
...
// ─── Qu'un e-mail rate ne parte pas en boucle, ni en silence ─────────────────
// Rend `true` quand l'appelant doit ABANDONNER (et donc journaliser pour arreter
// le rejeu), `false` quand il doit laisser le message repartir au prochain tick.
//
// Trois issues, et elles ne se confondent pas :
//   PERMANENT  (adresse invalide, expediteur non verifie, rien de configure)
//              -> aucun essai n'y changera rien. On abandonne tout de suite et on
//                 previent l'hote : c'est un geste de sa part qu'il faut.
//   QUOTA      (429/402) -> on repassera. NON COMPTE : un forfait journalier
//              epuise rend 429 toute la journee, et un plafond l'abandonnerait en
//              vingt minutes alors qu'il se retablit a minuit. Il a deja son
//              signalement propre, qui ne sonne qu'une fois.
//   TRANSITOIRE (5xx, reseau) -> on repassera, mais on COMPTE. Au plafond, on
//              abandonne et on le dit : un message qui rate depuis 24 h ne part
//              plus, et personne ne doit l'apprendre par hasard.
async function faut_il_abandonner_email (sendRes, { userId, property, bookingId, templateId, quoi }) {
  const { reportIncident } = require('./founder-notify')
  const detail = `${quoi} (réservation ${bookingId}) : ${fmtSendErr(sendRes)}`

  if (sendRes?.permanent) {
    console.error(`[Messages] e-mail ABANDONNE (echec definitif) ${bookingId} : ${fmtSendErr(sendRes)}`)
    await reportIncident('email_voyageur_abandon', {
      userId, propertyId: property?.id != null ? String(property.id) : null,
      propertyName: property?.name, threshold: 1,
      detail: `${detail} — echec definitif, le message NE PARTIRA PAS sans un geste de votre part.`
    })
    return true
  }

  if (sendRes?.quota) {
    console.warn(`[Messages] e-mail reporte (quota Brevo) ${bookingId} — non compte dans le plafond`)
    return false
  }

  await noterEchecEmail({
    userId, propertyId: property?.id, propertyName: property?.name,
    bookingId, templateId, raison: fmtSendErr(sendRes)
  })
  const tentatives = await compterEchecs(bookingId, templateId)
  if (tentatives >= MAX_TENTATIVES_EMAIL) {
    console.error(`[Messages] e-mail ABANDONNE apres ${tentatives} tentatives ${bookingId}`)
    await reportIncident('email_voyageur_abandon', {
      userId, propertyId: property?.id != null ? String(property.id) : null,
      propertyName: property?.name, threshold: 1,
      detail: `${detail} — ${tentatives} tentatives en 24 h, on arrete. Le message NE PARTIRA PAS.`
    })
    return true
  }
  console.warn(`[Messages] e-mail en echec ${bookingId} (tentative ${tentatives}/${MAX_TENTATIVES_EMAIL}) : ${fmtSendErr(sendRes)}`)
  return false
}
...
async function checkAndSendTemplate(userId, beds24Key, property, booking, template, results) {
  ...
  if (propMode === 'auto') {
    ...
    const sendRes = await sendGuestMessage(beds24Key, property, booking, message,
      { userId, eventType: template.event_type })

    if (!journalAvantEnvoi && !sendRes?.ok) {
      // RIEN N'EST ECRIT TANT QUE RIEN N'EST PARTI : ni journal, ni fil. Un fil
      // qui afficherait un message non delivre est le faux vert que ce chantier
      // ferme — l'hote y lisait « envoye » pendant que Channex rendait 422.
      results?.errors?.push({ context: 'send_message', property_id: String(property.id),
        booking_id: bookingId, error: sendRes?.error || 'echec envoi e-mail' })

      const abandon = sendRes?.differe
        ? false
        : await faut_il_abandonner_email(sendRes, { userId, property, bookingId,
            templateId: template.id, quoi: `E-mail « ${template.event_type} »` })

      // ⚠ L'ABANDON S'ECRIT. Sans cette ligne, un message qu'on a decide de ne
      // plus envoyer repartirait quand meme a chaque tick — le plafond ne
      // plafonnerait rien.
      if (abandon) {
        await noterEnvoi(supabase, { userId, bookingId, templateId: template.id,
          empreinte: codeOtaBrut(booking) })
    ...
    await recordMessage({
      ...
      canal:         decision.canal === CANAL.EMAIL ? 'email' : 'ota'
    })
  } else {
  ...
  for (const template of templates) {
    ...
    if (propMode === 'auto') {
      ...
      if (!journalAvantEnvoi && !sendRes?.ok) {
        ...
        //
        // ⚠ ET ICI, L'EVENEMENT EST ONE-SHOT. Contrairement au cron des templates,
        // rien ne rejouera ce `booking_confirmed` : un echec non definitif n'aura
        // pas de seconde chance, quoi qu'en dise le plafond. On le DIT donc en
        // erreur de cycle, meme quand on n'abandonne pas formellement.
        results?.errors?.push({ context: 'send_message_one_shot', property_id: String(property.id),
          booking_id: bookingId,
          error: `${eventType} non envoye et NON REJOUABLE : ${sendRes?.error || 'echec envoi e-mail'}` })

        // ⚠ ICI, ON N'UTILISE PAS LE PLAFOND — ET C'EST LE CONTRAIRE D'UN OUBLI.
        // `faut_il_abandonner_email` compte les tentatives pour decider quand
        // renoncer. Ce chemin-ci n'en aura JAMAIS qu'une : l'evenement est marque
        // traite juste apres, quoi qu'il arrive. Attendre cinq tentatives qui ne
        // viendront pas, c'est ne jamais alerter — et avant ce chantier,
        // `send_failure` remontait des la deuxieme occurrence. On aurait donc
        // remplace une alerte qui marchait par un compteur inatteignable.
        //
        // Un echec ici est un abandon par nature, quota compris : le quota se
        // retablira, pas l'evenement. On previent donc tout de suite.
        if (!sendRes?.differe) {
          const { reportIncident } = require('./founder-notify')
          await reportIncident('email_voyageur_abandon', {
            userId, propertyId: String(property.id), propertyName: property.name, threshold: 1,
            detail: `E-mail « ${eventType} » (réservation ${bookingId}) : ${fmtSendErr(sendRes)} — `
              + 'cet envoi n\'a pas de seconde chance, le message NE PARTIRA PAS.'
          })
        }
        continue
      }
...
module.exports = {
  noterEnvoi,
  // Exportee pour les scripts de rattrapage : recopier la requete `knowledge`
  // ailleurs, c'est perdre son filtre `type = 'fixed'` et son « je leve plutot
  // que de rendre {} » — deux gardes payees cher (cf. l'en-tete de la fonction).
  knowledgeDuBien,
