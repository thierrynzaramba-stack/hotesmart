...
// Vider la main de l'hote d'un bien : au retour en pilotage calendrier, les
// prix de l'hote vivent de nouveau directement dans le calendrier, et une
// ligne survivante ressortirait « votre prix » a la prochaine activation
// (releve en review).
async function viderPrixHote (supabase, { userId, propertyId }) {
  if (!UUID_RE.test(String(userId || '')) || !UUID_RE.test(String(propertyId || ''))) return { ok: false }
  const { error } = await supabase.from('prix_hote').delete().eq('user_id', userId).eq('property_id', propertyId)
  if (error && !tableAbsente(error)) { console.error('[prix_hote] vider', propertyId, error.message); return { ok: false } }
  return { ok: true }
}

module.exports = { prixHoteDuBien, poserPrixHote, retirerPrixHote, purgerPrixHotePasses, viderPrixHote }
