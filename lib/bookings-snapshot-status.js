...
// cancelled : annulée PAR LE VOYAGEUR OU L'OTA, ne génère rien.
...
// demapped  : neutralisée PAR LE SYSTÈME lors d'un démappage / d'une migration.
//             N'occupe rien, ne génère rien — exactement comme `cancelled`.
//
// ⚠ POURQUOI `demapped` N'EST PAS `cancelled`, ET C'EST UNE DEMANDE EXPLICITE
// DE THIERRY (10 septembre 2026). Les annulations servent de matière aux
// STATISTIQUES : taux d'annulation par canal, par période, par logement. Ranger
// sous le même mot « le voyageur s'est décommandé » et « nous avons débranché ce
// logement d'un OTA » fausserait ces chiffres POUR TOUJOURS, et sans laisser
// aucun moyen de les séparer après coup — les 15 séjours neutralisés à la
// migration des deux biens de Bagnères auraient compté comme des renoncements
// de voyageurs.
//
// Toute statistique d'annulation doit donc compter `cancelled` SEUL, jamais
// `cancelled` + `demapped`.
const STATUS = {
  CONFIRMED: 'confirmed',
  CANCELLED: 'cancelled',
...
// Une annulation VOYAGEUR, celle qui compte dans les statistiques. `demapped`
// n'en fait pas partie : c'est le système qui a débranché le logement.
function estAnnulationVoyageur(snapshot, defaultProvider) {
  return readStatus(snapshot, defaultProvider) === STATUS.CANCELLED
}
